...
/* This is due to a bug in the i386 compiler.  The "const" should not
   be applied to the whole construction that the type is applied to;
   as a result of the bug, all the macroes that attempt to assign a
   value to any component of the fail_stack array will cause a compiler
   error.  It doesn't really matter whether or not the elements point
   to const-characters; for safety reasons I disabled this for the
   i386vr4 compiler only. */
#ifdef i386
     ...
     struct re_pattern_buffer *bufp;
{
  ...
  while (p != pend || !FAIL_STACK_EMPTY ())
	...
	handle_on_failure_jump:
          ...
          if (p + j < pend)
            {
              if (!PUSH_PATTERN_OP ((unsigned char*) (p + j), fail_stack))
                return -2;
            }
          else
            bufp->can_be_null = 1;
