...
DECLARE (m4_dumpdef);
DECLARE (m4_errprint);
#if !defined( DOS ) && !defined( __GCC960 )
/* opens a pipe to a process and sends command -- this is nonsense on DOS or in gcc960 code */
DECLARE (m4_esyscmd);
#endif
DECLARE (m4_eval);
DECLARE (m4_format);
...
#if !defined( DOS ) && !defined( __GCC960 )
/* opens a pipe to a process and sends command -- this is nonsense on DOS and in gcc960 code. */
  ...
  { "mppexit",		FALSE,	FALSE,	m4_mppexit },
  { "mppwrap",		FALSE,	FALSE,	m4_mppwrap },
...
/* default argument for define - if no first arg, define as 0 */
/* set up in "builtin_init()" function */
struct token_data default_arg;
...
builtin_init (void)
{
  ...
  /* default argument for define - if no first arg, define as 0 */
  default_arg.type = TOKEN_TEXT;
  default_arg.u.u_t.text = "0";
}
...
{
  token_data *alt_argv[3];
  if (argc == 2) {
      alt_argv[0] = argv[0];
      alt_argv[1] = argv[1];
      alt_argv[2] = &default_arg;
      define_macro (3, alt_argv, SYMBOL_INSERT);
  } else {
...
m4_errprint (struct obstack *obs, int argc, token_data **argv)
{
  ...
  obstack_1grow (obs, '\0');
  error_out ("%s", (char *) obstack_finish (obs));
}
 ...
 * EOF is seen, and tracing macro calls.  That is: "mppexit", "mppwrap",
 ...
 */
static void
m4_mppexit (struct obstack *obs, int argc, token_data **argv)
{
  int exit_code = 0;
 ...
 */
static void
m4_mppwrap (struct obstack *obs, int argc, token_data **argv)
{
  if (no_gnu_extensions)
    {
      obstack_grow (obs, ARG (1), strlen (ARG (1)));
...
m4_index (struct obstack *obs, int argc, token_data **argv)
{
  ...
  if (l2 > 0) /* don't try to find strings in the null string */
  {
    last = ARG (1) + l1 - l2;
    ...
    /* the second arg is a null string, return failure */
    retval = -1;
  }
