...
divert_init (void)
{
  ...
  for (i = 0; i < ndiversion; i++)
    divtab[i] = NULL;
  divtab[0] = output;  /* this has been initialized by output_init already */
		       /* or by parse_args, if -o specifies a file instead */
}
...
/*
 * Change the standard output stream to FP.
 */
static void
set_file (FILE *fp)
{
...
/*
 * Change the standard output to file NAME.  If NAME is NULL, standard output
 * is reverted to stdout, and if empty standard output is discarded.
 * Return TRUE iff the output stream was changed.
 */
boolean
set_output (char *name)
{
  FILE *fp;
...
undivert_all (void)
{
  ...
  output = divtab[0];  /* make sure things go to the base diversion */
