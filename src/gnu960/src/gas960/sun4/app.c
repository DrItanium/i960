...
/* static const char rcsid[] = "$Id: app.c,v 1.12 1995/06/11 03:25:22 rdsmithx Exp $"; */
...
/*
 * This function is parameterized as the first arg to do_scrub_next_char.
 * So is scrub_to_file().  When we are producing a listing, we'll write
 * every char to a temp file.  We don't want to do that when not writing
 * a listing file.
 */
int scrub_from_file_normal() 
{
	register int 	c = getc(scrub_file);
#if defined ( DOS )
	/* Remove CTRL-Z's (decimal 26) from the input stream.  
	 * To facilitate this, the stream was opened in binary
	 * mode.  So we must also ignore CTRL-M's ('\r').
	 */
	while ( c == '\r' || c == 26 )
	{
		c = getc(scrub_file);
...
#else
	/* Unix; CTRL-Z is just like any other char */
#endif
 	return c;
}
...
int scrub_from_file_listing() 
{
	...
	/* Unix; CTRL-Z is just like any other char */
#endif
	echo_char_to_listing(c);
	return c;
}

void scrub_to_file_normal(ch)
int ch;
{
	ungetc(ch,scrub_file);
} /* scrub_to_file() */

void scrub_to_file_listing(ch)
int ch;
{
...
void (*unget)();
{
	...
	case 6:
		...
		switch( ch ) 
		{
			/* This is neet.  Turn "string more string" 
			 * into "string\n  more string"
			 */
		case '\n':
/* FIXME-SOMEDAY: the following "neet" logic ends up putting a 0x5c, 0x6d 
 * ('\', 'n') into the string; just about the weirdest thing you could do. 
 * But there's no time to mess with it (sigh) 
 */
			(*unget)('n');
			add_newlines++;
			return '\\';

/* The following also works; and puts a 0x0a ('\n') into the string,
 * which is still not right, but better than what was here ... must
 * test VERY thoroughly, though, to avoid breaking something else.
 *			add_newlines++;
 *			return 'n';
 */
		case '"':
		...
		default:
			as_warn("Ignoring unknown escape '\\%c' in string",ch);
			break;
...
recycle:
	...
	case LEX_IS_TWOCHAR_COMMENT_1ST:
		...
		if ( ch2 != EOF && lex[ch2] == LEX_IS_TWOCHAR_COMMENT_2ND ) 
		{
			for( ; ; ) 
			{
				...
				while( ch2!=EOF && (lex[ch2] != LEX_IS_TWOCHAR_COMMENT_2ND) );
				
				while( ch2!=EOF && (lex[ch2] == LEX_IS_TWOCHAR_COMMENT_2ND) )
					ch2=(*get)();
				
				if( ch2==EOF || lex[ch2] == LEX_IS_TWOCHAR_COMMENT_1ST )
					break;
				(*unget)(ch2);
			}
			if( ch2==EOF )
	...
	case LEX_IS_ONECHAR_QUOTE:
		...
		case EOF:
			...
			break;
		case '\\':
			/* Looks like a character constant */
		{
			int i = 0;
			char *c = out_buf;
			char *save_inbuf;
			...
			for ( ; i < 20; ++i, ++c )
			{
				*c = ch;
				...
				{
				case '\'':
					/* Use the routine that parses character
					 * constants for strings.
					 */
					*(c+1) = 0;
					save_inbuf = input_line_pointer;
					input_line_pointer = out_buf;
					sprintf (out_buf, "%d", next_char_of_string());
					input_line_pointer = save_inbuf;
					i = 99;
					break;
				case '\n':
					as_bad("Illegal character constant.");
					i = 99;
					break;
				case EOF:
					as_bad("End of file unexpected.");
