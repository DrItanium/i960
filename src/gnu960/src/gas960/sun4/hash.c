...
/* static const char rcsid[] = "$Id: hash.c,v 1.5 1994/08/11 19:19:56 peters Exp $"; */
				...
				/* return handle (address of struct hash) */
{
  ...
  if (( room = (struct hash_entry *) xmalloc( sizeof(struct
						    hash_entry)*((1<<START_POWER) + 1) ) ) != NULL)
				/* +1 for the wall entry */
    {
      if (( retval = (struct hash_control *) xmalloc(sizeof(struct
							   hash_control)) ) != NULL)
	{
     ...
     char * description;
{
  ...
  retval = xmalloc(strlen(answer)+1);
  if (!retval)
    {
      error("room");
    }
