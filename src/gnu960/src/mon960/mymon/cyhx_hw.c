...
*
* 19dec95       snc - uncommented init_squall and fixed LED diagnostic
*    		      sequence
*
*/
 ...
 * board identifier
 *********************************************************************/
int arch = ARCH_HX;            /* Processor type */
char arch_name[32] = "HX";     /* add speed and mem size in cpu_identify() */

#if BIG_ENDIAN_CODE
    char target_common_name[] = "cyhxbe";  /* Informal target name */
#else
    char target_common_name[] = "cyhx";       /* Informal target name */
#endif
...
const CONTROL_TABLE boot_control_table = {
    ...
    0x4000,         /* ICON - dedicated mode, level detected, enabled,
             	     * mask unchanged, not cached, fast */

    /* -- Groups 2-5 -- Bus Configuration Registers */
    REGION_0_CONFIG, REGION_1_CONFIG,
    REGION_2_CONFIG, REGION_3_CONFIG,
    REGION_4_CONFIG, REGION_5_CONFIG,
    REGION_6_CONFIG, REGION_7_CONFIG,
    REGION_8_CONFIG, REGION_9_CONFIG,
    REGION_A_CONFIG, REGION_B_CONFIG,
    REGION_C_CONFIG, REGION_D_CONFIG,
    REGION_E_CONFIG, REGION_F_CONFIG,
...
board_reset(void)
{
    ...
    set_pending(0);
    asm("ld _boot_g0, g11");
    send_sysctl(0x300, (unsigned int)reinit, (unsigned int)&rom_prcb);
