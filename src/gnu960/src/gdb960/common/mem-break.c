...
#if defined(IMSTG)
/* Default stub for targets that do not support hardware break/watchpoints.
   Return -1 to indicate no support. */
int
memory_insert_hw_breakpoint (addr, save)
    CORE_ADDR addr;
    char *save;
{
    return -1;
...
int
memory_remove_hw_breakpoint (addr, save)
    CORE_ADDR addr;
    char *save;
{
    return -1;
...
int
memory_insert_watchpoint (addr, len, type)
    CORE_ADDR addr;
    int len;
    int type;
{
...
int
memory_remove_watchpoint (addr, len, type)
    CORE_ADDR addr;
    int len;
    int type;
{
...
/* Default stub for targets that do not support hardware watchpoints.
   Return 0 to effect a nop in the calling code. */
CORE_ADDR
memory_stopped_data_address()
{
    return 0;
}
#endif
...
/* This gives the target the ability to change the breakpoint on the fly. */
int set_break_insn(insn)
    unsigned char *insn;
{
    memcpy(break_insn, insn, sizeof break_insn);
}
#endif /* IMSTG */
