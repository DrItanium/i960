...
#if defined(IMSTG)
  int  	      (*to_insert_hw_breakpoint) PARAMS ((CORE_ADDR, char *));
  int 	      (*to_remove_hw_breakpoint) PARAMS ((CORE_ADDR, char *));
  int  	      (*to_insert_watchpoint) PARAMS ((CORE_ADDR, int, int));
  int 	      (*to_remove_watchpoint) PARAMS ((CORE_ADDR, int, int));
  CORE_ADDR   (*to_stopped_data_address) PARAMS ((void));
#endif
...
#if defined(IMSTG)
  int	        to_hardware_breakpoint_limit;
  int	        to_hardware_watchpoint_limit;
#endif
...
#if defined(IMSTG)
/* Insert a hardware breakpoint at address ADDR in the target machine.
   SAVE is a pointer to memory allocated for saving the
   target contents.  It is guaranteed by the caller to be long enough
   to save "sizeof BREAKPOINT" bytes.  Result is 0 for success, or
   an errno value.  */
...
/* Remove a hardware breakpoint at address ADDR in the target machine.
   SAVE is a pointer to the same save area 
   that was previously passed to target_insert_breakpoint.  
   Result is 0 for success, or an errno value.  */
...
/* Insert a watchpoint at address ADDR in the target machine.
   LEN is the number of bytes that the watchpoint is expected to span.
   TYPE tells the target what kind of watchpoint this is.
   Result is 0 for success, or an errno value.  */

#define	target_insert_watchpoint(addr, len, type)	\
	(*current_target->to_insert_watchpoint) (addr, len, type)

/* Remove a watchpoint at address ADDR in the target machine.
   LEN is the number of bytes that the watchpoint is expected to span.
   TYPE tells the target what kind of watchpoint this is.
   Result is 0 for success, or an errno value.  */
...
/* Hardware access (read/write) watchpoint stopped address */

#define target_stopped_data_address() \
	(*current_target->to_stopped_data_address) ()

/* Allows different hardware (within same target type) to have different
   numbers of hardware breakpoint/watchpoint registers.  */

#define target_hardware_breakpoint_limit \
	current_target->to_hardware_breakpoint_limit

#define target_hardware_watchpoint_limit \
	current_target->to_hardware_watchpoint_limit
...
#if defined(IMSTG)
/* Include default stubs for hardware break/watchpoint functions (real ones
   provided by the target code if hw break/watchpoints are supported) */
extern int
memory_insert_hw_breakpoint PARAMS ((CORE_ADDR, char *));
extern int
memory_remove_hw_breakpoint PARAMS ((CORE_ADDR, char *));
extern int
memory_insert_watchpoint PARAMS ((CORE_ADDR, int, int));
extern int
memory_remove_watchpoint PARAMS ((CORE_ADDR, int, int));
extern CORE_ADDR
memory_stopped_data_address PARAMS ((void));
#endif
