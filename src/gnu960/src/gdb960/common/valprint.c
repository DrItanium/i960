...
#if defined(IMSTG)
/* If nonzero, causes (char *) to print string contents also */

int autoderef = 1;		/* Controls printing of char * variables. */
#endif
     ...
     GDB_FILE *stream;
{
  ...
  if (TYPE_LENGTH (type) > sizeof (LONGEST))
	...
	/* The target endianness is not known until runtime */
...
#if defined(IMSTG)	/* I960 EXTENDED-PRECISION */
/* Intel i960 long doubles change this routine a lot.
 * See below for original gdb 4.10 version.
 */
     ...
     GDB_FILE *stream;
{
  ...
  {
    long lsw, mw, msw;   /* least significant word, middle word and, most significant word. */
    /* Is the sign bit 0?  */
    int nonnegative,code;
	...
	code = 0;	      
	memcpy ((char *) &msw, valaddr, sizeof (msw));
	/* target -> host.  */
	SWAP_TARGET_AND_HOST (&msw, sizeof (float));
	nonnegative = msw >= 0;
	is_nan = ((((msw >> 23) & 0xFF) == 0xFF) 
		  && 0 != (msw & 0x7FFFFF));
	msw &= 0x7fffff;
	lsw = mw = 0;
      }
	...
	if ( TARGET_BIG_ENDIAN )
	{
	  memcpy (&mw, valaddr+4,  sizeof (mw));
	  memcpy (&msw, valaddr+0, sizeof (msw));
        }
	else
#endif
	{
	  memcpy (&mw, valaddr+0,  sizeof (mw));
	  memcpy (&msw, valaddr+4, sizeof (msw));
  	}
	SWAP_TARGET_AND_HOST (&mw, sizeof (mw));
	SWAP_TARGET_AND_HOST (&msw, sizeof (msw));
	nonnegative = msw >= 0;
	is_nan = (((msw >> 20) & 0x7ff) == 0x7ff
		  && ! ((((msw & 0xfffff) == 0)) && (mw == 0)));
	msw &= 0xfffff;
	lsw = 0;
      }
      ...
      {
	/* It's IEEE Extended precision */ 
	code = 2;
	memcpy (&lsw, valaddr+0, sizeof(lsw));
	memcpy (&mw, valaddr+4, sizeof(mw));
        memcpy (&msw, valaddr+8, sizeof (msw));
	SWAP_TARGET_AND_HOST (&lsw, sizeof (lsw));
	SWAP_TARGET_AND_HOST (&mw, sizeof (mw));
	SWAP_TARGET_AND_HOST (&msw, sizeof (msw));
	nonnegative = (msw & 0x8000) != 0x8000;
	is_nan = ((msw & 0x7fff) == 0x7fff) && ((mw & 0x7ffffff) || lsw);
      }
	...
	/* The meaning of the sign and fraction is not defined by IEEE.
	   ...
	   signaling and quiet NaN's.  */
        static char *formats[3] = {"-NaN(0x%x)","-NaN(0x%x%08x)","-NaN(0x%x%08x%08x)"};
	fprintf_filtered (stream, formats[code] + nonnegative,
			    msw, mw, lsw);
	return;
      }
  }
  ...
  if (inv)
    ...
    fprintf_filtered (stream, len <= sizeof(float) ? "%.7g" : "%.12g", doub);
}

#else /* Not IMSTG: original gdb version */
...
#if defined(IMSTG)
  /* Intel gdb960 extension; allow (char *) to just print as ptr, not string */
  add_show_from_set
    (add_set_cmd ("autoderef", class_support, var_boolean, (char *)&autoderef,
		  "Set whether to print char pointers as strings.",
		  &setprintlist),
     &showprintlist);
#endif
