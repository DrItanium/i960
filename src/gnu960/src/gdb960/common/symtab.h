...
#if defined(IMSTG)
/* Yeah, we are spoilsports; we are not the least bit interested in chill. */
...
struct block
{
  ...
  /* This flag will be set iff the block was created for a function that
     could *potentially* be inlined somewhere in the current module.
     The flag is checked in make_blockvector, where abstract instance blocks
     that are never inlined are removed from the blockvector so as not to
     create confusion when searching for symbols, etc. */

  unsigned char abstract_instance;
#endif
...
#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
#define BLOCK_ABSTRACT_INSTANCE(bl) (bl)->abstract_instance
#endif
...
enum address_class
{
  ...
  /* For DWARF, or other debugging formats that provide generalized
     descriptions of value location.  In optimized code, for example,
     a symbol's location may change several times within the section
     of code of interest to the user.

     This address class means "I don't know where the value lives until
     you ask me".  The location list itself will be stored in the 
     location_indirect field of a struct symbol.  It will be computed on
     the fly, using the location evaluator stored in the class_indirect
     field of a struct symbol. */

  LOC_INDIRECT,

  /* Same as LOC_INDIRECT but it is an argument. */
...
#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
/* This structure is used by debug formats, notably Dwarf, that have 
   support for function inlining.  One of these would be stored in the
   inlined_list member of struct symbol if and only if the symbol 
   represents a function that was inlined at least once in the same
   module in which it is defined.

   The structure is malloc'ed.  (FIXME: memory leak?)

   The primary use of this is in decode_line_1. */
struct inlin
{
    /* The symbol respresenting the starting address of the concrete inline */
    struct symbol *concrete_instance;
...
struct symbol
{
  ...
  /* Function to find the address class on demand, when address class is 
     INDIRECT (implemented for Dwarf version 2 only, so far) */
  enum address_class (*class_indirect) PARAMS ((struct symbol *, CORE_ADDR));

  /* Pointer to Dwarf location list -- used for INDIRECT addr_class */
  PTR location_indirect;

#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
  /* Used by Dwarf or other debug formats that support inlining.  If I am
     a function that is an abstract instance, this is the list of the 
     concrete instances. */
  struct inlin *inline_list;

  /* If I am a concrete instance, this points to the abstract instance. */
  struct symbol *abstract_origin;
#endif
...
#define SYMBOL_CLASS_INDIRECT(symbol, pc) \
    ((symbol)->class == LOC_INDIRECT || (symbol)->class == LOC_INDIRECT_ARG ? \
     (symbol)->class_indirect(symbol, pc) : (symbol)->class)
#define SYMBOL_TYPE(symbol)		(symbol)->type
#define SYMBOL_LINE(symbol)		(symbol)->line
#define SYMBOL_BASEREG(symbol)		(symbol)->aux_value.basereg

#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
#define SYMBOL_INLINE_LIST(symbol)	(symbol)->inline_list
#define SYMBOL_ABSTRACT_ORIGIN(symbol)	(symbol)->abstract_origin
#endif

...
#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
    /* Some debug formats (notably DWARF V2) support column information 
       in the line number table.  If so, we can use it to be more selective 
       in decode_line_1. */

    int linetable_has_columns;
#endif
...
#if defined(IMSTG)
/* When position-independent code is requested, line numbers
   must be relocated accordingly. */
extern void
relocate_linetab PARAMS ((struct objfile *));
#endif
...
decode_line_spec PARAMS ((char *, int, int));

extern struct symtabs_and_lines
decode_line_spec_1 PARAMS ((char *, int, int));

extern struct symtabs_and_lines
decode_line_1 PARAMS ((char **, int, int, struct symtab *, int, char ***));
