...
#if defined(IMSTG)
/* Append the symbol name and address offset to hex addresses when
   disassembling if set. */
static int print_symbolic_disas = 1;
#endif
...
#if defined(IMSTG)  /* i960 LONG_DOUBLE */
      else if (len == SIZEOF_LONG_DOUBLE)
     ...
     int from_tty;
{
  ...
  /* For debug info like Dwarf2, that supports location lists, 
     we need to evaluate the symbol's location using a live pc,
     not statically.  

     Furthermore, we must use the pc active in the selected frame, 
     not just the starting pc of the selected block. */

  if (selected_frame && selected_frame->next)
      /* Not innermost frame; use pc stored in frame */
      pc = selected_frame->virtual_pc;
  else
      /* Innermost frame; use current pc */
      pc = read_pc();
...
address_info_1(sym, pc)
    ...
    CORE_ADDR pc;
{
    enum address_class addr_class = SYMBOL_CLASS_INDIRECT (sym, pc);
    long val = SYMBOL_VALUE (sym);
    long basereg = SYMBOL_BASEREG (sym);
     ...
     GDB_FILE *stream;
{
  ...
  int args_printed = 0;
  CORE_ADDR pc = fi->pc;      /* For Dwarf LOC_INDIRECT locations */
  ...
  for (i = 0; i < nsyms; i++)
      ...
      case LOC_REF_ARG:
	{
	 ...
	 stack offsets in them.  */
      ...
      case LOC_INDIRECT_ARG:
	    /* FIXME: including CONST and CONST_BYTES is not exactly correct,
	       because it may falsely include a CONST local in the parameter
	       list.  But leaving them off means that we won't be able to
	       print CONST parameters, which gcc2 does for some inlines. */
      case LOC_CONST:
      case LOC_CONST_BYTES:
#endif
	break;
	...
	continue;
      ...
      switch (SYMBOL_CLASS_INDIRECT (sym, pc)) {
      case LOC_ARG:
      case LOC_REF_ARG:
	{
	  long current_offset = SYMBOL_VALUE (sym);
      ...
      if (*SYMBOL_NAME (sym))
	{
	  ...
	  if (SYMBOL_CLASS_INDIRECT (nsym, pc) == LOC_REGISTER)
	    {
	...
	fprintf_filtered (stream, ", ");
	 ...
	 2 for each recurse.  */
      ...
      if (val && ! VALUE_OPTIMIZED_OUT(val))
	  ...
	  switch (c = *s++)
	    ...
	    case '\\':
	      *f++ = '\\';
	      break;
	    case 'a':
#if ! defined(IMSTG) && defined(__STDC__)
	      /* Our stupid ANSI compiler warns about this; \a == 7 anyway */
	      *f++ = '\a';
#else
	      *f++ = '\007';  /* Bell */
#endif
...
#if defined(IMSTG)  /* i960 LONG_DOUBLE */
	    if (TYPE_LENGTH (VALUE_TYPE (val_args[nargs])) == SIZEOF_LONG_DOUBLE)
	      VALUE_TYPE (val_args[nargs]) = builtin_type_long_double;
#endif
	  }
	nargs++;
	s = s1;
...
#if defined(IMSTG)
  add_show_from_set (
      add_set_cmd ("symbolic-disassembly", no_class, var_boolean,
		   (char *)&print_symbolic_disas,
	"Set printing of <symbol+1234> with disassembly.",
		   &setprintlist),
      &showprintlist);
#endif
