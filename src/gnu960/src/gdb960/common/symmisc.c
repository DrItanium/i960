     ...
     GDB_FILE *outfile;
{
  ...
  if (l)
    {
      fprintf_filtered (outfile, "\nLine table:\n\n");
      fprintf_filtered (outfile, "%10s%10s%18s\n",
			"Line", "Column", "Address");
      len = l->nitems;
      for (i = 0; i < len; i++)
	{
	  fprintf_filtered (outfile, "%10d%10d        ", 
			    l->item[i].line,
			    l->item[i].column);
     ...
     char *args;
{
  ...
  if (SYMBOL_NAMESPACE (symbol) == STRUCT_NAMESPACE)
	...
	fprintf_filtered (outfile, "%s ", SYMBOL_SOURCE_NAME (symbol));
      ...
      switch (SYMBOL_CLASS_INDIRECT (symbol, read_pc()))
	{
	case LOC_CONST:
     ...
     GDB_FILE *outfile;
{
  ...
  while (count-- > 0)
	...
	case LOC_LOCAL_ARG:
	  ...
	  break;
        case LOC_INDIRECT:
	  fputs_filtered ("indirect", outfile);
	  break;
        case LOC_INDIRECT_ARG:
	  fputs_filtered ("indirect arg", outfile);
	  break;
	case LOC_OPTIMIZED_OUT:
...
_initialize_symmisc ()
{
  ...
  std_err = stderr;
#if defined(IMSTG)
  /* For compatibility with gdb960 2.2. */
  add_com ("printsyms", class_obscure, maintenance_print_symbols,
	   "Print dump of current symbol definitions to file OUTFILE.\n\
Synonym for `maintenance print symbols'.");
#endif
}
