  ...
  for (elt = 0; elt < exp -> nelts; elt++)
    {
      ...
      if ( numelts == 0 )
      {
	  /* We're starting a new subexp.  Print the opcode. 
	     Default number of elements is 1. */
	  numelts = 1;
      switch (exp -> elts[elt].opcode)
	{
	  ...
	  case OP_LONG: opcode_name = "OP_LONG"; numelts = 4; break;
	  case OP_DOUBLE: opcode_name = "OP_DOUBLE"; break;
	  case OP_VAR_VALUE: opcode_name = "OP_VAR_VALUE"; numelts = 4; break;
	  ...
	  case STRUCTOP_STRUCT: opcode_name = "STRUCTOP_STRUCT"; 
	      temp = longest_to_int (exp->elts[elt + 1].longconst);
	      numelts = 4 + BYTES_TO_EXP_ELEM (temp + 1);
	      break;
	  case STRUCTOP_PTR: opcode_name = "STRUCTOP_PTR";
	      temp = longest_to_int (exp->elts[elt + 1].longconst);
	      numelts = 4 + BYTES_TO_EXP_ELEM (temp + 1);
	      break;
