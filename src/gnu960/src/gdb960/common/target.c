...
#if defined(IMSTG)
/* For IMSTG, ignore becomes target_ignore and is declared extern 
   so we can get at it in utils.c:quit() */
#define ignore target_ignore
#else
static
#endif
void
...
find_default_run_target PARAMS ((char *));
    ...
    /* This variable allows runtime checking of TARGET_BYTE_ORDER */
int target_byte_order = LITTLE_ENDIAN;
#endif
...
struct cmd_list_element *targetlist = NULL;
...
#if ! defined(IMSTG)
/* Make ignore function extern so we can get at it in utils.c:quit() */
static
#endif
void
ignore ()
...
#if defined(IMSTG)
  de_fault (to_insert_hw_breakpoint,	memory_insert_hw_breakpoint);
  de_fault (to_remove_hw_breakpoint,	memory_remove_hw_breakpoint);
  de_fault (to_insert_watchpoint,	memory_insert_watchpoint);
  de_fault (to_remove_watchpoint,	memory_remove_watchpoint);
  de_fault (to_stopped_data_address,	memory_stopped_data_address);
#endif
...
#if defined(IMSTG)
  de_fault (to_hardware_breakpoint_limit,   0);
  de_fault (to_hardware_watchpoint_limit,   0);
#endif
#undef de_fault
}
