...
int mon960_dis_asm_read_memory PARAMS ((bfd_vma, bfd_byte *, int, disassemble_info *));
     ...
     GDB_FILE *stream;
{
  ...
  if ( current_target == &mon960_ops )
      info.read_memory_func = mon960_dis_asm_read_memory;
  return print_insn_i960 (memaddr, &info);
}

#if ! defined(IMSTG)
/* In Intel's gdb960, these are in i960-tdep.c */
/****************************************/
/* MEM format				*/
/****************************************/
