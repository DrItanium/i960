     ...
     int args;
{
  ...
  struct symtab_and_line inline_sal;
  struct symbol *func;
  register char *funname = 0;
  enum language funlang = language_unknown;
  CORE_ADDR pc; /* pc used for looking up containing function */
  ...
  func = find_pc_function (fi->virtual_pc != fi->pc ? fi->virtual_pc : pc);
#else
  ...
  if (fi->inlin)
  {
      if (func && SYMBOL_ABSTRACT_ORIGIN(func))
      {
	  /* For inlines, we want to use the relative PC of the concrete
	   *out*-of-line function for figuring out the source file/line
	   coordinates, not the *inline* PC which will always get us the 
	   caller's source coordinates. */
	  int diff = fi->pc - BLOCK_START(SYMBOL_BLOCK_VALUE(func));
	  CORE_ADDR abstract_pc = 
	      BLOCK_START(SYMBOL_BLOCK_VALUE(SYMBOL_ABSTRACT_ORIGIN(func)))
		  + diff;
	  inline_sal = find_pc_line(abstract_pc, 0);
      }
      else
	  inline_sal = find_pc_line(pc, 0);
  }
#endif
  ...
  if (func)
	...
	/* For i960 leafprocs, a minimal symbol shows up 4 - 12 bytes 
	   ...
	   our chances on the pathological case described above; the 
  ...
  if (source >= 0 || !sal.symtab)
      ...
      if (sal.symtab && sal.symtab->filename)
	{
	  ...
	  printf_filtered ("%d", fi->inlin ? inline_sal.line : sal.line);
  ...
  if ((source != 0) && sal.symtab)
    {
      ...
      if (annotation_level)
	if (fi->inlin)
	  done = identify_source_line (inline_sal.symtab, inline_sal.line, 
				       mid_statement, fi->pc);
        else
      ...
      if (!done)
	{
	  ...
	  if (fi->inlin)
	      print_source_lines (inline_sal.symtab, inline_sal.line, 
				  inline_sal.line + 1, 0);
	  else
	      print_source_lines (sal.symtab, sal.line, sal.line + 1, 0);
	}
      if (fi->inlin)
	  current_source_line = max (inline_sal.line - lines_to_list/2, 1);
      else
	  current_source_line = max (sal.line - lines_to_list/2, 1);
    }
  if (source != 0)
    set_default_breakpoint (1, fi->pc, 
			    fi->inlin ? inline_sal.symtab : sal.symtab,
			    fi->inlin ? inline_sal.line : sal.line);
...
#define	MAXARGS	4
  ...
  switch (numargs)
	...
	/* We couldn't identify the frame as an existing frame, but
     ...
     default:
#ifdef IMSTG
      error("Frame command takes one arg.");
#else
#ifdef SETUP_ARBITRARY_FRAME
      return SETUP_ARBITRARY_FRAME (numargs, args);
#else
     ...
     int from_tty;
{
  ...
  print_address_numeric (get_fsr_address(&fsr,SP_REGNUM), 1, gdb_stdout);
  printf_filtered ("\n");
  count = 0;
  numregs = ARCH_NUM_REGS;
  for (i = 0; i < numregs; i++)
    if (get_fsr_address(&fsr,i) && i != SP_REGNUM)
      {
	if (count == 0)
	...
	print_address_numeric (get_fsr_address(&fsr,i), 1, gdb_stdout);
	count++;
      }
  if (count)
    puts_filtered ("\n");
     ...
     register GDB_FILE *stream;
{
  ...
  CORE_ADDR pc = frame->pc;	/* For Dwarf LOC_INDIRECT locations */
