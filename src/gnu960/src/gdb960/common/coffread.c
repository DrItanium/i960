...
#if defined(IMSTG)
/*
 * Used to control name changing for backward compatibility with older
 * gcc960 compilers.
 */
static int ic_name_rules;
#endif
...
static void
coff_record_line PARAMS ((int, int, CORE_ADDR));
...
#if defined(IMSTG)
/* Perform relocation on coff symbols that are affected by PIC/PID.
   Note that this routine does NOT operate on absolute symbols.  
   So if the user prints the value of a symbol like heap_base, the value
   will not be correct.  This is a macro for speed in the inner loop of
   read_coff_symtab.  */
#define relocate_coff_symbol(cs, objfile) \
    if ( cs->c_secnum > 0 ) \
	cs->c_value += ANOFFSET (objfile->section_offsets, cs->c_secnum - 1);
#endif
...
#if defined(IMSTG)
	  relocate_coff_symbol(cs, objfile);
#endif
		...
		if (local_symbols && context_stack_depth > 0)
		  {
		    ...
		    finish_block (0, &local_symbols, new->old_blocks,
				  new->start_addr, cs->c_value, objfile, 0);
		  }
    ...
    register int last_line;
{
  ...
  for (;;) {
    ...
    if (L_LNNO32 (&lptr) && L_LNNO32 (&lptr) <= last_line)
      coff_record_line (first_line + L_LNNO32 (&lptr), 0, lptr.l_addr.l_paddr);
    else
      break;
  } 
}
     ...
     struct objfile *objfile;
{
  ...
  if (name[0] == '_')
  {
    SYMBOL_NAME (sym) += offset;
    underscore_stripped = 1;
  }
#endif
  ...
  if (ISFCN (cs->c_type))
      ...
      switch (cs->c_sclass)
	{
	  ...
	  case C_LABEL:
#if defined(IMSTG)
            if (underscore_stripped && ic_name_rules)
		    SYMBOL_NAME(sym) -= 1;
            if (strcmp(SYMBOL_NAME(sym), "ic_name_rules.") == 0)
		    ic_name_rules = 1;
	    relocate_coff_symbol(cs, objfile);
#endif
	    break;
...
#if defined(IMSTG) /* Intel BIG_ENDIAN mods: make this decision at runtime */
	  ...
	  case C_AUTOARG:
	    SYMBOL_CLASS (sym) = LOC_LOCAL_ARG;
            /*
             * Hack for ic960 making .argptr an autoarg, and it looking
             * stupid in the argument list during a printout.
             */
            if (strcmp(SYMBOL_NAME(sym), ".argptr") == 0)
		    SYMBOL_CLASS (sym) = LOC_LOCAL;
	    add_symbol_to_list (sym, &local_symbols);
	    break;
#endif
	       ...
	       a register value.  gnu@adobe, 26Feb93 */
	{
	  ...
	  case C_ENTAG:
	    ...
	    SYMBOL_NAMESPACE (sym) = STRUCT_NAMESPACE;
#if defined(IMSTG)
            if (ic_name_rules && underscore_stripped)
		    SYMBOL_NAME(sym) -= 1;

            /*
             * Special hack to deal better with names as ic960 and
             * newer gcc960 requires them.  All tag_names except fakes
             * end in _number.  Strip this from the name so the user
             * can refer to it properly.
             */
            if (ic_name_rules)
            {
		    char *start = SYMBOL_NAME(sym);
		    char *name = start;
		    while (*name != '\0')
			    name ++;
		    ...
		    {
			    name --;
			    if (name != start && isdigit(*name))
			    {
				    name --;
				    while (name != start && isdigit(*name))
					    name --;
				    if (name != start && *name == '_')
					    *name = '\0';
			    }
		    }
            }
     ...
     int lastsym;
{
  ...
  register int n;
  char *name;
#if defined(IMSTG) && defined(NAMES_HAVE_UNDERSCORE)
  int offset = 1;
#else
  int offset = 0;
#endif
