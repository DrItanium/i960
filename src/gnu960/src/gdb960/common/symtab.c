...
static struct symtabs_and_lines
expand_symbolic_line_spec PARAMS ((struct symbol *, int));

static struct symtabs_and_lines
expand_numeric_line_spec PARAMS ((struct symtab_and_line, int, char ***));

static void
expand_canonical_line_spec PARAMS ((struct symtabs_and_lines, char ***));
#endif
      ...
      while (bot < top)
	{
	  ...
	  if (SYMBOL_NAMESPACE (sym) == namespace &&
	      SYMBOL_MATCHES_NAME (sym, name))
	    {
		  ...
		  SYMBOL_CLASS_INDIRECT (sym, BLOCK_START(block));
	      sym_found = sym;
	      
	      if (addr_class != LOC_ARG &&
		  addr_class != LOC_LOCAL_ARG &&
		  addr_class != LOC_REF_ARG &&
		  addr_class != LOC_REGPARM &&
		  addr_class != LOC_REGPARM_ADDR &&
		  addr_class != LOC_BASEREG_ARG)
		{
		  break;
		}
	    }
  ...
  ALL_SYMTABS (objfile, symtab)
      ...
      for (blocknum = GLOBAL_BLOCK; blocknum <= STATIC_BLOCK; blocknum++)
	{
	  ...
	  for (bot = 0; bot < top; bot++)
	    {
	      sym = BLOCK_SYM (block, bot);
	      switch (SYMBOL_CLASS_INDIRECT (sym, BLOCK_START(block)))
		{
		case LOC_STATIC:	
		case LOC_LABEL:	
...
#if defined(IMSTG)
/* Perform relocation on sections affected by PIC.  Currently used only
   for COFF and Elf.  This will only be called if the user has requested 
   -picoffset n.  Stolen from objfile_relocate.  */
void
relocate_linetab(objfile)
    struct objfile *objfile;
{
    struct symtab *s;
...
/* Helper function for decode_line_1.
   In the weird world of inlined functions, one symbol name may expand
   into more than one pc. */
static
struct symtabs_and_lines
expand_symbolic_line_spec (sym, funfirstline)
    struct symbol *sym;
    int funfirstline;
{
    struct symtabs_and_lines values;
    if (SYMBOL_ABSTRACT_ORIGIN(sym) != NULL)
    {
	/* We found an existing inlined instance.  For purposes of
	   decoding a line spec, use the abstract instance; you'll 
	   get the inlined instances also. */
	sym = SYMBOL_ABSTRACT_ORIGIN(sym);
    }
    if (SYMBOL_INLINE_LIST (sym) != NULL)
    {
	/* The function has been inlined at least once.
	   Return a list of concrete instances, including both 
	   out-of-line instances and inlined instances. */
	/* First count how many inlines there are. */
	int i = SYMBOL_BLOCK_VALUE(sym) ? 1 : 0;
	struct inlin *inl = SYMBOL_INLINE_LIST(sym); 
	for ( ; inl; ++i, inl = inl->next)
	    ;
	/* Malloc that many */
	values.nelts = i;
	values.sals = (struct symtab_and_line *) 
	    xmalloc (sizeof(struct symtab_and_line) * i);
	
	/* Find the concrete out-of-line instance if there is one */
	i = 0;
	if ( SYMBOL_BLOCK_VALUE(sym) )
	{
	    values.sals[0] = find_function_start_sal (sym, funfirstline);
	    i = 1;
	}
	/* For each inline, initialize a symtab_and_line and add
	   to the array to be returned. */
	for (inl = SYMBOL_INLINE_LIST(sym); inl; ++i, inl = inl->next)
	{
	    values.sals[i] = 
		find_function_start_sal(inl->concrete_instance, 0);
	}
    }
    else
    {
...
/* Helper function for decode_line_1.
   In the weird world of inlined functions and loop unrolling, 
   one line number may expand into more than one pc.  Take a given 
   sal and expand it into many sal's, each with same line, different PC. 

   NOTE: This assumes that your line number table has useful column
   numbers -- i.e. not all zeros.  They are used to tell the difference
   between two otherwise identical line numbers. */
static
struct symtabs_and_lines
expand_numeric_line_spec (sal, funfirstline, canonical)
    struct symtab_and_line sal;
    int funfirstline;
    char ***canonical;
{
    struct symtabs_and_lines values;
    struct linetable *lt = LINETABLE(sal.symtab);
    register struct linetable_entry *ln;
    register int i;
    register int nlines;
    register int line, col;
    int exact, match, hit, hits;
    CORE_ADDR lastpc = 0;

    if (lt == NULL)
	error("No line table found for symtab: %s\n", sal.symtab->filename);

    match = find_line_common (lt, sal.line, &exact);

    if (match == -1)
    {
	/* Not sure what the right thing to do is.  For now, just return 
	   with a single sal and hope that the caller's code degrades nicely. */
	values.nelts = 1;
	values.sals = (struct symtab_and_line *)
	    xmalloc(sizeof(struct symtab_and_line));
	*values.sals = sal;
    ...
    /* Now go through the linetable looking for exact matches with 
       the line number chosen by find_line_common.  The collection of
       these will become the symtabs_and_lines. */
    for (hits = 0, 
	 i = 0, 
	 nlines = lt->nitems, 
	 line = lt->item[match].line,
	 col = lt->item[match].column,
	 ln = &lt->item[0];
	 i < nlines; 
	 ++i, ++ln)
    {
	if (ln->line == line && ln->column == col)
	    ++hits;
    }
    
    values.sals = (struct symtab_and_line *)
	xmalloc (sizeof (struct symtab_and_line) * hits);
    for (i = 0, hit = 0, ln = &lt->item[0]; hit < hits; ++i, ++ln)
    {
	if (ln->line == line && ln->column == col)
	{
	    /* Check for duplicates; note that this test is only needed
	       because the line number list is sorted by pc, not by line.
	       If another list were maintained, sorted by line, you could
	       filter duplicates at readin time, not here. */
	    if (ln->pc == lastpc)
	    {
		--hits;
		continue;
	    ...
	    values.sals[hit].pc = lastpc = ln->pc;
	    values.sals[hit].line = sal.line;
	    values.sals[hit].symtab = sal.symtab;
	    values.sals[hit++].end = 0;
	}
    }
    
    values.nelts = hits;
    expand_canonical_line_spec (values, canonical);
    return values;
}

/* Helper function for decode_line_1.
   Like build_canonical_line_spec, but works on a list of sal's, 
   not just one. */
static void
expand_canonical_line_spec (sals, canonical)
    ...
    char ***canonical;
{
    ...
    char *filename;
    int i;
    struct symtab *s = sals.sals[0].symtab;
    ...
    *canonical = canonical_arr;
    ...
    {
	canonical_name = xmalloc (strlen (filename) + 20);
	sprintf (canonical_name, "%s:%d", filename, sals.sals[i].line);
	canonical_arr[i] = canonical_name;
    }
}
#endif /* IMSTG INLINED_BREAKPOINTS */
...
struct symtabs_and_lines
decode_line_1 (argptr, funfirstline, expand_inlines, default_symtab, default_line, canonical)
     char **argptr;
     int funfirstline;
     int expand_inlines;
...
*/
  ...
  if (q != *argptr && (*q == 0 || *q == ' ' || *q == '\t' || *q == ','))
	...
	s = default_symtab;
      ...
      val.pc = 0;
#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
      if (expand_inlines && s->linetable_has_columns)
	  values = expand_numeric_line_spec(val, funfirstline, canonical);
      else
      {
	  values.sals = (struct symtab_and_line *)
	      xmalloc (sizeof (struct symtab_and_line));
...
#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
	  if (expand_inlines && sym_symtab && sym_symtab->linetable_has_columns)
	      values = expand_symbolic_line_spec(sym, funfirstline);
	  else
	  {
	      values.sals = (struct symtab_and_line *)
		  xmalloc (sizeof (struct symtab_and_line));
...
struct symtabs_and_lines
decode_line_spec (string, funfirstline, expand_inlines)
     char *string;
     int funfirstline;
     int expand_inlines;
{
