...
void
finish_block (symbol, listhead, old_blocks, start, end, objfile, abstract)
     struct symbol *symbol;
     struct pending **listhead;
     struct pending_block *old_blocks;
     CORE_ADDR start, end;
     struct objfile *objfile;
     int abstract; /* 1 == this block is an abstract inlined instance */
{
 ...
 /* Superblock filled in when containing block is made */
  ...
  BLOCK_ABSTRACT_INSTANCE (block) = abstract;
      ...
      if (BLOCK_SUPERBLOCK (pblock->block) == NULL)
	{
	  ...
	  if (BLOCK_START (pblock->block) < BLOCK_START (block) ||
	      BLOCK_END   (pblock->block) > BLOCK_END   (block))
	    {
		  ...
		  /* Only complain if this is NOT an abstract instance
		     because we will fill in the corrent low/hi pc values
		     for abstract instances later. */
	      if (!BLOCK_ABSTRACT_INSTANCE(block)
		  && !BLOCK_ABSTRACT_INSTANCE(pblock->block))
	      {
#endif
		if (symbol)
		{
...
#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
  /* We may have some useless blocks hanging around at this point.
     These pop out of Dwarf when the compiler emits an abstract instance
     for an inlined function that never gets inlined.  Probably this should
     be fixed in the compiler, but it wastes only a little time now. 
     This doesn't create a memory leak because all blocks are allocated
     on the objfile's symbol_obstack. (meaning they get freed when/if the
     objfile is replaced)

     Another similar block will come from a C lexical block, for which 
     a concrete version is seen later.  The abstract version is not 
     needed and can cause symbol lookup failures. */
  register struct pending_block *prev;
  for (next = pending_blocks; next; prev = next, next = next->next)
  {
      if (BLOCK_ABSTRACT_INSTANCE(next->block))
      {
	  if (next == pending_blocks)
	  {
	      pending_blocks = next->next;
	  }
	  else
	  {
	      prev->next = next->next;
...
void
record_line (subfile, line, column, pc)
     register struct subfile *subfile;
     int line;
     int column;
     CORE_ADDR pc;
{
  ...
  e->line = line; e->column = column; e->pc = pc;
}
  ...
  if (context_stack_depth > 0)
    {
      ...
      finish_block (cstk->name, &local_symbols, cstk->old_blocks,
		    cstk->start_addr, end_addr, objfile, 0);
     ...
     CORE_ADDR valu;
{
  ...
  new->name = NULL;
#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
  new->concrete_instance_root = NULL;
#endif
