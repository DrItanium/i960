...
That operation is not available on integers of more than %d bytes.",
  ...
  p = TARGET_BIG_ENDIAN ? startaddr : endaddr - 1;
     ...
     int len;
{
  if (len == sizeof (float))
    ...
    {
	/* This is a workaround for gdb's lack of long double support.
	 * Convert the target's 3 words of memory into a double for
	 * debugging purposes.  ext_to_doub() handles target endianness.
	 */
	double retval;
	extended_to_double(addr, &retval);
	return retval;
    }
#endif
  else
     ...
     double val;
{
  if (len == sizeof (float))
  ...
  else if (len == SIZEOF_LONG_DOUBLE)
    {
      unsigned tmp[4];
/*       SWAP_TARGET_AND_HOST (&val, sizeof (val)); */
      double_to_extended(&val, tmp);
      memcpy (addr, (void *) tmp, len);
    }
#endif
  else
    {
...
CORE_ADDR
find_saved_register (frame, regnum, alias_reg)
     FRAME frame;
     int regnum,*alias_reg;
{
  ...
  *alias_reg = regnum;
  ...
  if (REGISTER_IN_WINDOW_P(regnum))
	...
	frame1 = frame;	
      ...
      return get_fsr_address(&saved_regs,regnum);	/* ... which might be zero */
    }
  ...
  while (1)
	...
	break;
      ...
      get_frame_saved_regs (fi, &saved_regs);
      if (get_fsr_address(&saved_regs,regnum))
	      addr = get_fsr_address(&saved_regs,regnum);
      else if (has_fsr_reg_alias(&saved_regs,regnum))
	       *alias_reg = get_fsr_reg_alias(&saved_regs,regnum);
    }
     ...
     enum lval_type *lval;
{
  ...
  if (optimized != NULL)
    *optimized = 0;
  addr = find_saved_register (frame, regnum, &alias_reg);
  if (addr != 0)
    {
      if (lval != NULL)
	*lval = lval_memory;
  ...
  else
    {
      if (lval != NULL)
	*lval = lval_register;
      addr = REGISTER_BYTE (alias_reg);
      if (raw_buffer != NULL)
	      read_register_gen (alias_reg, raw_buffer);
    }
  if (addrp != NULL)
    *addrp = addr;
}
...
#if defined(IMSTG)
/* NUM_REGS is a variable, not a constant */
char register_valid[TOTAL_POSSIBLE_REGS];
#else
...
symbol_read_needs_frame (sym)
     struct symbol *sym;
{
  switch (SYMBOL_CLASS_INDIRECT (sym, read_pc()))
    {
     ...
     FRAME frame;
{
  ...
  addr_class = SYMBOL_CLASS_INDIRECT (var, frame ? frame->pc : read_pc());
  ...
  switch ( addr_class )
	...
	if ( addr_class == LOC_REGPARM_ADDR )
	  {
	    addr =
	      value_as_pointer (value_from_register (lookup_pointer_type (type),
						     SYMBOL_VALUE (var),
						     frame));
	    SWAP_TARGET_AND_HOST(&addr, sizeof(CORE_ADDR));
...
#if defined(IMSTG) /* Intel BIG_ENDIAN mods */
	/* The target endianness is not known until runtime */
	if ( TARGET_BIG_ENDIAN && len < REGISTER_RAW_SIZE (regnum) )
	{
