...
#if defined(IMSTG)
  /* We need to run the function prologue to have any prayer of getting 
     sp and fp right. */
  sals = decode_line_spec_1 (arg, 0, 1);
#else
  sals = decode_line_spec_1 (arg, 1, 1);
#endif
  if (sals.nelts != 1)
    {
     ...
     int from_tty;
{
  ...
  if (!func)
    ...
    {
#if defined(IMSTG)
	/* To make until work for optimized code also,
	   the source line of step_range_end MUST be greater than 
	   that of current line. */
	unsigned int current_line;
	struct symtab_and_line next_sal;
	...
	while (sal.end)
	{
	    next_sal = find_pc_line(sal.end, 0);
	    if (next_sal.end == 0 || 
		(next_sal.line > current_line && next_sal.end > pc))
		break;
	    else
		sal = next_sal;
	} 
     ...
     int from_tty;
{
  ...
  selected = get_frame_info (selected_frame);
  function = find_pc_function (selected->pc);

  /* Find frame we will return to. */
  prev_frame = get_prev_frame (selected_frame);
  if (prev_frame == 0)
    error ("\"finish\" not meaningful in the outermost frame.");

  clear_proceed_status ();

  prev = get_frame_info (prev_frame);
#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
  /* If the selected frame is a virtual inlined frame, return to the 
     "virtual pc" (i.e. one insn past the range of the inline) 
     not the real pc (i.e. somewhere inside the range of the inline) */
  if (selected->inlin)
  {
      sal = find_pc_line (prev->virtual_pc, 0);
      sal.pc = prev->virtual_pc;
  }
  else
  {
      sal = find_pc_line (prev->pc, 0);
      sal.pc = prev->pc;
  }      
#else
  sal = find_pc_line (prev->pc, 0);
...
#ifdef IMSTG /* Artistic license */
  add_com ("stepi", class_run, stepi_command,
	   "Step one instruction exactly.  Step into subroutine calls.\n\
Argument N means do this N times (or till program stops for another reason).");
  add_com_alias ("si", "stepi", class_alias, 0);

  add_com ("nexti", class_run, nexti_command,
	   "Step one instruction.  Step over, not into, subroutine calls.\n\
Argument N means do this N times (or till program stops for another reason).");
...
Upon return, the value returned is printed and put in the value history.");
  ...
  add_com ("next", class_run, next_command,
	   "Step program until it reaches a different source line. Step over,\n\
not into, subroutine calls.  Argument N means do this N times (or till\n\
program stops for another reason).");
  add_com_alias ("n", "next", class_run, 1);

  add_com ("step", class_run, step_command,
	   "Step program until it reaches a different source line.  Step into\n\
subroutine calls.  Argument N means do this N times (or till program\n\
