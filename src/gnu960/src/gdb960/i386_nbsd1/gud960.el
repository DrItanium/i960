;;; gud.el --- Grand Unified Debugger mode for gdb, gdb960, sdb, dbx, or xdb
;;;            under Emacs
...
;;; History of argument lists passed to gdb960.
(defvar gud-gdb960-history nil)

(defun gud-gdb960-massage-args (file args)
  (cons "-fullname" (cons file args)))
...
(defun gud-gdb960-marker-filter (string)
  (save-match-data
    (setq gud-marker-acc (concat gud-marker-acc string))
    (let ((output ""))
...
(defun gud-gdb960-find-file (f)
  (find-file-noselect f))

(defvar gdb960-minibuffer-local-map nil
  "Keymap for minibuffer prompting of gdb960 startup command.")
(if gdb960-minibuffer-local-map
    ()
  (setq gdb960-minibuffer-local-map (copy-keymap minibuffer-local-map))
  (define-key
    gdb960-minibuffer-local-map "\C-i" 'comint-dynamic-complete-filename))

;;;###autoload
(defun gdb960 (command-line)
  "Run gdb960 on program FILE in buffer *gud-FILE*.
The directory containing FILE becomes the initial working directory
and source-file directory for your debugger."
  (interactive
   (list (read-from-minibuffer "Run gdb960 (like this): "
			       (if (consp gud-gdb960-history)
				   (car gud-gdb960-history)
				 "gdb960 ")
			       gdb960-minibuffer-local-map nil
			       '(gud-gdb960-history . 1))))
  (gud-overload-functions '((gud-massage-args . gud-gdb960-massage-args)
			    (gud-marker-filter . gud-gdb960-marker-filter)
			    (gud-find-file . gud-gdb960-find-file)
			    ))
  ...
  (local-set-key "\C-i" 'gud-gdb960-complete-command)
  (setq comint-prompt-regexp "^(.*gdb960[+]?) *")
  (setq paragraph-start comint-prompt-regexp)
  (run-hooks 'gdb960-mode-hook)
  )
...
;; The completion process filter indicates when it is finished.
(defvar gud-gdb960-complete-in-progress)

;; Since output may arrive in fragments we accumulate partials strings here.
(defvar gud-gdb960-complete-string)

;; We need to know how much of the completion to chop off.
(defvar gud-gdb960-complete-break)

;; The completion list is constructed by the process filter.
(defvar gud-gdb960-complete-list)

(defvar gud-comint-buffer nil)

(defun gud-gdb960-complete-command ()
  "Perform completion on the GDB command preceding point.
This is implemented using the GDB `complete' command which isn't
available with older versions of GDB."
  (interactive)
  (let* ((end (point))
    ...
    ;; Find the word break.  This match will always succeed.
    (string-match "\\(\\`\\| \\)\\([^ ]*\\)\\'" command)
    (setq gud-gdb960-complete-break (match-beginning 2)
	  command-word (substring command gud-gdb960-complete-break))
    (unwind-protect
	(progn
	  ;; Temporarily install our filter function.
	  (gud-overload-functions
	   '((gud-marker-filter . gud-gdb960-complete-filter)))
	  ;; Issue the command to GDB.
	  (gud-basic-call (concat "complete " command))
	  (setq gud-gdb960-complete-in-progress t
		gud-gdb960-complete-string nil
		gud-gdb960-complete-list nil)
	  ;; Slurp the output.
	  (while gud-gdb960-complete-in-progress
	    (accept-process-output (get-buffer-process gud-comint-buffer))))
      ;; Restore the old filter function.
      (gud-overload-functions '((gud-marker-filter . gud-gdb960-marker-filter))))
    ;; Protect against old versions of GDB.
    (and gud-gdb960-complete-list
	 (string-match "^Undefined command: \"complete\""
		       (car gud-gdb960-complete-list))
	 (error "This version of GDB doesn't support the `complete' command."))
    ;; Sort the list like readline.
    (setq gud-gdb960-complete-list
	  (sort gud-gdb960-complete-list (function string-lessp)))
    ;; Remove duplicates.
    (let ((first gud-gdb960-complete-list)
	  (second (cdr gud-gdb960-complete-list)))
      (while second
	(if (string-equal (car first) (car second))
	    (setcdr first (setq second (cdr second)))
	  (setq first second
		second (cdr second)))))
    ;; Let comint handle the rest.
    (comint-dynamic-simple-complete command-word gud-gdb960-complete-list)))
    
;; The completion process filter is installed temporarily to slurp the
;; output of GDB up to the next prompt and build the completion list.
(defun gud-gdb960-complete-filter (string)
  (setq string (concat gud-gdb960-complete-string string))
  (while (string-match "\n" string)
    (setq gud-gdb960-complete-list
	  (cons (substring string gud-gdb960-complete-break (match-beginning 0))
		gud-gdb960-complete-list))
    (setq string (substring string (match-end 0))))
  (if (string-match comint-prompt-regexp string)
      (progn
	(setq gud-gdb960-complete-in-progress nil)
	string)
    (progn
      (setq gud-gdb960-complete-string string)
      "")))
