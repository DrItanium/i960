     ...
     enum noside noside;
{
  ...
  switch (op)
      ...
      if (noside == EVAL_AVOID_SIDE_EFFECTS)
	{
	  ...
	  switch (SYMBOL_CLASS_INDIRECT (sym, 0))
	    {
	    case LOC_CONST:
	    case LOC_LABEL:
    ...
    case OP_BOOL:
      (*pos) += 2;
#if defined(IMSTG)
	warning("Internal error: opcode OP_BOOL detected in evaluate_subexp");
	return 0;
#else
     ...
     enum noside noside;
{
  ...
  switch (op)
      ...
      if (noside == EVAL_AVOID_SIDE_EFFECTS)
	{
	  ...
	  enum address_class sym_class = SYMBOL_CLASS_INDIRECT (var, 0);
