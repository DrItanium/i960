...
struct frame_info
  {
    ...
    /* Same as pc, unless the "callee" of this frame is a virtual frame 
       associated with a function inline.  In that case the virtual PC 
       is the address one byte BEYOND the range of the inlined "callee".
       Use the real PC in backtraces, use the virtual PC for looking up
       function names/line numbers/symtabs.  This field is set (only)
       in create_new_frame and get_prev_frame_info. */
    CORE_ADDR virtual_pc;

    /* Nonzero if this frame is a virtual frame associated with a 
       function inline.  Set (only) in create_new_frame and
       get_prev_frame_info. */
    int inlin;
#endif
...
struct frame_saved_regs
  {
    ...
    /* For each register, address of where it was saved on entry to
       ...
       Additionally, if a register is aliased (in Elf/dwarf parlance, saved
       in another register), this indicates where it is saved.
  */

#if defined(IMSTG)
      /* I960 has more than one flavor; NUM_REGS is a variable. */

      struct fsr_val_struct {
#define FSR_ADDRESS        1
#define FSR_ALIAS_REG      2
#define FSR_SAME_VALUE     4
	  char fsr_flags;
	  CORE_ADDR fsr_value;
      } fsr_vals[TOTAL_POSSIBLE_REGS];

#define has_fsr_reg_alias(FSR,RN)   ((FSR)->fsr_vals[(RN)].fsr_flags & FSR_ALIAS_REG)

#define get_fsr_reg_alias(FSR,RN)   (((FSR)->fsr_vals[(RN)].fsr_flags & FSR_ALIAS_REG) ? \
				   (FSR)->fsr_vals[(RN)].fsr_value : 0)

#define is_fsr_same_value(FSR,RN)   ((FSR)->fsr_vals[(RN)].fsr_flags & FSR_SAME_VALUE)

#define get_fsr_address(FSR,RN)   (((FSR)->fsr_vals[(RN)].fsr_flags & FSR_ADDRESS) ? \
				   (FSR)->fsr_vals[(RN)].fsr_value : 0)

#define set_fsr_address(FSR,RN,A) ((get_fsr_address(FSR,RN) == (A)) ? 0 : (         \
				    (FSR)->fsr_vals[(RN)].fsr_flags = FSR_ADDRESS,  \
                                    (FSR)->fsr_vals[(RN)].fsr_value = (A)      ))

#define set_fsr_alias_reg(FSR,RN,A) (FSR)->fsr_vals[(RN)].fsr_flags = FSR_ALIAS_REG, \
                                    (FSR)->fsr_vals[(RN)].fsr_value = (A)

#define set_fsr_undefined(FSR,RN) (FSR)->fsr_vals[(RN)].fsr_flags = 0

#define set_fsr_same_value(FSR,RN) (FSR)->fsr_vals[(RN)].fsr_flags = FSR_SAME_VALUE
...
extern CORE_ADDR find_saved_register PARAMS ((FRAME, int, int *));
