...
/* Support for dumping the raw data from values in a human readable
   form.  */

int
dump_value(val, stream, note)
    struct value *val;
    GDB_FILE *stream;
    char *note;
{
    ...
    if (stream == NULL)
	stream = gdb_stdout;
    fprintf_filtered (stream, "Dump of value list @ ");
    gdb_print_address (val, stream);
    fprintf_filtered (stream, ", %s:\n", note ? note : "");
    
    fprintf_filtered (stream, "%16s  %8s  %6s  %10s  %4s  %12s\n", 
		      "Lval category", "Location", 
		      "Offset", "Type Size",
		      "Lazy", "Contents");
	...
	{
	case not_lval:
	    lval_name = "not_lval"; break;
	case lval_memory:
	    lval_name = "lval_memory";
	    if ( val->location.address == NULL )
		;
	    break;
	case lval_register:
	    lval_name = "lval_register"; break;
	case lval_internalvar:
	    lval_name = "lval_internalvar"; break;
	case lval_internalvar_component:
	    lval_name = "lval_internalvar_component"; break;
	case lval_reg_frame_relative:
	    lval_name = "lval_reg_frame_relative"; break;
	default:
	    lval_name = "<unknown>"; break;
	}

	fprintf_filtered (stream, "%16s  %8x  %6d  %10d  %4d  %12x\n", 
			  lval_name, val->location.address, 
			  val->offset, TYPE_LENGTH(val->type),
			  val->lazy, val->aligner.contents[0]);
    }
