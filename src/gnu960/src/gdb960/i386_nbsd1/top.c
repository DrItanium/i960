...
#include "language.h"
#if defined(IMSTG)
/* FIXME: logic of terminal.h doesn't work for DOS */
...
#include <sys/file.h>
#endif
#if !defined(IMSTG) || !defined(NO_SYS_PARAM)
#include <sys/param.h>
#endif
#include <sys/stat.h>
#include <ctype.h>
...
#if defined(DOS)
#include <direct.h> /* for _getdrive, _getcwd, _chdrive, _chcwd */
#endif
...
#if defined(IMSTG)
/* Some Intel-specific (gdb960) declarations. */
/* quiet and batch flags are used in remote-mon960.c */
int quiet;
int batch;

#if defined(DOS)
/* Flag found in ser-unix.c (not linked in DOS version of gdb960) */
int job_control = 0;

#endif /* IMSTG && DOS */
#endif /* IMSTG */

#ifdef SOLARIS_SIGNALS
   int sigint_longjmp;
#endif

/* isslash would typically be redefined in a DOS host config file */
#ifndef isslash
#define isslash(a) ((a) == '/')
#endif
     ...
     enum return_reason reason;
{
  quit_flag = 0;
  immediate_quit = 0;
#if defined(IMSTG) && defined(DOS) && defined(__HIGHC__)
  /* See comment at init_signals in top.c */
  ctrlbrk_flag = 0;
#endif
...
#ifdef SOLARIS_SIGNALS
  sigint_longjmp = (reason == RETURN_QUIT);
#endif
  ...
  if (setjmp (tmp_jmp) == 0)
  ...
  else
    {
      ...
      if (sigint_longjmp)
        {
          /*
           * We were interrupted by ^C and have just longjmp'd out of a
           * signal handler.  Solaris says if that we just longjmp'd out of
           * a signal handler that was setup with a call to sigaction(),
           * then the client must explicitly unblock the implicitly blocked
           * signal.  Refer to: 
           *
           * $ man -s5 signal
           *
           * The sigaction() init of interrupt handlers is repeatedly made
...
dump_mallinfo()
{
    ...
    printf("%-20s%d\n", "usmblks", mb.usmblks);
    printf("%-20s%d\n", "fsmblks", mb.fsmblks);
    printf("%-20s%d\n", "uordblks", mb.uordblks);
    printf("%-20s%d\n", "fordblks", mb.fordblks);
    printf("%-20s%d\n", "keepcost", mb.keepcost);
}
#endif
...
init_signals ()
{
...
#if defined(IMSTG) && defined(DOS) && defined(__HIGHC__)
  /* Work around severe problems with trying to trap SIGINT from CTRL-BREAK
     key on DOS (as opposed to CTRL-C).  Set it up as a direct Phar Lap 
     interrupt handler that just sets a flag.  Flag will be checked whenever
     QUIT macro is seen. */
  install_ctrlbrk_handler(ctrlbrk_handler, 64, &ctrlbrk_flag, 4);
#endif
     
#if defined(IMSTG) && defined(WIN95)
  /* CTRL-BREAK is considerably easier to handle on Win95.  Whenever you
     install/deinstall a handler for SIGINT, use the same handler for 
     SIGBREAK (a signal Microsoft uses especially for CTRL-BREAK) */
  signal (SIGBREAK, request_quit);
#endif
  ...
  /* If we initialize SIGQUIT to SIG_IGN, then the SIG_IGN will get
     ...
     to SIG_DFL for us.  */
#if defined(IMSTG) && !(defined(DOS) && defined(__HIGHC__))
  /* FIXME: This is screwing up CTRL-BREAK for as yet unknown reasons */
...
#if defined(IMSTG) && defined(DOS)
      /* Test for CTRL-C each time through the loop */
      QUIT;
#endif
...
{
#if defined(IMSTG)
  /* Defined in ver960.c (created by make each time gdb960 is relinked) */
     ...
     int from_tty;
{
  ...
  /* See comment at init_signals in main.c */
  de_install_ctrlbrk_handler();
#endif
  target_close((args && strstr(args, "-n")) || ( ! autoreset ));
#endif
  exit (0);
}
     ...
     int from_tty;
{
  ...
  char *p;
#if defined(DOS)  
  int save_curr_drive = 0;
  char *save_current_directory;
#endif
...
#if defined(DOS)
  /* Intercept N:\foo or N: or N:. before doing the chdir. 
     Some versions of chdir (notably Metaware's) will not change
     the default drive at the same time; so do it manually first. */
  if (dir[0] != ':' && dir[1] == ':')
  {
      /* Looks like a drive letter.  Change to the drive first,
	 and then everything that follows should work fine. */
      int desired_drive = toupper(dir[0]) - 'A' + 1; /* A=1, B=2, ... */
      save_curr_drive = _getdrive();
      if (_chdrive(desired_drive) < 0)
	  perror_with_name(dir);

      if (dir[2] == '\0' || (dir[2] == '.' && dir[3] == '\0'))
      {
	  /* dir is N: or N:. where N is a drive letter.
	     The convention in DOS or Windows is that a drive letter
	     by itself (or accompanied by ".") means the current directory
	     on the given drive.  Simplify everthing that follows by 
	     expanding into a full pathname now. */
	  char *desired_cwd = alloca(1024);
	  _getcwd(desired_cwd, 1024);
	  dir = savestring(desired_cwd, strlen(desired_cwd));
	  make_cleanup(free, dir);
      }
  }
#endif
  ...
  {
#if defined(DOS)
      if (save_curr_drive > 0)
	  /* Restore original default drive */
	  _chdrive(save_curr_drive);
#endif
      perror_with_name (dir);
  }

  /* Get rid of trailing slash, unless path is only "/" ("N:\" on DOS) */
  len = strlen (dir);
#if defined(DOS)
  if (isslash(dir[len-1]) &&
      (len > 3 || (len > 1 && dir[1] != ':')))
#else
  if (len > 1 && isslash(dir[len-1]))
#endif
      len -= 1;
  dir = savestring (dir, len);

  /* If dir starts with a slash, (or "N:\" on DOS) then replace existing 
     current_directory with new one.  Else append to existing one. */
#if defined(DOS)
  if (isslash(dir[0]) || (dir[0] != ':' && dir[1] == ':' && isslash(dir[2])))
#else
  if (isslash(dir[0]))
#endif
  ...
  {
      if (isslash(current_directory[strlen(current_directory) - 1]))
...
#if defined(DOS)
  /* There may be a drive letter at start of current_directory.
     If so, skip past it and then restore later so that following logic 
     will work for both DOS and Unix. */
  ...
  {
      if (isslash(p[0]) && p[1] == '.' && (p[2] == 0 || isslash(p[2])))
	  strcpy (p, p + 2);
	  ...
	  char *q = p;
	  while (q != current_directory && ! isslash(q[-1]))
	      --q;
...
#else
    history_filename = concat (current_directory, "/hist.gdb", NULL);
#endif
  }
...
#if defined(IMSTG)
  c = add_set_cmd ("editing", class_support, var_boolean, (char *)&command_editing_p,
