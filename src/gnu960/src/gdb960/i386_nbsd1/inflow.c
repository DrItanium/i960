...
#if !defined (HAVE_TERMIOS) && !defined (HAVE_TERMIO) && !defined (HAVE_SGTTY) && !defined (__GO32__) && !(defined(IMSTG) && defined(DOS))
#define HAVE_SGTTY
#endif
...
#if defined(IMSTG) && defined(DOS)
      /* This logic is not right, but it is better than relying on 
	 serial_fdopen, which always fails on DOS. */
      gdb_has_a_terminal_flag = quiet ? no : yes;
#else
...
#if ! defined(IMSTG) 
  /* (IMSTG) our target kill doesn't do anything; this is confusing */
