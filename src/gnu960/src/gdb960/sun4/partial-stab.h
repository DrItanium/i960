...
/* End of macro definitions, now let's handle them symbols!  */
	...
	record_it:
	  ...
	  SET_NAMESTRING();
#if defined(IMSTG)
	  /* Don't allow .text, .data, .bss into the minimal symtabs */
	  if (NAMESTRING_EQUALS(".text") || 
	      NAMESTRING_EQUALS(".data") ||
	      NAMESTRING_EQUALS(".bss"))
	      continue;
#endif
	bss_ext_symbol:
...
#ifdef DBXREAD_ONLY
#if defined(IMSTG)
	      /* Save the highest 'f' or 'F' value seen so far (see comment
		 at end of dbx_symfile_read) */
	      STORE_HIGHEST_FUNC_ADDRESS(CUR_SYMBOL_VALUE);
#endif
