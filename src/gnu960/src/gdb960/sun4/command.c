...
#if defined(IMSTG)
  (*c->function.sfunc) (arg, from_tty, c);
#else
     ...
     int from_tty;
{
  ...
  if ( arg )
  {
	  user_shell = xmalloc (strlen(arg) + 1);
	  strcpy(user_shell, arg);
  }
  else if ((user_shell = (char *) getenv ("SHELL")) == NULL &&
	   (user_shell = (char *) getenv ("COMSPEC")) == NULL)
      user_shell = "COMMAND";
