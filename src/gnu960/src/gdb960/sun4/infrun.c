...
#define CURRENTLY_STEPPING() \
  ...
  while (1)
	...
	case TARGET_WAITKIND_EXITED:
	  ...
	  if (w.value.integer)
#if defined(IMSTG)
	      /* Printing the exit code in OCTAL?  Why?  It's a signed int. */
...
#if !defined(IMSTG)
      /* This screws up when we call a hidden library function, e.g. 
	 float f1, f2, f3; f1 = f2 + f3; on the C-series processors.
	 FIXME: find another way to solve the avowed one-line function
