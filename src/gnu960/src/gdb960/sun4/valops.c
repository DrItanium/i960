  ...
  switch (VALUE_LVAL (toval))
    ...
    case lval_register:
	  ...
	  if (VALUE_BITSIZE (toval))
	      {
		  char buffer[sizeof (LONGEST)];
		  int len = REGISTER_RAW_SIZE (regno);
...
Can't handle bitfield which doesn't fit in a single register.");
	  ...
	  /* Assigning to the stack pointer, frame pointer, and other
	     ...
	     cause the frame cache to be out of date. */
	  switch (regno) {
      case PFP_REGNUM:
      case SP_REGNUM:
      case FP_REGNUM:
	      reinit_frame_cache ();
	      printf_filtered("Modificaton of pfp, fp or sp caused re-initialization of frames.\n");
...
#if defined(IMSTG)
  if (TYPE_CODE (type) == TYPE_CODE_FLT
      && TYPE_LENGTH (type) == SIZEOF_LONG_DOUBLE)
    return value_cast (builtin_type_double, arg);
#endif
  return arg;
}
