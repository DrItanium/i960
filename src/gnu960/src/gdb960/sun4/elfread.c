...
struct elfinfo {
  ...
  unsigned char dbversion;	/* 1 or 2, for Dwarf version 1 or 2 */
...
#if defined(IMSTG)
/* Perform relocation on elf symbols that are affected by PIC/PID.
   Used only on non-absolute symbols. */
#define relocate_elf_symbol(sym, objfile) \
   symaddr += ANOFFSET (objfile->section_offsets, sym->section->index);
#endif
...
/* We are called once per section from elf_symfile_read.  We
   ...
   UPDATE for DWARF 2: We're relying on the section names even more,
   to differentiate Dwarf 1 from Dwarf 2.  Dwarf 1 has ".debug" and
   ".line", Dwarf 2 has ".debug_info" and ".debug_line".
*/
     ...
     PTR eip;
{
  ...
  else if (STREQ (sectp -> name, ".debug_info"))
    {
      ei -> dbversion = 2;
    }
  else if (STREQ (sectp -> name, ".debug_line"))
    {
      ei -> dbversion = 2;
    }
  ...
  if (storage_needed > 0)
...
#ifdef DUMP_SYMTAB
      _dump_symtab(number_of_symbols, symbol_table, "before");
#endif
      ...
      for (i = 0; i < number_of_symbols; i++)
	{
	  ...
	  if (sym -> flags & BSF_FILE)
	    ...
	    {
#if defined(IMSTG)
		/* Strip out useless section name symbols. */
		if ( (sym->flags & BSF_LOCAL)
		    && STREQ(sym->name, sym->section->name) )
		    continue;
#endif
		...
		{
#if defined(IMSTG)
		  relocate_elf_symbol(sym, objfile);
#endif
...
static int suffix;
static
_dump_symtab(numsyms, symbol_table, str)
    int numsyms;
    asymbol **symbol_table;
    char *str;
{
    ...
    FILE *symdmp;
    char dmpfile[32];
    sprintf(dmpfile, "symdmp.%d", ++suffix);
    symdmp = fopen(dmpfile, "w");

    fprintf(symdmp, "\nDump of symbols %s bfd_canonicalize_symtab\n", str);

    for ( sym = symbol_table; numsyms; --numsyms, ++sym )
    {
	fprintf(symdmp, "%-40s%x\n", (*sym)->name, (*sym)->value);
    }
    fclose(symdmp);
}
#endif
     ...
     int mainline;
{
...
#ifdef IMSTG
  /* No such thing as a base offset for us; but we have both a text offset
     (picoffset) and data offset (pidoffset) */
  offset = 0;
#else
  ...
  if (ei.dbversion == 1 && ei.dboffset && ei.lnoffset)
    {
      /* DWARF 1 sections */
#if defined(IMSTG)
	error("Internal error: Dwarf version 1 not supported.");
#else
  ...
  if (ei.mdebugsect)
    {
	 ...
	 information.  */
#if defined(IMSTG)
      warning("Ignoring ECOFF section: .mdebug");
#else
     ...
     struct objfile *objfile;
{
  ...
  if (objfile -> sym_private != NULL)
    {
      /* Dwarf 2 uses this to store a pointer to libdwarf info */
