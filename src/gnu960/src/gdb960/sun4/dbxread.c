...
#if defined(IMSTG)
/* This is the highest 'f' or 'F' value seen so far (see comment
     at end of dbx_symfile_read) */
static CORE_ADDR highest_func_address;
#endif
...
#if defined(IMSTG)
  /* This is a workaround for the lack of a way to specify the size 
     of a function in stabs.  The *last* psymtab's texthigh is always
     absurdly large, encompassing all the library symbols.  Among other
     things, this breaks gdb's method of next-ing over library functions
     for which there is no debug info, e.g. printf.  To work around this
     problem, replace the last psymtab->texthigh value with the value of 
     the first minimal symbol whose address is greater than the last real
     symbol's address. */
  if (pst = find_pc_psymtab(highest_func_address))
  {
      struct minimal_symbol *msym =
	  lookup_minimal_symbol_by_pc(highest_func_address);
      if (msym)
      {
	  /* Find the *next* minimal symbol and use it for the end address
	     of the last real function, provided it's not off the end of 
	     the text section.  NOTE: this could be buggy if there's a
	     C or asm label within the last function in the partial symtab.
	     In that special case, this algorithm loses and a "disas foo" 
	     will not show all of "foo".  But that's better than the current 
	     situation where "disas foo" show all of "foo" plus all of the 
	     remainder of the text section. */
	  CORE_ADDR texthigh = SYMBOL_VALUE_ADDRESS(msym + 1);
	  if (texthigh > highest_func_address &&
	      texthigh < pst->texthigh)
	      pst->texthigh = texthigh;
      }
  }
#endif	 
  do_cleanups (back_to);
...
#if defined(IMSTG)
#define NAMESTRING_EQUALS(str) STREQ(namestring, str)
#define STORE_HIGHEST_FUNC_ADDRESS(address) \
    if (address > highest_func_address) highest_func_address = address;
#endif
     ...
     struct partial_symtab *pst;
{
  ...
  pst->symtab = end_symtab (text_offset + text_size, 0, 1, objfile,
			    SECT_OFF_TEXT);
  end_stabs ();
}
...
#define VARIABLES_INSIDE_BLOCK(desc, gcc_p) 0
      ...
      if (last_source_file)
	{
	  ...
	  end_symtab (valu, 0, 1, objfile, SECT_OFF_TEXT);
	  end_stabs ();
	}
    ...
    case N_SLINE:
	 ...
	 Enter it in the line list for this symbol table.  */
      ...
      last_pc_address = valu;	/* Save for SunOS bug circumcision */
#endif
      record_line (current_subfile, desc, 0, valu);
      break;
...
#if defined(IMSTG)
/* Include the notion of position-independent code and data in setting up
   section offsets.  If PIC or PID is not requested, set offsets to 0. */
     ...
     CORE_ADDR addr;
{
  ...
  objfile->num_sections = objfile->obfd->section_count;
  ...
  for ( s = objfile->obfd->sections; s; s = s->next )
    {
        if ( s->flags & SEC_CODE )
	    /* .text and user-named code sections */
	    ANOFFSET (section_offsets, s->index) = picoffset;
        else if ( s->flags & SEC_DATA || s->flags & SEC_ALLOC )
	    /* .data, .bss, and user-named data sections */
	    ANOFFSET (section_offsets, s->index) = pidoffset;
        else
	    ANOFFSET (section_offsets, s->index) = 0;
    }
