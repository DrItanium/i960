...
/* Here are 2 macros that can be redefined in a DOS host config file */
...
#if defined(DOS)
#include <direct.h> /* for _getdrive, _getcwd, _chdrive */
#endif
     ...
     register struct symtab *s;
{
  ...
  if (lookup_symbol ("main", 0, VAR_NAMESPACE, 0, NULL))
    {
      sals = decode_line_spec ("main", 1, 0);
      sal = sals.sals[0];
      free (sals.sals);
     ...
     char **which_path;
{
  ...
  do
	...
	name = tilde_expand (name);
      else if (!isslash(name[0]) && name[0] != '$'
#if defined(DOS)
	       /* name is either C: or C:\foo\bar */
	       && !(name[0] != ':' && name[1] == ':')
	       && !(name[1] == ':' && isslash(name[2]))
#endif
	       )
	...
	if (stat (name, &st) < 0)
	  {
	    ...
	    /* Metaware's stat won't find directories of the form F:\ */
     ...
     char **filename_opened;
{
  ...
  if (try_cwd_first || isslash(string[0]))
    {
      filename = string;
      fd = open (filename, mode, prot);
      if (fd >= 0 || isslash(string[0]))
	goto done;
      else
        {
	  /* Replaces strchr(string, '/') but also works on DOS */
	  char *c = string;
	  for ( ++c; *c; ++c )
	      if ( isslash(*c) )
		  goto done;
  ...
  /* ./foo => foo */
  while (string[0] == '.' && isslash(string[1]))
    string += 2;
  ...
  for (p = path; p; p = p1 ? p1 + 1 : 0)
	...
	filename[len] = 0;
      ...
      while (len > 0 && isslash(filename[len-1]))
       filename[--len] = 0;
  ...
  if (filename_opened)
    ...
    else if (isslash(filename[0]))
      *filename_opened = savestring (filename, strlen (filename));
#if defined(DOS)
    /* Also accept the beloved C:\path\name */  
    else if ((filename[1] == ':') && isslash(filename[2]))
      *filename_opened = savestring (filename, strlen (filename));
#endif /* DOS */
    else
      {
     ...
     int noerror;
{
  ...
  if (line < 1 || line > s->nlines)
    {
#if defined(IMSTG)
      /* warning, not error, in case there are still displays to be done. */
      warning ("Line number %d out of range; %s has %d lines.",
	       line, s->filename, s->nlines);
      if ( line < 1 )
	  line = 1;
      else if ( line > s->nlines )
	  return;
      if ( stopline > s->nlines )
	  stopline = s->nlines;
      current_source_line = line;
      first_line_listed = line;
#else
  ...
  if (*arg1 == ',')
      ...
      else
	{
	  if (dummy_beg)
	    ...
	    sals_end = decode_line_1 (&arg1, 0, 0, sal.symtab, sal.line, 0);
	  if (sals_end.nelts == 0) 
	    return;
	  if (sals_end.nelts > 1)
	    {
#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
		/* This test isn't quite right, but will work for most
		   non-C++ cases, and won't break C++ */
		int i;
		for (i = 1; i < sals.nelts; ++i)
		{
		    if (sals.sals[i].line != sals.sals[0].line)
			break;
		}
		if (i < sals.nelts)
		{
     ...
     int from_tty;
{
  ...
  if (arg == 0)
    ...
    {
      sals = decode_line_spec_1 (arg, 0, 1);
