...
int show_breakpoint_hit_counts = 0;
		...
		if (v->lval == lval_memory)
		  {
		    ...
		    /* The type parameter to target_insert_watchpoint means
		       you don't have to include breakpoint.h into target.h,
		       which would be ugly. */
		    type = 0;
...
#if defined(IMSTG)
		    /* NOTE: many value chains contain value types
		       too long to watch with mon960 watchpoints.
		       If we've made it this far we know there is at least
		       one value type that can be watched in 4 bytes. */
	  ...
	  if (v->lval == lval_memory)
	    {
	      ...
	      len = TYPE_LENGTH (VALUE_TYPE (v));
#if defined(IMSTG)
	      /* We only inserted the values whose types are 4 bytes or less.
		 See insert_breakpoints. */
	      if ( len <= REGISTER_SIZE )
#endif
...
#if defined(IMSTG) /* i960 HW_BREAKPOINTS */
      if ( bs->breakpoint_at->type == bp_hardware_breakpoint )
	  printf_filtered ("\nHardware Breakpoint %d, ", 
...
/*  We only print out user settable breakpoints unless the allflag is set. */
	...
	if (b->inlined)
	    printf_filtered ("inl %-10s ", bptypes[(int)b->type]);
	else
	...
	if (b->ignore_count)
	  {
	    ...
	    printf_filtered ("\tignore next %d hit%s\n", 
			     b->ignore_count, (b->ignore_count == 1 ? "" : "s"));
	  }
    ...
    int *other_type_used;
{
  ...
  *other_type_used = 0;
  ALL_BREAKPOINTS (b)
	...
	{
#if defined(IMSTG)
	    /* Add up ANY type of hardware watchpoint; they are all
	       the same to mon960 */
	    if ( b->type == bp_hardware_watchpoint 
		|| b->type == bp_read_watchpoint
		|| b->type == bp_access_watchpoint )
	    {
		/* Make sure to count all the values on the value chain */
		mark = value_mark();
		v = evaluate_expression(b->exp);
		i += can_use_hardware_watchpoint(v);
		value_free_to_mark(mark);
	    }
#else
          if (b->type == type) i++;
     ...
     struct breakpoint *b;
{
  ...
  switch (b->type)
    ...
    case bp_breakpoint:
#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
      printf_filtered ("Breakpoint %d%s", b->number, inlbuf);
#else
    ...
    case bp_hardware_breakpoint:
#if defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
      printf_filtered ("Hardware assisted breakpoint %d%s", b->number, inlbuf);
#else
...
#if defined(IMSTG)
/* To tell the difference between watch, awatch, and wwatch commands. */
static int i960_hardware_watchpoint_request;
#endif
  ...
  else
    {
	 ...
	 leaving default_breakpoint_* alone.  */
	  ...
	  && (!current_source_symtab
	      || (arg && (*arg == '+' || *arg == '-'))))
	sals = decode_line_1 (&arg, 1, 1, default_breakpoint_symtab,
			      default_breakpoint_line, &canonical);
      else
	sals = decode_line_1 (&arg, 1, 1, (struct symtab *)NULL, 0, &canonical);
...
#if defined(IMSTG)
	  /* artistic license */
        error ("No more than %d hardware breakpoints may be enabled at a time.", target_hardware_breakpoint_limit);
#else
...
#if ! defined(IMSTG)  /* i960 INLINED_BREAKPOINTS */
  /* Confusing, given our implementation of inlined breakpoints. */
  if (sals.nelts > 1)
    {
     ...
     int from_tty;
{
...
#if defined(IMSTG)
  /* For backward compatibility, wwatch and awatch MUST be done in 
     hardware, or fail */
  if (mem_cnt == 0 && i960_hardware_watchpoint_request)
    error ("Expression cannot be implemented with hardware watchpoints.");
#else
  ...
  if (mem_cnt != 0) { 
	...
	TARGET_CAN_USE_HARDWARE_WATCHPOINT(bp_type, i + mem_cnt, 
					   other_type_used);
#if defined(IMSTG)
    if ( target_resources_ok == 0 && i960_hardware_watchpoint_request )
	error ("Target does not support hardware watchpoints.");
    /* else this will degrage into a software watchpoint below */
    else if ( target_resources_ok < 0 && i960_hardware_watchpoint_request )
	error ("Total number of target watchpoint registers used exceeds %d.", target_hardware_watchpoint_limit);
    /* else this will degrage into a software watchpoint below */
#else
...
static void watch_command (arg, from_tty)
     ...
     int from_tty;
{
#if defined(IMSTG)  /* i960 HW Watchpoints */
    /* For backward-compatibility, tell watch_command that this did
       not start life as an awatch or wwatch command. */
    i960_hardware_watchpoint_request = 0;
#endif
...
{
#if defined(IMSTG)  /* i960 HW Watchpoints */
    /* For backwards-compatibility, this needs to FORCE a HW watchpoint,
       else error.  GDB's system degrades to software watchpoint in the
       absense of this flag. */
    i960_hardware_watchpoint_request = 1;
#endif
    watch_command_1 (arg, 2, from_tty);
}

#if defined(IMSTG)  /* i960 HW Watchpoints */
/* For backwards-compatibility; asks for write-only HW watchpoint. */
static void wwatch_command (arg, from_tty)
    char *arg;
    int from_tty;
{
    /* For backwards-compatibility, this needs to FORCE a HW watchpoint,
     ...
     int from_tty;
{
  ...
  if (default_breakpoint_valid)
    ...
    sals = decode_line_1 (&arg, 1, 1, (struct symtab *)NULL, 0, (char ***)NULL);
  ...
  switch (b->type)
      ...
      if (b->addr_string == NULL)
	{
	 ...
	 its status if we succeed.  */
      ...
      s = b->addr_string;
      sals = decode_line_1 (&s, 1, 1, (struct symtab *)NULL, 0, (char ***)NULL);
      for (i = 0; i < sals.nelts; i++)
	{
	  resolve_sal_pc (&sals.sals[i]);
...
#if defined(IMSTG)
      /* In case we have a problem, disable this watchpoint.  We'll restore
	 its status if we succeed.  */
      save_enable = b->enable;
      b->enable = disabled;
#endif
...
decode_line_spec_1 (string, funfirstline, expand_inlines)
     char *string;
     int funfirstline;
     int expand_inlines;
{
  ...
  if (default_breakpoint_valid)
    sals = decode_line_1 (&string, funfirstline, expand_inlines,
...
\twatchpoint     - normal (software) watchpoint\n\
\thw breakpoint  - hardware-assisted breakpoint\n\
\thw watchpoint  - hardware-assisted watchpoint (write access)\n\
\tacc watchpoint - hardware-assisted watchpoint (any access)\n\
...
#if defined(IMSTG)
  add_com ("wwatch", class_breakpoint, wwatch_command,
	   "Set a hardware watchpoint for an expression (write access).\n\
A memory write watchpoint stops execution of your program whenever the\n\
value of an expression changes.  It uses target hardware support to\n\
set the watchpoint.  The expression must make use of at least one memory\n\
object; e.g. not a register, or expression, but a variable like arg[10].\n\
Note that one expression may require more than one hardware watchpoint\n\
in the target hardware.  There is a limit of 2 hardware watchpoints on the\n\
i960 CA.  Not all flavors of i960 have hardware watchpoint support.\n\
You must be connected to the target before using this command.");

  add_com ("awatch", class_breakpoint, awatch_command,
	   "Set a hardware watchpoint for an expression (any access).\n\
A memory access watchpoint stops execution of your program whenever any\n\
memory object used in the expression is read or written to by the program.\n\
See wwatch for more information.");

  add_com ("watch", class_breakpoint, watch_command,
	   "Set a watchpoint for an expression.\n\
A watchpoint stops execution of your program whenever the value of\n\
an expression changes.  This command will make use of target hardware\n\
support if available, else it degrades to the much slower software-only\n\
watchpoint.  See wwatch for more information about hardware watchpoints.");
...
#if defined(IMSTG)
  /* GACK, the hit counts are ugly.  Defaults to 0 above. */
  add_show_from_set
      (add_set_cmd ("hit-counts", no_class, var_boolean,
		    (char *) &show_breakpoint_hit_counts,
		    "Set printing of breakpoint hit counts.",
		    &setprintlist),
       &showprintlist);
#endif
