...
#if defined(IMSTG)
/* We have a name conflict with serial_open */

serial_t gdb_serial_open PARAMS ((const char *name));

#define SERIAL_OPEN(NAME) gdb_serial_open(NAME)
...
#if defined(IMSTG)
/* We have a name conflict with serial_close */

void gdb_serial_close PARAMS ((serial_t));

#define SERIAL_CLOSE(SERIAL_T) gdb_serial_close(SERIAL_T)
