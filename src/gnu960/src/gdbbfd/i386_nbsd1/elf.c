...
/* FIXME: Should be in header file elf-i960.h.  There are too many conflicts
   with elf-common.h to just #include elf.h from IMSTG's include directory.
   If anyone ever fleshes out the elf-i960 backend in gdbbfd (not sure why
   this would be necessary) then that would be a good time to consolidate 
   the Cygnus Elf header files with the IMSTG versions.  */
#define SHF_960_MSB 0x10000000
...
#if defined(IMSTG)
  if ((hdr->sh_flags & SHF_960_MSB) != 0)
    flags |= SEC_BIG_ENDIAN;
#endif
