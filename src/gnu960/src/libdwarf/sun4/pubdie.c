...
/* 
 * dwarf_pubdie - Searches the .debug_pubnames section for an offset/name
 * pair whose name is "name".  This function uses the first occurrence 
 * of "name" if there is more than one occurrence.  dwarf_cu_pubdie() is 
 * used to find the CU DIE containing the name in question, this is then 
 * passed to dwarf_cu_offdie (because the tuple offsets are CU-relative).
 *
 * RETURNS: The Dwarf_Die that the offset/name pair refers to if it 
 * can find one.  Otherwise, NULL.
 *
 * ERROR: Return NULL
 */
Dwarf_Die
dwarf_pubdie(dbg, name)
    Dwarf_Debug dbg; 
    char *name; 
{
	...
	if (pn_table->pn_list) 
	{
	    tuple = _dw_lookup_pn_tuple(dbg, name, pn_table->pn_list, 
					pn_table->pn_count);
	    if ( tuple )
		return dwarf_cu_offdie(dbg,
				       pn_table->pn_cu_die,
				       tuple->offset);
	}
	else
