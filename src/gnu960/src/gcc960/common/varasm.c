...
int var_labelno;
#ifdef IMSTG
/* previous_func_number is negative if emitting const_data. */
static int previous_func_number = 0;
/* If naming text sections, this holds number of function within a file. */
...
static enum in_section {no_section, in_text, in_data, in_named
		...
		, in_debug_info, in_debug_abbrev, in_debug_pubnames,
		in_debug_aranges, in_debug_loc, in_debug_line,
		in_debug_frame, in_debug_macinfo, in_debug_str
#endif
...
#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
/* DWARF's .debug_macinfo output must switch into the .debug_macinfo section.
   Because macros can appear in the middle of an aggregrate initializer,
   DWARF must switch back to the section previously in effect after it's
   done with .debug_macinfo.  These functions support this saving and
   restoring of the previous section.
 */

static enum in_section imstg_saved_section = no_section;
static char* imstg_saved_in_named_name = NULL;

void
imstg_save_current_section()
{
	imstg_saved_section = in_section;
	if (in_section == in_named)
		imstg_saved_in_named_name = in_named_name;
}

void
imstg_resume_previous_section()
{
	/* It is only necessary to restore a data section (or .text
	   for readonly data).
	   Other sections will be restored automatically
	   prior to emitting the next object that goes there.
	 */
	switch (imstg_saved_section)
	{
	case in_data:
		data_section();
		break;
	case in_text:
		...
		break;
	case in_named:
		if (!imstg_saved_in_named_name)
			abort();
		named_section(imstg_saved_in_named_name);
		break;
	}
	imstg_saved_section = no_section;
	imstg_saved_in_named_name = NULL;
}
#endif
#endif
...
#else
#if IMSTG
  previous_func_number = -1;
#endif
  text_section ();
#if IMSTG
  previous_func_number = 0;
#endif
#endif
}
...
/* Define functions similar to "text_section()" and "data_section()"
   that support switching into DWARF debug sections.
 */


#if !defined(DWARF_SECTION_ASM_OP)
#define DWARF_SECTION_ASM_OP	".section"
#endif

#if !defined(DWARF_SECTION_ATTRIBUTES)
#define DWARF_SECTION_ATTRIBUTES	""
	/* attribute string should be of the form  ", attr [, attr ] ..." */
#endif

#define DWARF_SECTION_FUNC(func_name, enum_name, asm_name)		\
void func_name () {							\
  if (in_section != enum_name) {					\
    (void) fprintf(asm_out_file, "%s\t%s%s\n",				\
	DWARF_SECTION_ASM_OP, asm_name, DWARF_SECTION_ATTRIBUTES);	\
    in_section = enum_name;						\
  }									\
}

DWARF_SECTION_FUNC(debug_info_section, in_debug_info, ".debug_info")
DWARF_SECTION_FUNC(debug_abbrev_section, in_debug_abbrev, ".debug_abbrev")
DWARF_SECTION_FUNC(debug_pubnames_section, in_debug_pubnames, ".debug_pubnames")
DWARF_SECTION_FUNC(debug_aranges_section, in_debug_aranges, ".debug_aranges")
DWARF_SECTION_FUNC(debug_loc_section, in_debug_loc, ".debug_loc")
DWARF_SECTION_FUNC(debug_line_section, in_debug_line, ".debug_line")
DWARF_SECTION_FUNC(debug_frame_section, in_debug_frame, ".debug_frame")
DWARF_SECTION_FUNC(debug_macinfo_section, in_debug_macinfo, ".debug_macinfo")
DWARF_SECTION_FUNC(debug_str_section, in_debug_str, ".debug_str")
#endif
#endif /* IMSTG */

     ...
     tree decl;
{
  ...
  if (!TREE_PUBLIC(decl)
      ...
      && DECL_RTL (decl) == 0)
    {
      ...
      if (decl_function_context (decl) != 0)
        fatal ("%s: nested functions are not supported.", name);

      ASM_FORMAT_PRIVATE_NAME (label, name, 0);
      name = obstack_copy0 (saveable_obstack, label, strlen (label));
    }
#endif
  ...
  if (DECL_RTL (decl) == 0)
      ...
      if (DECL_RTL (decl) == 0)
	{
	  /* Can't use just the variable's own name for a variable
	     ...
	     Concatenate a distinguishing number.  */
#ifdef IMSTG
          if (((!top_level && DECL_EXTERNAL(decl)) ?
                TREE_LINKAGE_INTERNAL(decl) : !TREE_PUBLIC(decl)) &&
              asmspec == 0)
	    { char *label;
...
#if defined(IMSTG) && defined(PID_CHG_DECL)
          PID_CHG_DECL(decl);
#endif
     ...
     char *fnname;
{
...
#else
  if (write_symbols == SDB_DEBUG ||
      write_symbols == NO_DEBUG && flag_coff)
  ...
  if (TREE_PUBLIC (decl))
#else
  if (TREE_PUBLIC (decl) || GLOBALIZE_STATICS)
#endif
    {
...
#ifdef IMSTG
      SYMREF_SIZE(XEXP(DECL_RTL(decl), 0)) = TREE_INT_CST_LOW (size_tree);
      SYMREF_ETC(XEXP(DECL_RTL(decl), 0)) |= SYMREF_DEFINED;
#endif
    }
...
#else
  if ((TREE_PUBLIC(decl)||GLOBALIZE_STATICS)&& DECL_NAME (decl))
#endif
    {
      if (!first_global_object_name)
     ...
     char *name;
{
  ...
  char *real_name;
#else
  static char *real_name = 0;

  if (real_name == 0 || strlen(real_name) < strlen(name))
  {
    char *t = real_name;
    real_name = xmalloc(strlen(name)+1);
    if (t != 0) free(t);
  }
#endif
...
#ifdef IMSTG
  /* If we have something between a pair of @@'s, just output the
     part between the @@.

     We do this when we need to make a SYMBOL_REF for an object with
     a known offset.  There may be stuff to distinguish the name after
     the last @ which we do not print.  */
...
#else
      output_constant_def_contents (p->exp, p->reloc, p->labelnm);
#endif
      next = p->next;
      free (p);
    }
...
#else
      ASM_FUNCTION_LOCAL_LABEL (label, GENERATE_LABEL);
#endif
...
#ifdef IMSTG
{ int s = int_size_in_bytes (TREE_TYPE(exp));
  if (s > SYMREF_SIZE(def))
    SYMREF_SIZE(def) = s;
}
#endif
  TREE_CST_RTL (exp)
...
#else
	output_constant_def_contents (exp, reloc, desc->label);
#endif
    }
...
output_constant_def_contents (exp, reloc, labelno)
     ...
     int labelno;
#else
output_constant_def_contents (exp, reloc, labelnm)
     tree exp;
     int reloc;
     char* labelnm;
#endif
...
#else
  ASM_FUNCTION_LOCAL_LABEL (labelnm, OUTPUT_LABEL + GLOBALIZE_LABEL);
#endif
     ...
     rtx x;
{
  ...
  if (found == 0)
	  ...
	  && GET_CODE (x) == CONST)
	{
      ...
      pool->mode = mode;
#ifdef IMSTG
      pool->labelno = ASM_FUNCTION_LOCAL_LABEL (label, GENERATE_LABEL);
#else
...
#else
      def = gen_rtx (MEM, mode, gen_rtx (SYMBOL_REF, Pmode, label));
      desc->label = found = XSTR(XEXP(def,0),0);
#endif
     ...
     tree fndecl;
{
  ...
  for (pool = first_pool; pool; pool = pool->next)
	...
	ASM_OUTPUT_ALIGN (asm_out_file, exact_log2 (pool->align));
      ...
      /* Output the label.  */
#ifdef IMSTG
      ASM_FUNCTION_LOCAL_LABEL (pool->desc->label,OUTPUT_LABEL+GLOBALIZE_LABEL);
#else
...
#else
      {
        rtx x = expand_expr (exp, NULL_RTX, VOIDmode, EXPAND_INITIALIZER);

#ifdef CHECK_PIC_PID
        CHECK_PIC_PID(x);
#endif
        if (! assemble_integer (x, size, 0))
	  error ("initializer for integer value is too complicated");
      }
#endif
      size = 0;
      ...
      else
	{
	  ...
	  while (next_offset < end_offset)
		...
		abort ();
		 ...
		 and pack them starting at the least significant
		 bits of the bytes.  */
	      shift = (bytes_big_endian
                      ? (end_offset - next_offset - this_time)
                      : (next_offset - TREE_INT_CST_LOW (DECL_FIELD_BITPOS (field))));
	      /* Don't try to take a bunch of bits that cross
		 the word boundary in the INTEGER_CST.  */
	      if (shift < HOST_BITS_PER_WIDE_INT
		  && shift + this_time > HOST_BITS_PER_WIDE_INT)
