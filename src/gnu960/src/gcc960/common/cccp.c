...
#if defined(IMSTG)
#include "i_toolib.h"
static void pass_thru_directive();
#endif
...
#define getrusage(x,p) (bzero (p, sizeof (struct rusage)))
#endif
#else
#include <time.h>
#include <fcntl.h>
...
#include <signal.h>
#include "assert.h"
static int open_preinclude_file();
static int dos_filenames = 0;	/* Enable backslash as path seperator */
int flag_ic960;			/* Try to behave like the ic960 preprocessor */
static char* in_fname;

static int list_file_state = -1;
static int next_mvect;
...
static void imstg_skip_if_group();
#define skip_if_group(ip,any,op) \
do { \
  flush_sinfo(ip,(ip)->bufp,0,0,0); \
  imstg_skip_if_group((ip),(any),op); \
  flush_sinfo(ip,(ip)->bufp,1,0,0); \
} while (0)

char *lbuf;

unsigned sinfo_index=1;
unsigned sinfo_tell()
{
  return sinfo_index;
}
...
push_mvect()
{
  assert (mvect && mvect_alloc > 0);
  if (mvect_alloc <= ++next_mvect)
    mvect = (macro_rec*)
      xrealloc (mvect, (mvect_alloc = next_mvect * 2) * sizeof(mvect[0]));
}

char* macbuf;
int macbuf_alloc, macbuf_pos, macbuf_last;

add_pending_macro (p, size, lo_sub, hi_sub)
char* p;
{
  int i,info_len,asize;
  char* r;
  ...
  info_len = 1 + strlen(mvect[0].str) + 1;
  for (i=lo_sub; i<hi_sub; i++)
    info_len += strlen (mvect[i].str) + 1;
  ...
  { macbuf_pos = 0;
    macbuf_alloc = asize;
    macbuf = xmalloc (macbuf_alloc);
  }
  else
    if ((macbuf_pos + asize) > macbuf_alloc)
    { macbuf_alloc = macbuf_pos + asize;
      macbuf = xrealloc (macbuf, macbuf_alloc);
    }

  r = macbuf+macbuf_pos;

  for (i=hi_sub-1; i >= lo_sub; i--)
  { *r++ = (i&1) ? 'm' : 't';
    strcpy (r, mvect[i].str);
    r += strlen(r);
  }
  *r++ = (i&1) ? 'm' : 't';
  strcpy (r, mvect[0].str);
  r += strlen(r);
  *r++ = ':';
  strncpy (r, p, size);
  r[size-1] = '\0';
  assert ((r-(macbuf+macbuf_pos))+size == asize);
  macbuf_pos += asize;
  assert (macbuf[macbuf_pos-1] == '\0');
}

void
flush_sinfo(p,end,is_compiled_out,op,obp)
FILE_BUF* p;
U_CHAR* end;
int is_compiled_out;
FILE_BUF* op;
U_CHAR* obp;
{
  if (sinfo_file && p->fname && !no_output)
  {
    U_CHAR* buf  = p->buf;
    U_CHAR* bufq = p->bufq;
    int lineno   = p->start_lineno;
    int i,t,tick;

    if (mvect == 0)
      mvect = (macro_rec*) xmalloc ((mvect_alloc = 10) * sizeof(mvect[0]));
    ...
    if (bufq == 0)
    { assert (lineno == 0 && !is_compiled_out);
      p->start_lineno = lineno = 1;
      bufq = buf;
    }
    ...
    { U_CHAR *so, *r;
      ...
      /* Make up a seperate entry for each line;  output 1 entry
         after each sinfo line. */

      i  = 1;
      si = 1;
      se = mvect[0].ip_end;
      r = so = outbuf.buf+mvect[0].op_end;
      strcpy (mvect[0].str, "0 0,");
      while (r != obp)
      { if (r == outbuf.buf+mvect[i].op_end)
          /* Associate the start or end of an expansion with the input
             that generated it. */
        { char *c = mvect[i].str;
 
          sprintf (c,"%d ", (outbuf.buf+mvect[i].op_end)-so);
          c += strlen(c);

          sprintf (c, "%d,", mvect[i].ip_end-se);
          i++;
        }

        if (*r++ == '\n' || r == obp)
        { char* t;
          add_pending_macro (so, r-so, si, i);
          so = r;
          si = i;
          if (t=index(p->buf+se, '\n'))
            se = (t+1) -(char*) p->buf;
    ...
    { static int lalloc;
      ...
      /* lbuf always contains the last source line */
      if (lalloc < (end-bufq) + 10)
      { lalloc = (end-bufq) + 1024;
        if (lbuf==0)
          lbuf=xmalloc(lalloc);
        else
          lbuf=xrealloc(lbuf,lalloc);
      }

      if ((!is_compiled_out) || flag_list_compiled_out)
      {
        if (list_file)
        { if (list_file_state < 0)
            init_list_file (in_fname);
          list_file_state = 0;
        }
        ...
        { int c, m, lnext,l;
          ...
          l = (bufq==buf) ? '*' : ' ';
          m = ((macbuf_last < macbuf_pos) ? 'm' : ' ');

          fprintf(sinfo_file,"%c%c%3d%c%7d%c  ", t, m, indepth, l, lineno,tick);

          if (list_file && (indepth==0 || flag_list_include))
            fprintf (list_file, " %4d%c%7d%c  ",    indepth, l, lineno,tick);
    
          if (m == 'm')
          { inf = macbuf+macbuf_last;
            exp = index (inf, ':')+1;
            exp[-1] = 0;
            fprintf (sinfo_file, "%s:", inf);
            exp[-1] = ':';
          }
          ...
          { 
            if (flag_list_macro)
            { fprintf (sinfo_file, "m%4d%c%7d   %s\n",
                                 indepth, l, lineno, exp);
              if (list_file && (indepth==0 || flag_list_include))
                fprintf (list_file, "%s%s\n", list_pluses, exp);
            }
            macbuf_last += strlen (inf) + 1;
            if (macbuf_last >= macbuf_pos)
              macbuf_last = macbuf_pos = 0;
          }
          last_line = lineno++;
          last_file = p->nominal_fname;
        }
      }
      else
      {
    ...
    p->start_lineno = lineno;
    p->bufq = bufq;
    mvect[next_mvect].ip_end = bufq - buf;
    mvect[next_mvect].op_end = obp-outbuf.buf;
  }
}
...
out_message (severity, ip, f, l, msg, arg1, arg2, arg3)
FILE_BUF* ip;
char* f;
char* msg;
{
  ...
  char buf[2048], mbuf[2048], *m, *p;

  assert (severity>=0 && severity <= 3);

  /* If there is no file and no ip, we want to show the file and line number,
     and we do not want to look up the source line.  Otherwise,
     we show the source line, and if the line is non-current we put
     that in the listing as well. */
  ...
  if (lookup = (ip != 0))
  {
    /* Catch the sinfo up so the message will go in the right place ... */
    flush_sinfo(ip,ip->bufp,0,0,0);
    ...
    lookup = (file != 0 && file == last_file && lbuf && lbuf[0]);
  }
  showl = file && (!lookup || line != last_line || file != last_file ||
                   (indepth!=0 && flag_list_include==0));
  ...
  { int i;
    ...
    /* Emit the message to the sinfo file for eventual use by the
       lister.  We cannot support multi-line messages; the stuff that
       searches the sinfo file in cc1.960 will go bonkers if a newline gets
       embedded in a message, so die if we find one. */

    fprintf (sinfo_file, "e%d%s",lookup,m);
    fprintf (sinfo_file, "\n");
    i=strlen(m);
    while (--i >= 0)
      if (m[i] == '\n')
  ...
  if (list_file)
  {
    /* If we are making a list file in the preprocessor (cc1.960 won't
       run) put out the message to the listing file. */

    if (list_file_state < 0)
      init_list_file (in_fname);

    if (list_file_state < 1)
      fprintf (list_file, "\n");
    ...
    /* Remember that newlines went out. */
    list_file_state = 1;
  }
}
...
char* file;
{
  ...
  char *vers;
  assert (list_file != 0 && list_file_state < 0);
...
{
#ifdef IMSTG
  int zz_dummy_zz = set_abort_signals(0);
#endif
  int st_mode;
  long st_size;
#ifndef IMSTG
  ...
  FILE_BUF *fp;
#if defined(IMSTG)
  int xx_dummy_xx = argc = get_response_file (argc, &argv);
  int yy_dummy_yy = argc = get_cmd_file (argc, argv);
#endif
...
/* special hack for 960 version */
  ...
  for (i = 1; i < argc; i++) {
...
#if defined(IMSTG) && defined(DOS)
	out_fname = normalize_file_name(argv[i]);
#else
	out_fname = argv[i];
#endif
      else
#if defined(IMSTG) && defined(DOS)
	in_fname = normalize_file_name(argv[i]);
#else
	in_fname = argv[i];
#endif
    } else {
      ...
      argv[i][0] = '-';               /* treat '/' like '-' */
#endif
      if (i+1 < argc && set_list_option (argv[i], argv[i+1]))
        i++;
      else
#endif
      switch (argv[i][1]) {
...
#ifdef IMSTG
      case 'f':
        if (!strcmp (argv[i], "-ffancy-errors"))
          flag_fancy_errors = 1;
        break;
#endif
      case 'i':
#ifdef IMSTG
	if (!strcmp (argv[i], "-ic960"))
          flag_ic960 = 1;
#endif
...
#if defined(IMSTG) && defined(DOS)
	    pend_includes[i] = normalize_file_name(argv[i+1]), i++;
#else
...
#if defined(IMSTG) && defined(DOS)
	    pend_files[i] = normalize_file_name(argv[i+1]), i++;
#else
...
#if defined(IMSTG) && defined(DOS)
	    include_prefix = normalize_file_name(argv[++i]);
#else
...
#if defined(IMSTG) && defined(DOS)
	  (void) normalize_file_name(dirtmp->fname);
#endif
	  dirtmp->got_name_map = 0;
...
#if defined(IMSTG) && defined(DOS)
	    dirtmp->fname = normalize_file_name(argv[++i]);
#else
	    dirtmp->fname = argv[++i];
#endif
	  dirtmp->got_name_map = 0;
	...
	out_fname = argv[++i];
#if defined(IMSTG) && defined(DOS)
	(void) normalize_file_name(out_fname);
#endif
	if (!strcmp (out_fname, "-"))
	  out_fname = "";
	break;
...
#ifdef IMSTG
      case 's':
	if (!strcmp (argv[i], "-sinfo")) {
	  if (i + 1 == argc)
	    fatal ("Filename missing after `-sinfo' option");
	  else
          { sinfo_name = argv[++i];
#if defined(IMSTG) && defined(DOS)
	    (void) normalize_file_name(sinfo_name);
#endif
            sinfo_file = fopen (sinfo_name, "w");

            if (sinfo_file == 0)
	      pfatal_with_name (sinfo_name);
          }
	}
      break;
#endif
	...
	if (!strcmp (argv[i], "-MD")
	  ...
	  deps_file = argv[i];
#if defined(IMSTG) && defined(DOS)
	  (void) normalize_file_name(deps_file);
#endif
	  deps_mode = "w";
	} else {
...
#if defined(IMSTG)
  {
    /* For ic960-style pre-includes, the file is searched for as if
     * it was a #include "".  Thus its true name may include a path. */
    char*  full_file_name;
#endif
    if (pend_includes[i]) {
#if defined(IMSTG)
      int fd = open_preinclude_file(pend_includes[i], &full_file_name);
#else
...
#else
	pfatal_with_name (pend_includes[i]);
#endif
	return FAILURE_EXIT_CODE;
      }
#if defined(IMSTG)
      finclude (fd, full_file_name, &outbuf, 0, NULL_PTR);
#else
  ...
  flush_sinfo(fp,fp->bufp,0,0,0);
  output_line_command (fp, &outbuf, 0, same_file);
  if (sinfo_file && sinfo_file != stdout
      && (ferror (sinfo_file) || fclose (sinfo_file) != 0))
    fatal ("I/O error on `-sinfo' output");
#endif
...
do { ip->macro->type = T_MACRO;		\
     ...
     --indepth; } while (0)
#else
/* We need to intercept the outermost macro call, so we can mark the
   final expansion in the source information file. */
int outer_macro = -2;
#define POPMACRO \
do { ip->macro->type = T_MACRO;		\
     if (ip->free_ptr) free (ip->free_ptr);	\
     --indepth; \
     if (sinfo_file && indepth == outer_macro) { \
       push_mvect(); \
       mvect[next_mvect].ip_end = instack[indepth].bufp-instack[indepth].buf; \
       mvect[next_mvect].op_end = obp-op->buf; \
       outer_macro = -2; } } while (0)
#endif
...
#else
      if (! handle_directive (ip, op, beg_of_line)) {
#endif
#ifdef USE_C_ALLOCA
	alloca (0);
#endif
	  ...
	  for (hp = hashtab[MAKE_POS (hash) % HASHSIZE]; hp != NULL;
		...
		*obp++ = ' ';
		 ...
		 and push the expansion on the input stack.  */
	      ...
	      op->bufp = obp;
#ifdef IMSTG
              /* If we are starting a new top level expansion,
                 mark where we are in the line for listing info, to
                 show where the current text stops and the expansion
                 starts. Also, set outer_macro so that pop_macro will
                 know to push a record of the end of the expansion.  Thus,
                 every odd entry in mvect marks the start of an expansion,
                 and every even entry marks the start of text that is
                 between expansions.  TMC Fall '93 */

              if (sinfo_file && outer_macro==-2 && ip->fname && op==&outbuf)
              { outer_macro = indepth;
                push_mvect();
                mvect[next_mvect].ip_end = ibp-ip->buf-ident_length;
                mvect[next_mvect].op_end = obp-op->buf;
              }
#endif
	      macroexpand (hp, op);
...
#else
handle_directive (ip, op, beg_of_line)
U_CHAR* beg_of_line;
#endif
     FILE_BUF *ip, *op;
{
...
#ifdef IMSTG
      if (ip->fname && !no_line_commands && !no_output)
      { /* To keep from getting an extra blank line followed by a #line
           directive, we need to go ahead and eat the newline at the end of
           the #line.  I have no idea why this wasn't done before;  the
           previous behaviour seems sort of bizarre.  TMC fall '93 */

        ip->lineno++;
        ip->bufp++;
      }

      flush_sinfo (ip,ip->bufp,0,op,0,0);
      output_line_command (ip, op, 0, same_file);
#endif
      ...
      if (string)
	{
...
#else
	  /* Need to escape special characters, especially
	   * backslash which is common in DOS file names.
	   */
	  int i, len = 0;
	  char *p = string;
	  ...
	  for (; *p; p++) {
		...
		if (isprint(*p))
		  ...
		  len += 4;	/* Print in octal, as "\nnn" */
	    }
	  }

	  buf = (char *) alloca (3 + 4 * len);
	  p = buf;
	  *p++ = '"';
...
#ifdef IMSTG
	/* WARNING:  Much of this code is duplicated in function
	 * open_preinclude_file().  If you make a change here, be
	 * sure to change it there as well.
	 */

	/* To emulate ic960, search first in the directory containing
	 * the original input file being preprocessed.  This should be
	 * the first real file on the bottom of instack.
	 * gcc960 searches the directory containing the most recent
	 * real file on the top of instack, ie the file that immediately
	 * included "filename".
	 */
      fp = NULL;
      if (flag_ic960) {
	FILE_BUF *found;
	for (found = instack; found <= &instack[indepth]; found++)
	  if (found->nominal_fname != NULL) {
		fp = found;
		break;
	  }
      } else {
	FILE_BUF *found;
	for (found = &instack[indepth]; found >= instack; found--)
	  if (found->nominal_fname != NULL) {
		fp = found;
		break;
	  }
...
#else
	{
		/* On DOS, only append a trailing slash if one
		 * isn't already there.  Otherwise open will fail.
		 * Do not append a slash to the end of a drive,
		 * ie, don't change -Ic: to -Ic:/ !!!
		 */
...
/* Open a preinclude file and return its file descriptor.
 * Under gcc960 behavior, the include file is opened by name.
 * Under ic960 behavior, the include file is searched for using
 * the same rules as looking for #include "file".
 * Allocate space for, and return the successfully opened full file name
 * in the parameter 'full_file_name'.
 *
 * This function repeats much of the same functionality of "do_include()".
 */

static int
open_preinclude_file (input_name, full_file_name)
char	*input_name;
char	**full_file_name;
{
	int			fd;

	/* Open the file by name if not in ic960 mode, or if the -I-	*/
	/* option was given, or if an absolute path was used.		*/

	if (!flag_ic960 || ignore_srcdir || input_name[0] == '/'
#if defined(DOS)
		|| input_name[0] == '\\'
		|| (input_name[0] && input_name[1] == ':')
#endif
	   ) {
		*full_file_name = (char *)xmalloc(strlen(input_name) + 1);
		(void) strcpy(*full_file_name, input_name);
#if defined(DOS)
		fd = open (input_name, O_RDONLY | O_TEXT, 0666);
#else
		fd = open (input_name, O_RDONLY, 0666);
#endif
	}
	else
	{
		...
		struct file_name_list	primary_dir[1];
		char			*nam;
		char			*slashp;

		/* Get full path of primary input file. */
		fp = NULL;
		for (found = instack; found <= &instack[indepth]; found++)
			if (found->nominal_fname != NULL) {
				fp = found;
		...
		if (fp == NULL)
		{
			/* This is an internal error, it should not happen
			 * under ic960 mode. */
			fatal("No primary input file, unable to search for %s",
						input_name);
		}

		/* Temporarily prepend the primary source files's
		 * directory to the list of search directories. */

		nam = fp->nominal_fname;
		slashp = rindex (nam, '/');
#if defined(DOS)
		{
			char	*bslashp = rindex (nam, '\\');
			if (slashp == NULL && bslashp == NULL)
			{
				if (nam[0] && nam[1] == ':')
					slashp = nam + 1;
			}
			else if ((slashp == NULL && bslashp != NULL)
					|| (bslashp > slashp))
				slashp = bslashp;
		}
#endif
		if (slashp != NULL) {
			int	n = slashp - nam;
			primary_dir[0].fname = (char *) xmalloc(n + 1);
			(void) strncpy (primary_dir[0].fname, nam, n);
			primary_dir[0].fname[n] = '\0';
			if (n + INCLUDE_LEN_FUDGE > max_include_len)
				max_include_len = n + INCLUDE_LEN_FUDGE;
		}
		else
			primary_dir[0].fname = NULL; /* Current directory */

		primary_dir[0].next = search_start;
		search_start = primary_dir;

		/* Allocate the full search name permanently, because it gets
		   stored in the definitions of macros.  */

		*full_file_name = (char *)xmalloc(max_include_len
						+ strlen(input_name) + 2);
		(*full_file_name)[0] = '\0';

		/* Search directory path, trying to open the file.
		   Copy each filename tried into full_file_name.  */
		...
		for (searchptr = search_start; searchptr; searchptr = searchptr->next)
		{
			if (searchptr->fname) {
				...
				if (searchptr->fname[0] == 0)
					continue;
				(void) strcpy (*full_file_name, searchptr->fname);
				(void) strcat (*full_file_name, "/");
				(void) strcat (*full_file_name, input_name);
			} else if (searchptr == primary_dir) {
				/* Primary source directory is the current
				 * directory.  Just open the file by name.
				 */
				(void) strcpy (*full_file_name, input_name);
			} else {
				continue;
			...
			/* We must be done using primary_dir by now (in first
			 * loop iteration).  Deallocate the string allocated
			 * above. */

			if (primary_dir[0].fname != NULL) {
				free(primary_dir[0].fname);
				primary_dir[0].fname = NULL;
			}

#if defined(DOS)
			fd = open (*full_file_name, O_RDONLY | O_TEXT, 0666);
#else
			fd = open (*full_file_name, O_RDONLY, 0666);
#endif

#ifdef EACCES
			if (fd == -1 && errno == EACCES)
				fatal("Pre-include file %s exists, but is not readable",
						*full_file_name);
#endif
			if (fd >=0)
				break;
	...
	if (fd >= 0) {
		struct file_name_list* ptr;
		for (ptr = all_include_files; ptr; ptr = ptr->next) {
			/* TBD, strcmp is not sufficient on DOS. */
			if (!strcmp (ptr->fname, *full_file_name))
				break;	/* This file was included before. */
		}
		...
		if (ptr == 0) {
			...
			ptr = (struct file_name_list *)
				xmalloc (sizeof (struct file_name_list));
			ptr->control_macro = 0;
			ptr->next = all_include_files;
			all_include_files = ptr;
			ptr->fname = savestring (*full_file_name);
		}

		if (flag_ic960 && print_deps) {
			deps_output ("", 0);
			deps_output (*full_file_name, 0);
			deps_output (" ", 0);
		}
	  ...
	  if (! strcmp (map->map_from, from))
	    {
	      /* Found a match.  */
#if defined(IMSTG) && defined(DOS)
	      return open (map->map_to, O_RDONLY | O_TEXT, 0666);
#else
...
#else
      return open (filename, O_RDONLY | O_TEXT, 0666);
#endif
    }
     ...
     struct file_name_list *dirptr;
{
  ...
  input_file_stack_tick++;
#ifdef IMSTG
  flush_sinfo(&instack[indepth],instack[indepth].bufp,0,0,0);
#endif /* new IMSTG */
...
#else
  /* Allow room for the potential space character and newline added below. */
  check_expand (op, 3 + keyword_length + (limit - buf));
#endif
  *op->bufp++ = '#';
...
#endif
#if defined(IMSTG)
  /* Make sure a newline is emitted at the end of op->bufp.
     We already bumped op->lineno in handle_directive.
   */
  if (*(op->bufp - 1) != '\n')
    *op->bufp++ = '\n';
#endif
...
#else
	msg = (U_CHAR *) alloca (mdef.symlen + 2048);
	*msg = '`';
	bcopy (mdef.symnam, msg + 1, mdef.symlen);
	if (hp->type == T_MACRO)
        { sprintf ((char *) (msg + mdef.symlen + 1),
            "' is defined another way at \"%s\", line %d",
            hp->value.defn->file, hp->value.defn->line-1);
        }
        else
	  strcpy ((char *) (msg + mdef.symlen + 1), "' redefined");
	pedwarn (msg);
...
#else
  new_lineno = atoi (bp);	/* We ate the newline in handle_directive. */
  if (pedantic && new_lineno <= 0)
#endif
...
#else
      char e[1024];
      sprintf(e, "`#elif' after `#else' (matches line %d)",if_stack->lineno);
      if (if_stack->fname != NULL && ip->fname != NULL &&
          strcmp (if_stack->fname, ip->nominal_fname) != 0)
        sprintf ((e+strlen(e))-1, ", file %s)", if_stack->fname);
      error ("%s", e);
#endif
    }
...
#else
      char e[1024];
      sprintf(e, "`#else' after `#else' (matches line %d)",if_stack->lineno);
      if (strcmp (if_stack->fname, ip->nominal_fname) != 0)
        sprintf ((e+strlen(e))-1, ", file %s)", if_stack->fname);
      error ("%s", e);
#endif
     ...
     int rest_args;
{
  ...
  if (!(ip->fname != 0 && (newlines != 0 || comments != 0))
    ...
    if (argptr != 0) {
      ...
      argptr->raw = ip->bufp;
#else
      /* We cannot safely set raw to point into ip->buf
       * (== instack[indepth].buf).  The problem is that a subsequent call
       * to macarg() from macroexpand() might need to pop the instack and
       * call "free (ip->free_ptr);" (see below).  This implies
       * that this argptr->raw would be a dangling pointer.
       * Hence, we need to make a copy.
       */
      argptr->raw = (U_CHAR *) xmalloc ((bp - ip->bufp) + newlines + 1);
      argptr->free1 = argptr->raw;	/* Remember to free it */
      bcopy(ip->bufp, argptr->raw, bp - ip->bufp);
#endif
     ...
     char *arg1, *arg2, *arg3;
{
  ...
  fprintf (stderr, "\n");
#else
  out_message (1, ip, 0, 0, msg, arg1, arg2, arg3);
#endif
  errors++;
}
...
#else
  out_message (1, ip, 0, 0, "%s: %s", name, my_strerror (errno), 0);
#endif
     ...
     char *arg1, *arg2, *arg3;
{
  ...
  fprintf (stderr, "\n");
#else
  out_message (0, ip, 0, 0, msg, arg1, arg2, arg3);
#endif
}
     ...
     char *arg1, *arg2, *arg3;
{
  ...
  fprintf (stderr, "\n");
#else
  out_message (1, ip, 0, line, msg, arg1, arg2, arg3);
#endif
  errors++;
}
     ...
     char *arg1, *arg2, *arg3;
{
  ...
  fprintf (stderr, "\n");
#else
  out_message (0, ip, 0, line, msg, arg1, arg2, arg3);
#endif
}
     ...
     char *arg1, *arg2, *arg3;
{
  ...
  fprintf (stderr, "\n");
#else
  out_message (!!pedantic_errors, 0, file, line, msg, arg1, arg2, arg3);
#endif
}

  ...
  if (debug_output)
    {
      ...
      if (instack[0].fname)
      {
        sprintf (directive, " __FILE__ ");
	(void) quote_string(directive+strlen(directive), instack[0].fname);
	(void) strcat(directive, "\n");
        output_line_command (inp, outp, 0, same_file);
        pass_thru_directive(directive, &directive[strlen(directive)], outp, dp);
      }

      sprintf (directive, " __BASE_FILE__ ");
      (void) quote_string(directive+strlen(directive),instack[0].nominal_fname);
      (void) strcat(directive, "\n");
      output_line_command (inp, outp, 0, same_file);
      pass_thru_directive (directive, &directive[strlen (directive)], outp, dp);
#endif /* IMSTG */
...
#if defined(IMSTG)
  if (debug_output)
    pass_thru_directive (buf, buf + strlen(buf), op, kt);
#endif
...
#if defined(IMSTG)
  if (debug_output)
    pass_thru_directive (str, str + strlen(str), op, kt);
#endif
...
#if defined(IMSTG)
/* NOTE: Outputting the empty string ALWAYS causes a line break
 * under flag_ic960, since ic960 prints one dep per line.
 * Also note that ic960 reprints the target on each line, rather
 * than continuing the line with a backslash.
 */
#endif
static void
     ...
     char *str, *arg;
{
  ...
  fprintf (stderr, "\n");
#else
  out_message (2, 0, 0, 0, str, arg, 0, 0);
#endif
  exit (FAILURE_EXIT_CODE);
}
#ifdef IMSTG
internal_error(str, arg1, arg2, arg3)
char* str;
{
  out_message (3, 0, 0, 0, str, arg1, arg2, arg3);
  exit (FAILURE_EXIT_CODE);
}
#endif
     ...
     char *name;
{
  ...
  fprintf (stderr, "%s: %s\n", name, my_strerror (errno));
#else
  out_message (1, 0, 0, 0, "%s: %s", name, my_strerror (errno), 0);
#endif
  errors++;
}
     ...
     char *name;
{
  ...
  perror_with_name (name);
#else
  out_message (2, 0, 0, 0, "%s: %s", name, my_strerror (errno), 0);
#endif
#ifdef VMS
  exit (vaxc$errno);
#else
