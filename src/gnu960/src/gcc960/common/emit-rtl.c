...
#ifdef IMSTG
int cur_insn_uid = 1;	/* for PUT_NEW_UID */
#else
...
#ifdef IMSTG
static int last_list_start = 0;
static int last_list_end = 0;

/* Generate unique "instance numbers" for duplicated line number notes */
int	imstg_note_instance_counter = 0;
#endif
...
gen_rtx VPROTO((enum rtx_code code, enum machine_mode mode, ...))
{
  ...
  if (code == CONST_INT)
    ...
    rt_val = gen_symref_rtx (mode, va_arg (p, char *));
#endif
  else
    {
...
#ifdef IMSTG
        if (code==CALL && XEXP(rt_val,0) && GET_CODE(XEXP(rt_val,0))==MEM)
          set_rtx_type (XEXP(rt_val,0), void_type_node);
#endif
    }
  va_end (p);
...
#else
	return set_rtx_type (gen_rtx (REG, mode, REGNO (x) + word), get_rtx_type(x));
#endif
      else
     ...
     register rtx x;
{
  ...
  if (GET_CODE (x) == CONST_DOUBLE
	...
	offset = (MAX (GET_MODE_SIZE (GET_MODE (x)), UNITS_PER_WORD)
      ...
      if (!bytes_big_endian && GET_MODE_SIZE (mode) < UNITS_PER_WORD)
...
#else
	   ? gen_rtx (CODE_LABEL, VOIDmode, NULL, bc_get_bytecode_label (), 0,0,0,0)
	   : gen_rtx (CODE_LABEL, VOIDmode, 0, 0, 0, label_num++, NULL_PTR, 0));
#endif
...
/* Return the previous insn before INSN that is not a line number NOTE.
   This routine does not look inside SEQUENCEs.  */

rtx
prev_nonlinenote_insn (insn)
     rtx insn;
{
  while (insn)
    {
      insn = PREV_INSN (insn);
      if (insn == 0 || GET_CODE (insn) != NOTE || NOTE_LINE_NUMBER(insn) < 0)
	break;
    }
...
rtx
emit_call_insn_after (pattern, after)
...
#ifdef IMSTG
  NOTE_LISTING_START(note) = 0;
  NOTE_LISTING_END(note) = 0;
#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
  if (subtype >= 0 && flag_linenum_mods)
  {
    NOTE_AUX_SRC_INFO(note) = 0;
    NOTE_SET_INSTANCE_NUMBER(note, imstg_note_instance_counter);
  }
#endif /* DWARF */
#endif
...
emit_note (file, line)
#else
imstg_emit_note (file, line, list_start, list_end)
#endif
     char *file;
     int line;
{
...
#ifdef IMSTG
          && list_start == last_list_start
          && list_end == last_list_end
#endif
	  && line == last_linenum)
	return 0;
      last_filename = file;
  ...
  NOTE_LINE_NUMBER (note) = line;
#ifdef IMSTG
  NOTE_LISTING_START(note) = list_start;
  NOTE_LISTING_END(note) = list_end;
#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
  if (line >= 0 && flag_linenum_mods)
  {
    NOTE_AUX_SRC_INFO(note) = 0;
    NOTE_SET_INSTANCE_NUMBER(note, imstg_note_instance_counter);
  }
...
#ifdef IMSTG
rtx
imstg_emit_line_note (file, line, list_start, list_end)
     char *file;
     int line;
{
  if (output_bytecode)
  ...
  emit_filename = file;
  emit_lineno = line;
  emit_list_lineno = list_start;

  return imstg_emit_note (file, line, list_start, list_end);
}
...
int line;
{ 
  /* Emit a non line-number note at the current source position.
     All callers who want line notes should use imstg_emit_line_note;
     callers who want line-line notes but with specific positions should use
     imstg_emit_note. */
  return imstg_emit_note (file, line, LOOK_LPOS, 0);
}
#endif
...
#ifdef IMSTG
rtx
imstg_emit_line_note_force (file, line, start, end)
     char *file;
     int line;
{
  last_linenum = -1;
  return imstg_emit_line_note (file, line, start, end);
}
#endif
...
init_emit ()
{
  ...
  last_filename = 0;
#ifdef IMSTG
  last_list_start = last_list_end = 0;
#endif
...
#endif
#if defined(IMSTG) && defined(PID_BASE_REGNUM)
  if (pid_flag || TARGET_PID)
    pid_reg_rtx = gen_rtx(REG, Pmode, PID_BASE_REGNUM);
#endif
}
