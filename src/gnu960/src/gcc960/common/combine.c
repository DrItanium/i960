...
#if defined(IMSTG) && defined(DEBUG_COMBINE)
...
#if defined(IMSTG) && defined(DEBUG_COMBINE)
int try_combine_calls;
rtx
try_combine (t3,t2,t1)
rtx t3,t2,t1;
{
  extern rtx base_try_combine();
  rtx ret;

  ret = base_try_combine (t3,t2,t1);
...
#ifdef IMSTG
  /* added_sets_1 could be unitialized otherwise */
  added_sets_1 = 0;
  added_sets_2 = 0;
#endif
...
#else
      while (GET_CODE (inner) == SUBREG && subreg_lowpart_p (inner) &&
             GET_MODE_SIZE(GET_MODE (SUBREG_REG (inner))) <= UNITS_PER_WORD)
        inner = SUBREG_REG (inner);
#endif
...
#ifdef IMSTG
	  COPY_MEM_MRD_FIELDS(newmem,inner);
#endif
	  inner = newmem;
	}
    }
...
#else
          rtx newer = gen_lowpart_for_combine(mode, tem);
#endif
...
#else
        /*
         * leave special case where we put dummy clobber of const0_rtx in
         * to keep from matching.
         */
        if (GET_CODE (XVECEXP (pat, 0, i)) != CLOBBER ||
            XEXP( XVECEXP (pat, 0, i), 0) == const0_rtx)
#endif
...
#ifdef IMSTG
      /* For comparisions using VOIDmode, we have to say we have
         a full constants width of significance, else things break
         in the switch stmt below because const_op goes to 0 when op1
         is not really 0.  Consider (op0 le x) <= 1;  we cannot
         treat this like (op0 le x) <= 0. TMC summer '92 */
...
#else
      {
        if (code == LTU || code == GEU || code == LEU || code == GTU ||
            const_op >= 0)
          const_op &= mask;
        else
        {
