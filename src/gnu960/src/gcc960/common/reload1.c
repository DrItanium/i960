...
#ifdef IMSTG
	  in_struct_unsafe = MEM_IN_STRUCT_UNSAFE (reg_equiv_mem[i]);
#endif
	}
...
#ifdef IMSTG
	      MEM_IN_STRUCT_UNSAFE (reg) = in_struct_unsafe;
#endif
	      PUT_CODE (reg, MEM);
	    }
...
#ifdef IMSTG
	  new->report_undef = x->report_undef;
	  new->bit8_flag = x->bit8_flag;
#endif
	  return new;
	}
      else
...
order_regs_for_reload ()
{
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
    {
#if defined(IMSTG) && defined(HARD_REGNO_FUNCTION_RESERVED_P)
      if (fixed_regs[i] || HARD_REGNO_FUNCTION_RESERVED_P(i))
#else
      if (fixed_regs[i])
#endif
	{
...
#ifdef IMSTG
      if (asm_noperands (PATTERN (insn)) >= 0)
        error_for_asm (insn, "can't find reload register for `asm' operand");
#endif
      goto failure;
    }
