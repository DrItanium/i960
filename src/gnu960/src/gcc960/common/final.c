...
#endif /* not USG */
#else
#include "stab.h" /* use IMSTG stab.h */
#endif
...
#ifdef IMSTG
/* Highest line number seen yet. */
static int largest_linenum;
#endif
     ...
     int optimize;
{
  ...
  {
    if (flag_linenum_mods && !TARGET_CAVE)
      dwarfout_simplify_notes(first);

    /* Mark the boundaries of location expressions' valid ranges */
...
#if defined(IMSTG)
    /* This has already been done for the CAVE stub function. */
    if (!TARGET_CAVE)
#endif
    {
...
#else
      {
        /* we cheat like crazy here to get sdb info decently done.
         * First do the .bf for the function, then we immediately put out
         * the source line for the function. */
        last_linenum = NOTE_LINE_NUMBER (first);
        largest_linenum = last_linenum;
...
/* Perform final_start_function for a CAVE stub function. */
void
...
FILE	*file;
{
  if (first_lnote)
  {
    ...
    if (write_symbols == SDB_DEBUG)
    {
      last_linenum = NOTE_LINE_NUMBER (first_lnote);
      largest_linenum = last_linenum;
      sdbout_begin_function(last_linenum);
    }
#endif
    ...
    if (write_symbols == DWARF_DEBUG)
    {
      NOTE_SET_REAL_SRC_COL(first_lnote,
	imstg_map_sinfo_col_to_real_col(NOTE_LISTING_START(first_lnote)));
    }
#endif
    output_source_line(file, first_lnote);
  }
}

/* Output line number info for 'return' instruction of a CAVE stub function. */
void
...
FILE	*file;
{
  ...
  if (write_symbols == DWARF_DEBUG)
  {
    NOTE_SET_REAL_SRC_COL(last_lnote,
	imstg_map_sinfo_col_to_real_col(NOTE_LISTING_START(last_lnote)));
  }
#endif
  if (last_lnote)
    output_source_line(file, last_lnote);
}

/* Perform final_end_function for a CAVE stub function. */
void
...
#ifdef SDB_DEBUGGING_INFO
  if (write_symbols == SDB_DEBUG)
    sdbout_end_function (largest_linenum);
...
#ifdef IMSTG
  start_func_listing_info (asm_out_file, first);
#endif
  ...
  switch (GET_CODE (insn))
	...
	break;
      ...
      if (NOTE_LINE_NUMBER(insn) > 0 &&
          (NOTE_LISTING_START(insn) || NOTE_LISTING_END(insn)))
        output_list_note_info (file, insn);

#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
      if (NOTE_LINE_NUMBER (insn) == NOTE_INSN_DWARF_LE_BOUNDARY)
        {
	  if (write_symbols == DWARF_DEBUG)
	    dwarfout_le_boundary(NOTE_LE_BOUNDARY_NUMBER(insn));
	  break;
        }
#endif
#endif
...
#endif
#ifdef DWARF_DEBUGGING_INFO
#if !defined(IMSTG) || DWARF_VERSION < 2
	  if (write_symbols == DWARF_DEBUG)
...
#else
	    sdbout_begin_block (file, largest_linenum, next_block_index);
#endif
#endif
#ifdef XCOFF_DEBUGGING_INFO
	  if (write_symbols == XCOFF_DEBUG)
...
#else
	    sdbout_end_block (file, largest_linenum, 
				pending_blocks[block_depth]);
#endif
#endif
#ifdef DWARF_DEBUGGING_INFO
...
#ifdef IMSTG
          /* If this is the only place this line number will occur,
           * make sure it gets put out here.  Also output it if generating
	   * enhanced line number info, since redundant/useless notes will
	   * already have been eliminated.
	   */
          if (flag_linenum_mods || line_note_exists[NOTE_LINE_NUMBER(insn)] ==1)
            note_after = 0;
#endif
     ...
     rtx insn;
{
  ...
  if (write_symbols != NO_DEBUG)
...
#endif
#ifdef IMSTG /* no they don't, using wrong line numbers screw up badly */
	  /* COFF can't handle multiple source files--lose, lose.  */
	  && !strcmp (filename, main_input_filename)
#endif
	...
	{
#ifdef IMSTG
          if (last_linenum > largest_linenum) largest_linenum = last_linenum;
#endif
...
#else
      /* SUBREG_WORD(x) is not available after PUT_CODE(x,REG), because
         PUT_CODE clears dfa fields, which overlap SUBREG_WORD. */
      REGNO (x) = REGNO (y) + SUBREG_WORD (x);
      PUT_CODE (x, REG);
#endif
    }
