...
#if !defined(IMSTG) || !defined(SELFHOST)
#include <sys/stat.h>
...
#ifdef IMSTG
/* Counter is used to uniquely name function sections within a file. */
int func_number = 0;
#endif
...
#else
void error_with_file_and_line PVPROTO((char *file, int line, int list_pos, char *s, ...));
void warning_with_file_and_line PVPROTO((char *file, int line, int list_pos, char *s, ...));
void pedwarn_with_file_and_line PVPROTO((char *file, int line, int list_pos, char *s, ...));
#endif
...
#ifdef IMSTG
int sizeof_f_options = sizeof (f_options);
#endif
...
#else
	  char* s = "warnings being treated as errors";
          message_with_file_and_line (ER_WARNING,0,0,LOOK_LPOS,"%s",s,0,0);
#endif
	  warning_message = 1;
	}
      errorcount++;
...
#endif
  if (name && name[0])
    message_with_file_and_line (ER_FATAL,0,0,LOOK_LPOS, "%s: %s",
                                name, sys_errlist[errno], 0);
  else
    message_with_file_and_line (ER_FATAL,0,0,LOOK_LPOS,"%s",
                                sys_errlist[errno], 0, 0);
  if (flag_find_bname) abort_find_bname(0);  else
#endif
  exit (35);
}
...
#else
  message_with_file_and_line (ER_FATAL,0,0,LOOK_LPOS,"%s: I/O error",name,0,0);
  if (flag_find_bname) abort_find_bname(0);  else
#endif
  exit (35);
}
...
#ifdef IMSTG
internal_error (s, v, v1, v2, v3)
char *s;
{
  if (s)
  { extern void report_error_function();
    count_error (0);
    report_error_function (input_filename);
    message_with_file_and_line (ER_INTERNAL,input_filename,lineno,LOOK_LPOS,s,v,
                                v1, v2, v3);
  }
  if (flag_find_bname)
    abort_find_bname(0);
...
#else
v_error_with_file_and_line (file, line, list_pos, s, ap)
#endif
     char *file;
     int line;
#ifdef IMSTG
...
#else
  v_message_with_file_and_line (ER_ERROR, file, line, list_pos, s, ap);
#endif
}
...
#else
error_with_file_and_line VPROTO((char *file, int line, int list_pos, char *s, ...))
#endif
{
#ifndef __STDC__
  char *file;
...
#else
  v_message_with_decl (ER_ERROR, decl, s, ap);
#endif
}
     ...
     va_list ap;
{
  ...
  v_message_with_file_and_line (file, line, (char *)NULL, s, ap);
#else
  v_message_with_file_and_line (ER_ERROR, file, line, 0, s, ap);
#endif
}
...
#else
  v_error_with_file_and_line (input_filename, lineno, LOOK_LPOS, s, ap);
#endif
}
     ...
     va_list ap;
{
  ...
  if (s)
  {
    ...
    count_error (0);
    report_error_function (input_filename);
    v_message_with_file_and_line (ER_FATAL, input_filename, lineno,
                                  LOOK_LPOS, s, ap);
  }
  if (flag_find_bname) abort_find_bname(0);  else
#endif
...
#else
v_warning_with_file_and_line (file, line, list_pos, s, ap)
#endif
     char *file;
     int line;
#ifdef IMSTG
...
#else
      v_message_with_file_and_line (ER_WARNING, file, line, list_pos, s, ap);
#endif
    }
}
...
#else
warning_with_file_and_line VPROTO((char *file, int line, int list_pos, char *s, ...))
#endif
{
#ifndef __STDC__
  char *file;
...
#else
      v_message_with_decl (ER_WARNING, decl, s, ap);
#endif
    }
}
  ...
  if (count_error (1))
    {
      ...
      v_message_with_file_and_line (file, line, "warning", s, ap);
#else
      v_message_with_file_and_line (ER_WARNING, file, line, 0, s, ap);
#endif
    }
}
...
#else
  v_warning_with_file_and_line (input_filename, lineno, LOOK_LPOS, s, ap);
#endif
}
...
#else
v_pedwarn_with_file_and_line (file, line, list_pos, s, ap)
#endif
     char *file;
     int line;
#ifdef IMSTG
...
#else
pedwarn_with_file_and_line VPROTO((char *file, int line, int list_pos, char *s, ...))
#endif
{
#ifndef __STDC__
  char *file;
     ...
     va_list ap;
{
  ...
  char mibuf[1024];
  char*p = mibuf;
  strcat (mibuf, "sorry, not implemented: ");
  p += strlen (p);
  vsprintf (p, s, ap);
  error ("%s", p);
#else
     ...
     char *name;
{
...
#ifdef IMSTG
  scan_opt_pass++;
  { int i; for (i = 1; i < save_argc; i++) scan_option (i); }
#endif
  ...
  if (finput == 0)
    pfatal_with_name (name);
#ifdef IMSTG
  setup_input_file(&name,&finput);
#endif

#ifndef IMSTG	/* This is illegal if setup_input_file seeks finput */
...
#else
    jump_opt_dump_file = open_dump_file (dump_base_name, ".jp");
#endif
...
#else
    loop_dump_file = open_dump_file (dump_base_name, ".lp");
#endif
...
#else
    flow_dump_file = open_dump_file (dump_base_name, ".flw");
#endif
...
#else
    sched_dump_file = open_dump_file (dump_base_name, ".sc1");
#endif
...
#else
    local_reg_dump_file = open_dump_file (dump_base_name, ".lrg");
#endif
...
#else
    global_reg_dump_file = open_dump_file (dump_base_name, ".grg");
#endif
...
#else
    sched2_dump_file = open_dump_file (dump_base_name, ".sc2");
#endif
...
#else
	warning ("Errors detected in input file (your bison.simple is out of date)");
#endif
...

  {
	...
	if (TREE_CODE (decl) == VAR_DECL && DECL_SIZE (decl) == 0
	  ...
	  (*incomplete_decl_finalize_hook) (decl);
        ...
        {
          /*
           * Special hacks to get unknown structure tags to come out correctly
           * in COFF debug info.
           */
...
#ifdef IMSTG
		    || TREE_LANG_FLAG_6(decl)
		    || TREE_LANG_FLAG_6(DECL_ASSEMBLER_NAME (decl))
#endif
		    || TREE_SYMBOL_REFERENCED (DECL_ASSEMBLER_NAME (decl))))
	      {
		reconsider = 1;
...
#ifdef IMSTG
  if (flag_find_bname)
  { if (top_level && DECL_INITIAL(decl) && TREE_PUBLIC (decl))
      found_bname (IDENTIFIER_POINTER (DECL_NAME (decl)));
  }
  if (top_level)
...
#else
#ifdef PRAGMA_INLINE
      int specd = DECL_INLINE(decl) || PRAGMA_INLINE(decl);
#else
      int specd = DECL_INLINE (decl);
#endif
#endif
...
#ifdef IMSTG
      { extern char* function_dump_name;
        if (function_dump_name)
          if (strcmp (function_dump_name,IDENTIFIER_POINTER(DECL_NAME(decl))))
            turn_dumps_off ();
          else
            turn_dumps_on ();
      }
...
/*   fixup_stack_slots (); */
...
#ifdef IMSTG
  /* func_number uniquely names each section within a file. */
  func_number++;
  if (flag_find_bname || imstg_function_start (current_function_decl))
    goto exit_rest_of_compilation;
#endif
  ...
  if (flag_coerce || flag_dead_elim)
    TIMEVAR (coerce_time, insn_subst_si_for_qihi(1));

  if (flag_shadow_mem || flag_coerce)
  {
    TIMEVAR (dataflow_time, update_dataflow (-1, flag_shadow_mem));

    if (flag_coerce)
      coerce_opt (current_function_decl, 1);

    if (flag_shadow_mem)
      shadow_mem_opt (current_function_decl);

    if (flag_coerce)
      TIMEVAR (coerce_time, insn_subst_si_for_qihi (2));
  }
#endif
...
{
#ifdef TMC_TS
  int save_flag_unroll_loops, save_flag_unroll_all_loops, save_flag_sblock;
#endif
  ...
  save_flag_sblock = flag_sblock;
  if (ts_info_target (TS_SBLK) > 0)
    flag_sblock = 0;
#endif

  imstg_sblock (decl, insns);

#ifdef TMC_TS
  flag_sblock = save_flag_sblock;
#endif
}
#endif
  if (cse2_dump)
...
#ifdef IMSTG
  if (flag_shadow_mem > 1 ||flag_coerce)
  {
    TIMEVAR (dataflow_time, update_dataflow(-1,(flag_shadow_mem ? 1 : 0)));

    if (flag_coerce)
      coerce_opt (current_function_decl, 2);

    if (flag_shadow_mem > 1)
      shadow2_mem_opt (current_function_decl);

    if (flag_coerce)
      TIMEVAR (coerce_time, insn_subst_si_for_qihi (3));
  }
  split_mem_opt(decl);
  ...
  if (obey_regdecls)
    {
      ...
      /* We currently want to do this just before register allocation. */
      if (write_symbols == DWARF_DEBUG)
        TIMEVAR (symout_time, dwarfout_prep_for_location_tracking(insns));
#endif
#endif
      TIMEVAR (flow_time,
	       {
...
#ifdef IMSTG
  /* Do BBR after sched2, since BBR messes up data flow structures */
...
#else
      TIMEVAR (jump_time, jump_optimize (insns, (flag_sblock == 0), 1, 0));
#endif
    }
...
#ifdef IMSTG
#ifdef TMC_TS
  ts_info_target (TS_END);
#endif
#ifdef PRE_FINAL_OPT
  PRE_FINAL_OPT(decl, insns);
#endif
#endif
...
#if defined(IMSTG)
#ifdef IMSTG_CPLUSPLUS
#define gnu960_ver cc1plus960_ver
#else
#define gnu960_ver cc1960_ver
#endif
     ...
     char **envp;
{
...
#endif
#ifdef SELFHOST
  { typedef struct { int argc; char **argv; } ac_av;
    extern ac_av _get_argv();
    register ac_av t;
    t = _get_argv();
    argc = t.argc;
    argv = t.argv;
  ...
  check_db_file (&argc, &argv, &filename);
  bzero ((db_argv=xmalloc(argc+1)), argc+1);
#endif
...
#if defined(IMSTG) && defined(DOS)
      else if ((argv[i][0] == '-' || argv[i][0] == '/') && argv[i][1] == 'O')
#else
...
#ifdef IMSTG
  if ((BUILD_DB || PROF_CODE || USE_DB) && optimize < 1)
    optimize = 1;
#endif
...
#ifndef IMSTG
  /* Target_flags needs to be set by OPTIMIZATION_OPTIONS */
  target_flags = 0;
  set_target_switch ("");
#else
  set_target_switch ("",0);
  ...
  for (i = 1; i < argc; i++)
      ...
      else if (argv[i][0] == '-' && argv[i][1] != 0)
	{
	  ...
	  if (str[0] == 'm')
...
#else
	    set_target_switch (&str[1],i);
#endif
	  else if (!strcmp (str, "dumpbase"))
	    {
	      dump_base_name = argv[++i];
#if defined(IMSTG) && defined(DOS)
	      (void) normalize_file_name(dump_base_name);
#endif
	    }
	  else if (str[0] == 'd')
	    {
	    ...
	    inhibit_warnings = 1;
#ifdef IMSTG
	  else if (!strcmp (str, "w0") || !strcmp (str, "w1"))
	  {
		/* -w1: enable warnings ic960 issues by default. */
		/* -w0: -w1 plus warnings ic960 issues as remarks. */
		...
		if (!strcmp (str, "w0"))
		{
			/* If warn_uninitialized hasn't been explicitly set,
			 * then enable it implicitly.  Use 2 instead of 1 to
			 * suppress warning when not using high optimization.
			 */
...
You Lose!  You must define PREFERRED_DEBUGGING_TYPE!
...
#if defined(IMSTG) && defined(DOS)
	      (void) normalize_file_name(asm_file_name);
#endif
	    }
	  else if (str[0] == 'G')
	    {
...
#if defined(IMSTG) && defined(DOS)
	      (void) normalize_file_name(aux_info_file_name);
#endif
	    }
	  else
	    error ("Invalid option `%s'", argv[i]);
	...
	{
	  filename = argv[i];
	  (void) normalize_file_name(filename);
	}
#endif
    }
  ...
  if (version_flag)
    {
      ...
      fprintf (stderr, "%s\n", gnu960_ver);
#else
      fprintf (stderr, "%s %s", language_string, version_string);
