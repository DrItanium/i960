...
%token IMSTG_ASM_IDENTIFIER IMSTG_INTADDR
%type <itype> IMSTG_ASM_IDENTIFIER
%type <ttype> ASM_KEYWORD
%type <ttype> imstg_asm_body_or_error
%type <ttype> imstg_asm_body
%type <ttype> imstg_asm_leaf_list
%type <ttype> imstg_asm_leaf
%type <ttype> imstg_asm_percent
%type <ttype> imstg_asm_newline
%type <ttype> imstg_asm_ctrl_sect
%type <ttype> imstg_asm_ctrl_list
%type <ttype> imstg_asm_ctrl
%type <ttype> imstg_asm_ctrl_class_id_list
%type <ttype> imstg_asm_const
%type <ttype> imstg_asm_expr


%{
...
structsp:
	...
	| STRUCT '{'
		{ $$ = start_struct (RECORD_TYPE, NULL_TREE);
		  /* Start scope of tag before parsing components.  */
		}
	  component_decl_list '}'
...
if_prefix:
	  IF '(' expr ')'
		{ imstg_emit_line_note($<filename>-1, $<lineno>0, @0.lpos,0);
	...
	| FOR
	  ...
	  xexpr ')'
		{ 
		  ...
		  imstg_emit_line_note($<filename>8, $<lineno>7, $<lineno>2,0);
		  if ($6)
		    expand_exit_loop_if_false (NULL_PTR,
...
/* gcc960 does not support computed goto.
	| GOTO '*' expr ';'
		{ stmt_count++;
		  imstg_emit_line_note($<filename>-1, $<lineno>0, @0.lpos,0);
...
/* Special productions for ic960 asm functions. */
identifier
	: ASM_KEYWORD
	;
...
identifiers:
	ASM_KEYWORD
		{ $$ = build_tree_list (NULL_TREE, $1); }
	| identifiers ',' ASM_KEYWORD
		{ $$ = chainon ($1, build_tree_list (NULL_TREE, $3)); }
	;

fndef:
	  ASM_KEYWORD typed_declspecs setspecs declarator
	  { if (!start_asm_function ($2, $4, 0))
	    { clear_asm_state();
	      YYERROR1;
	    }
	    check_asm_parms();
	  }
	  imstg_asm_body_or_error
	  { finish_asm_function (0); }

	| ASM_KEYWORD setspecs notype_declarator
	  { if (! start_asm_function (NULL_TREE, $3, 0))
	    { clear_asm_state();
	      YYERROR1;
	    }
	    check_asm_parms();
...
imstg_asm_leaf_list
	: imstg_asm_leaf
	| imstg_asm_leaf_list imstg_asm_leaf {}
	;

imstg_asm_leaf
	: imstg_asm_percent imstg_asm_ctrl_sect imstg_asm_newline
	| imstg_asm_percent imstg_asm_newline 
	| error { clear_asm_state(); } imstg_asm_leaf {}
	;
...
imstg_asm_ctrl_list
	: imstg_asm_ctrl
	| imstg_asm_ctrl_list ';' imstg_asm_ctrl {}
	;

imstg_asm_ctrl
	: IMSTG_ASM_IDENTIFIER			{ asm_ctrl_class($1,NULL_TREE);}
	  imstg_asm_ctrl_class_id_list		{}

	| IMSTG_ASM_IDENTIFIER imstg_asm_const	{ asm_ctrl_class($1,$2); }
	  imstg_asm_ctrl_class_id_list		{}

	| IMSTG_ASM_IDENTIFIER			{ asm_ctrl_key($1); }
	;

imstg_asm_ctrl_class_id_list
	: IMSTG_ASM_IDENTIFIER
	  { asm_ctrl_class_id ($1); }

	| imstg_asm_ctrl_class_id_list ',' IMSTG_ASM_IDENTIFIER
	  { asm_ctrl_class_id ($3); }
	;
...
unary_expr
	: IMSTG_INTADDR '(' expr  ')'
	{ $$ = build_c_cast (integer_type_node,$3); }
	;

%%
