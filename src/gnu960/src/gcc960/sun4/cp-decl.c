...
#else
		      warning_with_file_and_line (file, line, LOOK_LPOS,
#endif
...
#ifdef IMSTG
  bool_type_node = make_unsigned_type (INT_TYPE_SIZE); /* for better code */
#else
...
#else
		  imstg_emit_line_note (DECL_SOURCE_FILE (decl),
				        DECL_SOURCE_LINE (decl),
				        DECL_SOURCE_POS (decl), 0);
#endif
...
#ifdef IMSTG
      DECL_SOURCE_POS (no_return_label) = LOOK_LPOS;
#endif
