...
#ifdef IMSTG
tree ptr_default_function_type;
#endif
...
#ifdef IMSTG
unsigned pending_invalid_xref_pos;
#endif
...
#ifdef IMSTG
static unsigned current_function_prototype_pos;
#endif
...
#if defined(IMSTG) && defined(DEFAULT_SIGNED_BITFIELDS)
     ...
     register tree newdecl, olddecl;
{
  ...
  /* Permit char *foo () to match void *foo (...) if not pedantic,
	...
	{
#ifndef IMSTG
          /* This is bogus - these messages can happen when an inlineable
             definition follows a declaration that did not say 'inline'.
             TMC Fall '92 */
...
#else
          {
	    warning_with_decl (newdecl, "static declaration for `%s' follows non-static; static ignored");
            TREE_PUBLIC(newdecl) = 1;
          }
#endif
...
#ifdef IMSTG
	  DECL_SOURCE_POS (newdecl) = DECL_SOURCE_POS (olddecl);
#endif
...
#ifdef IMSTG
  DECL_SOURCE_LINE (decl) = PREV_LINE;
  DECL_SOURCE_FILE (decl) = PREV_FILE;
  DECL_SOURCE_POS  (decl) = PREV_LPOS;
#endif
...
#ifdef IMSTG
		  pending_invalid_xref_file = LOOK_FILE;
		  pending_invalid_xref_line = LOOK_LINE;
		  pending_invalid_xref_pos  = LOOK_LPOS;
#endif
		}
...
#ifdef IMSTG
  builtin_function ("__builtin_argptr",
		    build_function_type (ptr_type_node, NULL_TREE),
		    BUILT_IN_ARGPTR, 0);
  builtin_function ("__builtin_argsize", default_function_type,
		    BUILT_IN_ARGSIZE, 0);
#endif
...
#ifdef IMSTG
#define EXACTLY_INT (( \
  (explicit_int && !(((1<<(int)RID_CHAR)|(1<<(int)RID_SHORT)|(1<<RID_LONG)| \
                    (1<<(int)RID_SIGNED)|(1<<RID_UNSIGNED)) & specbits)) || \
  (typedef_decl && C_TYPEDEF_EXACTLY_INT (typedef_decl)) ))
	  && (!(TARGET_IC_COMPAT3_0) || EXACTLY_INT)
#endif
  ...
  while (declarator && TREE_CODE (declarator) != IDENTIFIER_NODE)
      ...
      else if (TREE_CODE (declarator) == CALL_EXPR)
	{
	 ...
	 callers would ignore them, and pass arguments wrong.  */
	  ...
	  volatilep = 0;
#else
	/* Actually, ANSI says the application of const or volatile
	 * to a function type yields undefined behavior.  We define
	 * the behavior to be the GNU extensions, so that const and
	 * volatile apply to the function, not to its return value.  (mlb)
	 */
#endif
...
#if defined(IMSTG) && defined(STRUCT_SET_PRAGMA_ALIGN)
  STRUCT_SET_PRAGMA_ALIGN(ref);
#endif
      return ref;
    }
...
#if defined(IMSTG) && defined(MEMBER_SET_PRAGMA_PACK)
  MEMBER_SET_PRAGMA_PACK(value);
#endif
...
#if defined(IMSTG) && defined(STRUCT_DONE_PRAGMA_ALIGN)
  STRUCT_DONE_PRAGMA_ALIGN(t);
#endif
  ...
  for (x = fieldlist; x; x = TREE_CHAIN (x))
      ...
      if (DECL_INITIAL (x))
	{
	  ...
	  if (tree_int_cst_sgn (DECL_INITIAL (x)) < 0)
	    ...
	    {
#ifdef IMSTG
	      /* If ic960, we honor the field width, assuming we can */
	      if (!(TARGET_IC_COMPAT3_0) || width > TYPE_PRECISION(integer_type_node))
#endif
	      DECL_INITIAL (x) = NULL;
...
#else
  i960_setup_enum (&enumtype, &minnode, &maxnode);
#endif
     ...
     int nested;
{
  ...
  /* Instance number isn't 32-bits, prevent overflow by starting over at 0
     for each function.
   */
  imstg_note_instance_counter = 0;
#endif
...
#ifdef IMSTG
      current_function_prototype_pos  = DECL_SOURCE_POS (old_decl);
#endif
    }
...
#if defined(IMSTG) && defined(PROMOTE_RETURN)
      if (PROMOTE_RETURN)
#endif
...
#ifdef IMSTG
	      DECL_SOURCE_POS (found) = DECL_SOURCE_POS (fndecl);
#endif
	      if (extra_warnings)
...
#ifdef IMSTG
					    current_function_prototype_pos,
#endif
...
#else
  init_function_start (fndecl, LOOK_FILE, LOOK_LINE, LOOK_LPOS);
#endif
...
/*  TREE_READONLY (fndecl) = 1;
  ...
  if (write_symbols == SDB_DEBUG)
  {
     ...
     * in COFF debug info.
     */
    for (link = current_binding_level->tags; link != 0; link = TREE_CHAIN (link))
    {
      tree type = TREE_VALUE(link);
      if (type != 0 &&
