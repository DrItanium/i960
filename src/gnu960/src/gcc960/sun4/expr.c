...
int get_pointer_alignment PROTO((tree, unsigned));
#else
...
#else
      pat = gen_load_multiple (embed_type (gen_rtx (REG, word_mode, regno), t), x,
#endif
			       GEN_INT (nregs));
      if (pat)
	{
...
#else
    emit_move_insn (embed_type (gen_rtx (REG, word_mode, regno + i), t),
#endif
...
#else
      pat = gen_store_multiple (x, embed_type (gen_rtx (REG, word_mode, regno), t),
#endif
				GEN_INT (nregs));
      if (pat)
	{
...
#else
      emit_move_insn (tem, embed_type (gen_rtx (REG, word_mode, regno + i), t));
#endif
    }
}
...
#else
	  move_by_pieces (gen_typed_mem_rtx (type, BLKmode, gen_push_operand ()), xinner,
#endif
...
#else
	      move_by_pieces (gen_typed_mem_rtx (type, BLKmode, temp), xinner,
#endif
			      INTVAL (size), align);
	      goto ret;
	    }
...
#else
      emit_move_insn (gen_typed_mem_rtx (type, mode, addr), x);
#endif
    }
     ...
     enum expand_modifier modifier;
{
  ...
  switch (code)
...
#else
	  return change_address_type (DECL_RTL (exp), mode,addr,TREE_TYPE(exp));
#endif
	}
...
#else
    {
      rtx ret_decl_rtl = DECL_RTL(exp);

      if (!in_static_init &&
          GET_CODE(ret_decl_rtl) == MEM &&
          GET_CODE(XEXP(ret_decl_rtl,0)) == SYMBOL_REF &&
          (pid_flag || TARGET_PID) &&
          (SYMREF_ETC(XEXP(ret_decl_rtl,0)) & SYMREF_PIDBIT) != 0)
      {
        char buf[1000];
        rtx new_sym;
        rtx old_sym = XEXP(ret_decl_rtl,0);

        if (pid_flag)
        {
          sprintf (buf, "%s.v", XSTR(old_sym,0));
          new_sym = gen_symref_rtx(Pmode, buf);

          SYMREF_ETC(new_sym) = 0;
          SYMREF_ADDRTAKEN(new_sym) = SYMREF_ADDRTAKEN(old_sym);
          SYMREF_SIZE(new_sym) = SYMREF_SIZE(old_sym);
          RTX_TYPE(new_sym) = RTX_TYPE(old_sym);
        }
        else
          new_sym = old_sym;

        ret_decl_rtl = copy_rtx(ret_decl_rtl);
        XEXP(ret_decl_rtl,0) = gen_rtx(PLUS, Pmode, pid_reg_rtx, new_sym);
      }

      /* This is the case of an array whose size is to be determined
	 from its initializer, while the initializer is still being parsed.
	 See expand_decl.  */
      if (GET_CODE (ret_decl_rtl) == MEM
	  && GET_CODE (XEXP (ret_decl_rtl, 0)) == REG)
	return change_address_type (ret_decl_rtl, GET_MODE (ret_decl_rtl),
			       XEXP (ret_decl_rtl, 0), TREE_TYPE(exp));
      if (GET_CODE (ret_decl_rtl) == MEM
	  && modifier != EXPAND_CONST_ADDRESS
	  && modifier != EXPAND_SUM
	  && modifier != EXPAND_INITIALIZER)
	{
	  ...
	  if (!memory_address_p (DECL_MODE (exp), XEXP (ret_decl_rtl, 0))
	      || (flag_force_addr
		  && CONSTANT_ADDRESS_P (XEXP (ret_decl_rtl, 0))))
	    return change_address_type (ret_decl_rtl, VOIDmode,
				   copy_rtx (XEXP (ret_decl_rtl, 0)), TREE_TYPE(exp));
	}
      return embed_type (ret_decl_rtl, TREE_TYPE(exp));
    }
#endif

    case INTEGER_CST:
#ifdef IMSTG
      return cvt_expr (exp, immed_double_const (TREE_INT_CST_LOW (exp),
	       ...
	       || (modifier == EXPAND_INITIALIZER && TREE_CONSTANT (exp)))
	{
	  ...
	  if (modifier != EXPAND_CONST_ADDRESS
	    ...
	    constructor = change_address (constructor, VOIDmode,
					  XEXP (constructor, 0));
#else
	    constructor = change_address_type (constructor, VOIDmode,
					  XEXP(constructor,0),TREE_TYPE(exp));
#endif
	  return constructor;
	}
...
#ifdef IMSTG
	  target = embed_type (target, TREE_TYPE(exp));
#endif
	  store_constructor (exp, target);
	  return target;
	}
...
#else
	temp = gen_typed_mem_rtx (TREE_TYPE(exp), mode, op0);
#endif
...
#else
	/* What crap.  Addresses produced by addition can alias scalars. */
	if (0
#endif
...
#ifdef IMSTG
        /* Put in field type.  */
        op0 = embed_type (op0, get_outer_union_type(exp));
#endif
...
#else
	  op0 = gen_typed_mem_rtx (type, mode1, plus_constant (XEXP (op0, 0),
						    (bitpos / BITS_PER_UNIT)));
#endif
	else
...
#ifdef IMSTG
      op0 = cvt_expr (exp, op0, EXPAND_NORMAL);
#endif
...
#if defined(IMSTG) && defined(ARGSIZE_RTX)
    case BUILT_IN_ARGSIZE:	/* return # bytes in argument list */
      return ARGSIZE_RTX;
#endif
#if defined(IMSTG)
    case BUILT_IN_ARGPTR:	/* return argument pointer */
      return copy_to_reg (current_function_internal_arg_pointer);
#endif
...
#ifdef IMSTG
	  tree mtype = TREE_TYPE(TREE_TYPE (TREE_VALUE (arglist)));
#endif
...
#else
				      gen_typed_mem_rtx (mtype, BLKmode, src_rtx),
#endif
...
#ifdef IMSTG
	  tree dtype = TREE_TYPE(TREE_TYPE (dest));
	  tree stype = TREE_TYPE(TREE_TYPE (src));
#endif
...
#else
	  dest_mem = gen_typed_mem_rtx (dtype, BLKmode,
#endif
			      memory_address (BLKmode, dest_rtx));
#ifndef IMSTG
	  src_mem = gen_rtx (MEM, BLKmode,
#else
	  src_mem = gen_typed_mem_rtx (stype, BLKmode,
#endif
...
#ifdef IMSTG_UNALIGNED_MACROS
    case BUILT_IN_GET_UNALIGNED:
    case BUILT_IN_SET_UNALIGNED:
    {   extern rtx imstg_expand_unaligned_builtin();
        return imstg_expand_unaligned_builtin (exp, target, ignore);
    }
#endif /* IMSTG_UNALIGNED_MACROS */
...
#ifdef IMSTG
  /* Indicate that this thing can't alias anything we care about. */
  RTX_TYPE(vector)=build_function_type(integer_type_node,0);
  IS_CASE_MEM(vector) = 1;
#endif
