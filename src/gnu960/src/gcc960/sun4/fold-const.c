...
#ifdef IMSTG
  /*
   * We do this because make_bit_field_ref doesn't preserve the volatile
   * informaton.
   */
  if (lvolatilep)
    return 0;
      ...
      if (!wins)
	{
	  ...
	  if (split_tree (arg0, code, &var, &con, &varsign))
	      ...
	      if (varsign == -1)
		{
		  ...
		  TREE_OPERAND (t, 0)
#ifdef IMSTG
		    = fold (build (code, pick_fold_type(t,con,arg1),con,arg1));
#else
	      ...
	      else
		{
		  ...
		  TREE_OPERAND (t, 1) = tem
#ifdef IMSTG
		    = fold (build (code, pick_fold_type(t,arg1,con),arg1,con));
#else
	  ...
	  if (split_tree (arg1, code, &var, &con, &varsign))
		  ...
		  && operand_equal_p (var, arg0, 0))
		{
	      ...
	      TREE_OPERAND (t, 0)
#ifdef IMSTG
		= fold (build (code, pick_fold_type (t,arg0,con), arg0, con));
#else
...
#ifdef IMSTG
      /*
       * This is ifdefed out because we don't believe that this is a
       * completely legal thing to do.
       */
#else
