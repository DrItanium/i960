...
#ifdef IMSTG
  /* They have this wrong.  0x8000 0000 0000 0000 is infinity, not Nan */
  for (i = 0; i < NE - 2; i++)
    {
      if (*x++ != 0)
        return (1);
...
/* Possibly the `reserved operand' patterns on a VAX can be
   used like NaN's, but probably not in the same way as IEEE. */
    ...
    case XFmode:
#ifdef IMSTG
      nan[n=5]=0;  /* Clr nan[5], copy nan[0..3] in loop, set nan[4] after */
#else
      n = 6;
#endif
      p = XFnan;
