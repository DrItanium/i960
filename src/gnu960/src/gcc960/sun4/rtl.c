...
/* Create a new copy of an rtx.
   ...
   Also correctly handles copying ASM_OPERANDS stuff.  */

rtx
imstg_copy_rtx (orig, orig_asm_ops, copy_asm_ops, copy_asm_cons)
     register rtx orig;
     register rtvec * orig_asm_ops;
     register rtvec * copy_asm_ops;
     register rtvec * copy_asm_cons;
{
  register rtx copy;
  register int i, j;
      ...
      if (*orig_asm_ops == XVEC (orig, 3))
        {
          ...
          XVEC (copy, 3) = *copy_asm_ops;
          XVEC (copy, 4) = *copy_asm_cons;
...
#ifdef IMSTG
  copy->bit8_flag = orig->bit8_flag;
  copy->report_undef = orig->report_undef;
#endif
  ...
  for (i = 0; i < GET_RTX_LENGTH (GET_CODE (copy)); i++)
	...
	case 'e':
	  ...
	  if (XEXP (orig, i) != NULL)
	    XEXP (copy, i) = imstg_copy_rtx (XEXP (orig, i),
                                             orig_asm_ops,
                                             copy_asm_ops,
                                             copy_asm_cons);
	  break;
	...
	case 'V':
	  ...
	  if (XVEC (orig, i) != NULL)
		...
		XVECEXP (copy, i, j) = imstg_copy_rtx (XVECEXP (orig, i, j),
                                                       orig_asm_ops,
                                                       copy_asm_ops,
                                                       copy_asm_cons);
	    }
  ...
  if (code == ASM_OPERANDS && *orig_asm_ops == 0)
    {
      *orig_asm_ops = XVEC (orig, 3);
      *copy_asm_ops = XVEC (copy, 3);
      *copy_asm_cons = XVEC (copy, 4);
    }
