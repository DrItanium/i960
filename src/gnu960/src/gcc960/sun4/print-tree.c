...
#else
      fprintf (file, " file %s line %d pos %x",
	       DECL_SOURCE_FILE (node), DECL_SOURCE_LINE (node), DECL_SOURCE_POS (node));
#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
#if defined(DECL_DWARF_FIRST_ALID)
      fprintf (file, " dwarf_first_alid %d", DECL_DWARF_FIRST_ALID(node));
#endif
#endif
#endif
