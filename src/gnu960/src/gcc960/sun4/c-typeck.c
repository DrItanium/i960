     ...
     tree type;
{
  ...
  if (TREE_CODE(type1) == ENUMERAL_TYPE)
    return type_for_size (TYPE_PRECISION (type1), 1);
#endif
  return type;
}
...
#ifdef IMSTG
  if (TREE_CODE(type1) == ENUMERAL_TYPE)
    return type_for_size (TYPE_PRECISION (type1), 0);
#endif
  return type;
}
     ...
     tree array, index;
{
  ...
  if (TREE_CODE (TREE_TYPE (array)) == ARRAY_TYPE
      ...
      if (TREE_CODE (TREE_TYPE (index)) != INTEGER_TYPE)
	{
	  ...
	  return error_mark_node;
#else
          warning ("array subscript is not an integer");
          index = build_c_cast(integer_type_node, index);
...
#else
        warning ("array subscript is not an integer");
        ind = build_c_cast(integer_type_node, ind);
        if (ind == error_mark_node)
            return error_mark_node;
#endif
      }
...
#else
  /* We can't use the result_type here, because the intop can end-up
     as a pointer constant. Later, function cvt_expr in i_dataflow.c
     attaches symbols to pointer constants preventing subsequent folding.

     Alex Dreyzen. */
  intop = convert (type_for_size (POINTER_SIZE, 0),
#endif
     ...
     tree exp;
{
  register tree x = exp;
#ifdef IMSTG
  if (IS_TREE_ASM_FUNCTION(exp))
  { error ("cannot take address of asm function %s",
           IDENTIFIER_POINTER(DECL_NAME(exp)));
    return 0;
  }
  imstg_note_addr_taken(x);
#endif
  while (1)
    switch (TREE_CODE (x))
      {
...
#else
		/* Always warn about this.  Warning may come in contexts
		   other than argument passing (e.g., in a simple assignment),
		   so remove 'argument' from message.
		 */
		pedwarn ("ANSI C prohibits conversion to union type");
#endif
     ...
     tree endtype;
{
  switch (TREE_CODE (value))
	...
	return initializer_constant_valid_p (TREE_OPERAND (value, 0),
					     endtype);
#ifdef IMSTG
      /* Allow int to pointer provided ptr is as wide as the int.  */
      if (TREE_CODE (TREE_TYPE (value)) == POINTER_TYPE
	  && TREE_CODE (TREE_TYPE (TREE_OPERAND (value, 0))) == INTEGER_TYPE
     ...
     int require_constant, constructor_constant;
{
  ...
  if (flag_traditional)
#else
  /* gcc960 allows unbracketed initializers, but we warn if not trad */
  if (!flag_traditional)
    warning ("invalid initializer for `%s'",
