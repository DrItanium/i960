...
#ifdef IMSTG
  p->emit_list_lineno = emit_list_lineno;
#endif
...
#ifdef IMSTG
  emit_list_lineno = p->emit_list_lineno;
#endif
...
#ifdef IMSTG
  /*
   * emit a line note here.  This may be useful for tracking various case
   * constants for debugging or profiling.
   */
  imstg_emit_line_note (DECL_SOURCE_FILE (label),
                        DECL_SOURCE_LINE (label),
                        DECL_SOURCE_POS (label), 0);
#endif
...
expand_end_loop ()
{
  ...
  if (optimize
      ...
      if (last_test_insn != 0 && last_test_insn != get_last_insn ())
	{
...
#ifdef IMSTG
	  rtx lnote = get_last_insn();
	  /* Find the note that is currently in effect ... */
	  while (lnote && (GET_CODE(lnote)!=NOTE || NOTE_LINE_NUMBER(lnote)<=0))
	    lnote = PREV_INSN(lnote);
#endif
	  ...
	  if (lnote)
	  {
	    /* Restore the note that was in effect */
	    rtx copy = imstg_emit_line_note (NOTE_SOURCE_FILE(lnote), NOTE_LINE_NUMBER(lnote), 0,0);
	    if (copy && flag_linenum_mods)
	    {
	      NOTE_COPY_COLUMN_NUMBER(copy, lnote);
	      /* 'lnote' marks the statement beginning, not 'copy' */
...
#else
    INSN_INLN_TRANSLATE(
      emit_insn (gen_rtx (USE, VOIDmode, return_reg))) = INLN_DEL_CALLEE;
#endif
     ...
     int dont_jump_in;
{
  ...
  if (write_symbols == SDB_DEBUG)
  {
     ...
     * in COFF debug info.
     */
    for (decl = vars; decl != 0; decl = TREE_CHAIN (decl))
...
#else
      imstg_emit_line_note (DECL_SOURCE_FILE (decl), DECL_SOURCE_LINE (decl),DECL_SOURCE_POS (decl),0);
#endif
...
#ifdef IMSTG
                    {
                      /* Have to convert case constants up to this bigger type,
                         or we may generate incorrect code. */
                      tree new_type = unsignedp ?
                              make_unsigned_type(GET_MODE_BITSIZE(wider_mode)) :
                              make_signed_type(GET_MODE_BITSIZE(wider_mode));

                      for (n = thiscase->data.case_stmt.case_list; n;
                           n = n->right)
	              {
	                n->low = convert (new_type, n->low);
	                n->high = convert (new_type, n->high);
                      }
                    }
#endif
		    break;
