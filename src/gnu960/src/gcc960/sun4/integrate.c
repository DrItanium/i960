     ...
     register tree fndecl;
{
  ...
  for (parms = DECL_ARGUMENTS (fndecl); parms; parms = TREE_CHAIN (parms))
	...
	return "no prototype, and parameter address used; cannot be inline";
	 ...
	 reading or writing.  */
      /* 'parm_map' code in expand_inline_function appears not to work
         for register parms which get assigned to memory because their
         address was taken. TMC summer '92 */
      if (/* (TREE_CODE (TREE_TYPE (parms)) == RECORD_TYPE
	   || TREE_CODE (TREE_TYPE (parms)) == UNION_TYPE)
	  && */ GET_CODE (DECL_RTL (parms)) == MEM)
	return "address of a parameter is used; cannot be inline";
    }
#endif
...
#ifdef IMSTG
  NOTE_LISTING_START (first_insn) = NOTE_LISTING_START (insn);
  NOTE_LISTING_END   (first_insn) = NOTE_LISTING_END (insn);
#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
  if (flag_linenum_mods && NOTE_LINE_NUMBER(insn) >= 0)
    NOTE_AUX_SRC_INFO(first_insn) = NOTE_AUX_SRC_INFO (insn);
#endif
#endif
...
#ifdef IMSTG
	  NOTE_LISTING_START (copy) = NOTE_LISTING_START (insn);
	  NOTE_LISTING_END (copy) = NOTE_LISTING_END (insn);
#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
	  if (flag_linenum_mods && NOTE_LINE_NUMBER(insn) >= 0)
	    NOTE_AUX_SRC_INFO(copy) = NOTE_AUX_SRC_INFO (insn);
#endif
#endif
	  break;
...
#ifdef IMSTG
          INSN_INLN_TRANSLATE(copy) = INSN_INLN_TRANSLATE(insn);
          if (GET_CODE (insn) == CALL_INSN)
            INSN_CALL_PTYPE (copy) = db_cp_encode_buf(INSN_CALL_PTYPE(insn));
#endif
	  break;
     ...
     rtx structure_value_addr;
{
  ...
  for (formal = DECL_ARGUMENTS (fndecl),
    ...
    /* This makes emit_note() distinguish duplicate line number notes
       from the originals.
     */
...
#else
      rtx note = imstg_emit_note (NOTE_SOURCE_FILE (parm_insns),
				  NOTE_LINE_NUMBER (parm_insns), 0, 0);
#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
      if (note && flag_linenum_mods)
      {
	NOTE_COPY_COLUMN_NUMBER(note, parm_insns);
	NOTE_COPY_STMT_BEGIN(note, parm_insns);
      }
#endif
#endif
      if (note)
...
#else
	  rtx note = imstg_emit_note (DECL_SOURCE_FILE (formal),
				      DECL_SOURCE_LINE (formal), 0, 0);
#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
          if (note && flag_linenum_mods)
	    SET_COL(NOTE_LISTING_START(note),GET_COL(DECL_SOURCE_POS(formal)));
#endif
#endif
	  if (note)
	    RTX_INTEGRATED_P (note) = 1;
	  ...
	  if (NOTE_LINE_NUMBER (insn) != NOTE_INSN_FUNCTION_END
...
#else
	    {
	      copy=imstg_emit_note(NOTE_SOURCE_FILE(insn),
					NOTE_LINE_NUMBER(insn), 0, 0);
#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
	      if (copy && flag_linenum_mods && NOTE_LINE_NUMBER(insn) >= 0)
	      {
...
#else
  {
    rtx note = imstg_emit_line_note (input_filename, lineno, 0, 0);

#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
    /* A line number note for the statement containing the call has already
       been emitted (before the inlined insns).  We don't want to set a
       breakpoint *after* the inlined insns as well.
     */
    if (note && flag_linenum_mods)
      NOTE_CLEAR_STMT_BEGIN(note);
#endif
  }
#endif
  ...
  for (t = BLOCK_VARS (let); t; t = TREE_CHAIN (t))
    {
      ...
      d = copy_node (t);
#ifdef IMSTG
      DECL_SOURCE_POS (d) = DECL_SOURCE_POS (t);
#endif
      pop_obstacks ();
...
#else
  init_function_start (fndecl, "lossage", 1, 0);

#if defined(CHANGE_OPTIONS_FOR_FUNC)
  CHANGE_OPTIONS_FOR_FUNC(fndecl);
#endif
#endif
