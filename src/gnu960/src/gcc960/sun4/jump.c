...
rtx end;
{
  ...
  while (1)
  {
    ...
    {
      /* just change it to a NOTE_INSN_DELETED note. */
...
#else
      if (insn && GET_CODE (insn) != BARRIER &&
          (GET_CODE(insn)!=JUMP_INSN || GET_CODE(PATTERN(insn))!=RETURN))
#endif
	{
...
#else
	    /* Also delete insns to store bit fields if they are no-ops.  */
	    else if (!bytes_big_endian && (GET_CODE (body) == SET
		     && GET_CODE (SET_DEST (body)) == ZERO_EXTRACT
		     && XEXP (SET_DEST (body), 2) == const0_rtx
		     && XEXP (SET_DEST (body), 0) == SET_SRC (body)
		     && ! (GET_CODE (SET_SRC (body)) == MEM
...
#else
          if (JUMP_LABEL (insn)
              && ((temp1 = next_active_insn (JUMP_LABEL (insn))) == 0
                  || (GET_CODE(temp1) == JUMP_INSN
                      && GET_CODE(PATTERN(temp1)) == RETURN)))
#endif
...
#if defined(IMSTG)
                       	/* delete any no VTOP, or CONT notes in
			   range2.  They won't be valid any more. */
			imstg_del_cont_vtop(range2beg, range2end);
#endif
...

  {
      ...
      if (GET_CODE (insn) == NOTE && NOTE_LINE_NUMBER (insn) >= 0)
	{
	  ...
	  if (flag_linenum_mods)
	    {
	      if (last_note
		  && NOTES_REP_SAME_SOURCE_P(insn, last_note)
	          && (NOTE_GET_INSTANCE_NUMBER(insn)
		      == NOTE_GET_INSTANCE_NUMBER(last_note))
	          && (NOTE_STMT_BEGIN_P(last_note) || !NOTE_STMT_BEGIN_P(insn)))
	        {
	          delete_insn (insn);
	          continue;
	        }
     ...
     rtx loop_start;
{
  ...
  rtx *reg_map = 0;
#if defined(IMSTG)
  rtx loop_prev_insn; /* PREV_INSN(loop_start), before duplicating exit test. */
#endif
  ...
  for (insn = exitcode;
	...
	case INSN:
	  if (++num_insns > 20
#ifdef IMSTG
	      || (GET_CODE(PATTERN(insn))==PARALLEL && asm_noperands(PATTERN(insn))>=0)
#endif
...
#if defined(IMSTG)
  loop_prev_insn = PREV_INSN(loop_start);

#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
  /* This makes emit_note*() distinguish duplicate line number notes from
     the originals.
   */
  if (flag_linenum_mods)
    imstg_note_instance_counter++;
#endif
#endif
	...
	if (NOTE_LINE_NUMBER (insn) >= 0)
	  {
	    ...
	    NOTE_SOURCE_FILE (copy) = NOTE_SOURCE_FILE (insn);
            ...
            if (flag_linenum_mods)
	    {
	      NOTE_COPY_COLUMN_NUMBER(copy, insn);
	      NOTE_COPY_STMT_BEGIN(copy, insn);
	    }
#endif
	  }
	break;
...
#ifdef IMSTG
        JUMP_THEN_PROB(copy) = JUMP_THEN_PROB(insn);
#endif
	if (reg_map)
	  replace_regs (PATTERN (copy), reg_map, max_reg, 1);
	mark_jump_label (PATTERN (copy), copy, 0);
...
#if defined(IMSTG)
  /* Prevent OMF line number note differences from altering code generation. */
  while (copy && copy != loop_prev_insn && GET_CODE(copy) == NOTE)
    copy = PREV_INSN(copy);

  if (copy == loop_prev_insn)
    copy = NULL_RTX;
#endif
...
#ifdef IMSTG
        /*
         * if i1 is a conditional branch than stop, cause
         * we may not ever have to do the absolute branch
         */
      if ( GET_CODE (i1) == JUMP_INSN && condjump_p (i1) )
                break;
#endif
...
#if defined(IMSTG) && defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
    /* If we are deleting the only remaining insn corresponding to some
       line number note(s), delete those notes as well.  If we don't do
       this, the debugger may end up placing a breakpoint at an erroneous
       position.  Loop exit test duplication and constant folding lead to
       situations where deleting these notes is necessary.  As an alternative,
       we could simply mark the note as not a statement beginning.

       Don't do this if optimization is disabled, since instructions don't
       actually get deleted in that case.

       Also, don't patch out the note insns, because some optimizations
       depend on insn I being still attached after delete_insn(NEXT_INSN(I)).
     */
    if (flag_linenum_mods && optimize && !dont_really_delete)
     ...
     rtx insn;
{
  ...
  if (code == IF_THEN_ELSE)
	...
	return 1;
#else
	{	/* tricky, done if you change the cc but not if you swap operands */
		JUMP_THEN_PROB(insn) = PROB_BASE - JUMP_THEN_PROB(insn);
		return 1;
	}
#endif
