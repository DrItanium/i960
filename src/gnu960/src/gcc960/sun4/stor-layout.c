...
#endif
#if defined(IMSTG) && defined(ROUND_FIELD_ALIGN)
      ROUND_FIELD_ALIGN (field, &const_size, &desired_align, &record_align);
#endif
...
#if defined(IMSTG) && defined(TMCDEBUG)
...
#if defined(IMSTG) && defined(TMCDEBUG)
  printf ("rec s,a:%d,%d\n", TREE_INT_CST_LOW(TYPE_SIZE(rec)), TYPE_ALIGN(rec));
#endif
...
enum machine_mode
#ifdef IMSTG
get_best_mode (bitsize, bitpos, align, largest_mode, volatilep, cheat_align_ok)
#else
...
#ifdef IMSTG
      || (!cheat_align_ok && MIN (unit, BIGGEST_ALIGNMENT) > align)
#else
