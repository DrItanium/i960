...
#else
	  int icode = (int)movstrict_optab->handlers[(int) fieldmode].insn_code;
#endif
...
#ifdef IMSTG
			    GET_CODE (op0) == MEM && MEM_VOLATILE_P (op0), 1);
#else
...
#else
  /* Do not utter a non-shareable operand twice! This
     breaks big time for SUBREG, or some MEMs.  TMC Fall '92 */
  subtarget = (GET_CODE (op0) == REG) ? op0 : 0;
  op0 = copy_rtx (op0);
#endif
...
#else
    emit_move_insn (copy_rtx(op0), copy_rtx(temp));
#endif
}

     ...
     int align;
{
  ...
  if (GET_CODE(value)==MEM ||
      (CONSTANT_P (value) && GET_CODE (value) != CONST_INT))
#endif
    {
...
#endif
#else
              && (bytes_big_endian ? bitpos + bitsize == BITS_PER_WORD : bitpos == 0)
#endif /* IMSTG_BIG_ENDIAN */
	      )))
    {
...
#else
      mult_cost = rtx_cost (gen_rtx (MULT, mode, virtual_stack_vars_rtx, op1), SET);
#endif
