      ...
      for (i = 0; i < noperands; i++)
	{
	  ...
	  while (*p && (c = *p++) != ',')
		...
		break;
	      ...
	      case '4':
#ifdef IMSTG
	      /* Can you believe this ?  Without the following, cannot
	         match operands higher than 4! */
	      case '5':
	      case '6':
	      case '7':
	      case '8':
...
#else
	    decompose (recog_operand[i], &early_data);
#endif
