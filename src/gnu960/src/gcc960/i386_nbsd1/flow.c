...
unsigned char *max_bb_reg_pressure;

/*
 * allow extra flow space for some new pseudo's to be allocated by
 * 1st pass superblock scheduling, and for define_split patterns.
 */
#define ROOM_FOR_NEW_REGS 150
#endif
...
#ifdef IMSTG
void find_basic_blocks		PROTO((rtx, rtx));
#else
...
{
#ifdef IMSTG
  extern int second_time_flow;

  prep_for_flow_analysis (f);

  if (max_bb_reg_pressure != 0)
    free(max_bb_reg_pressure);
  max_bb_reg_pressure = (unsigned char *)xmalloc(N_BLOCKS);
  bzero(max_bb_reg_pressure, N_BLOCKS);

  nregs = max_reg_num();
  if (! second_time_flow && flag_schedule_insns)
    nregs += ROOM_FOR_NEW_REGS;
  ...
  xunalloc (&basic_block_drops_in);
  xunalloc (&basic_block_loop_depth);
#else
  register rtx insn;
  register int i;
	...
	if (!block_live[i])
	  {
		...
		for (j = i; j < n_basic_blocks; j++)
		  if (block_live[j])
			...
			{
#if defined(IMSTG) && defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
		          /* NO!  calling delete_insn below will do this.
		          if (flag_linenum_mods)
		            dwarfout_disable_notes(insn);
		           */
#endif
...
#ifdef IMSTG
          /* Consider a set to the stack pointer to not be dead ever.  If
             it happens to be making space, or releasing space, in either
             case, we want it to happen. */
      ...
      if (GET_RTX_CLASS (GET_CODE (insn)) == 'i')
	{
	  ...
	  int libcall_is_dead 
	    ...
	    = (insn_is_dead && note != 0
          ...
          if (note != 0)
          {
            libcall_is_dead = libcall_dead_p (PATTERN (insn), old, note, insn);
            if (libcall_is_dead == 2 ||
                (libcall_is_dead == 1 && insn_is_dead))
              libcall_is_dead = 1;
            else
              libcall_is_dead = 0;
          }
...
#else
	  if (final && (insn_is_dead || libcall_is_dead))
	    {
	      /* If this insn is copying the return value from a library call,
		 delete the entire library call.  */
	      if (libcall_is_dead)
	  ...
	  if (libcall_is_dead)
#else
          if (libcall_is_dead && insn_is_dead)
#endif
	    {
...
#else
		  register struct sometimes *p = regs_sometimes_live;
                  register int n_live = 0;
                  extern unsigned char * f_info_reg_pressure_pos;
		  ...
		  for (i = 0; i < sometimes_max; i++, p++)
		    if (old[p->offset] & (1 << p->bit))
		      ...
		      reg_n_calls_crossed[p->offset * REGSET_ELT_BITS + p->bit]+= 1;
                  ...
                  if ((BUILD_DB || PROF_CODE) && INSN_CALL_NUM(insn) >= 0)
                  {
                    if (n_live > 32) n_live = 32;
                    f_info_reg_pressure_pos[INSN_CALL_NUM(insn)] = n_live;
                  }
#endif
		}
	    }
		...
		for (i = 0; i < sometimes_max; i++, p++)
		  {
		    if (old[p->offset] & ((REGSET_ELT_TYPE) 1 << p->bit))
		      ...
		      reg_live_length[p->offset * REGSET_ELT_BITS + p->bit]++;
                ...
                if (n_live > max_bb_reg_pressure[BLOCK_NUM(insn)])
                  max_bb_reg_pressure[BLOCK_NUM(insn)] = n_live;
#endif
	      }
#ifdef IMSTG
              /*
               * Delete any REG_LIBCALL or REG_RETVAL notes.  This is
               * necessary because we may be called several times, and the
               * first time I may have deleted either the LIBCALL, or the
               * RETVAL, and next time I get called these hanging notes may
               * cause a problem (like a core dump).
               */
              {
                rtx prev_note, rg_note;
                     ...
                     prev_note = rg_note, rg_note = XEXP(rg_note, 1))
                {
                  if (REG_NOTE_KIND(rg_note) == REG_LIBCALL ||
                      REG_NOTE_KIND(rg_note) == REG_RETVAL)
                  {
                    /* delete it. */
                    if (prev_note == 0)
