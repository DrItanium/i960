...
#else
void add_dependence		PROTO((rtx, rtx, enum reg_note));
...
#ifdef IMSTG
      if (GET_CODE(x0) == PLUS && GET_CODE(XEXP(x0,1)) == CONST_INT &&
          GET_CODE(x1) != CONST_INT && GET_MODE(x) == GET_MODE(x0))
      {
        return gen_rtx(PLUS, GET_MODE(x),
                       gen_rtx(PLUS, GET_MODE(x), XEXP(x0, 0), x1),
                       XEXP(x0, 1));
      }
#endif
    }
     ...
     rtx x;
{
 ...
 if (MEM_VOLATILE_P(x) && MEM_VOLATILE_P (mem))
    return 1;
 if (!imstg_mems_overlap (mem,x))
    return 0;
 if (!memrefs_conflict_p (SIZE_FOR_MODE (mem), XEXP (mem, 0),
                          SIZE_FOR_MODE (x), XEXP (x, 0), 0))
    return 0;

 if (MEM_REALLY_IN_STRUCT (mem) && rtx_addr_varies_p (mem)
 &&  GET_MODE(mem)!=QImode
 &&  MEM_REALLY_NOT_IN_STRUCT (x) && !rtx_addr_varies_p (x)
 &&  GET_MODE(x)!=QImode)
    return 0;

 if (MEM_REALLY_IN_STRUCT (x) && rtx_addr_varies_p (x)
 &&  GET_MODE(x)!=QImode
 &&  MEM_REALLY_NOT_IN_STRUCT (mem) && !rtx_addr_varies_p (mem)
 &&  GET_MODE(mem)!=QImode)
    return 0;
