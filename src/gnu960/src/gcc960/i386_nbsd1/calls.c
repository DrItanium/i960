...
#ifdef IMSTG
  INSN_CALL_PTYPE(call_insn) = (char *)call_ptype_info;
#endif
     ...
     int ignore;
{
...
#ifdef IMSTG
              /*
               * We show that code must be generated for this function by
               * setting extra language flag 6 to indicate this.  We don't
               * use mark_addressable because this screws up the information
               * we want to collect about which functions are passed by
               * address, as opposed to just called.
               */
              TREE_LANG_FLAG_6(fndecl) = 1;
              TREE_LANG_FLAG_6(DECL_NAME (fndecl)) = 1;
#else
...
#else
#ifdef PRAGMA_PURE
	  if ((TREE_READONLY (fndecl) || PRAGMA_PURE (fndecl))
...
#ifdef IMSTG
	    struct_value_size =
	      ROUND(struct_value_size,(BIGGEST_ALIGNMENT/BITS_PER_UNIT));
#endif
...
#ifdef IMSTG
#ifdef SMALL_AGGR_RETURN_IN_REGS
          store_struct_after_call = FUNCTION_VALUE(TREE_TYPE(exp),fndecl);
#endif
#endif
      }
  ...
  if (is_integrable)
      ...
      if ((HOST_WIDE_INT) temp != -1)
	{
	  ...
	  if (write_symbols == DWARF_DEBUG)
	    dwarfout_set_function_inlined(fndecl);
#endif
	  if (flag_short_temps)
	    {
...
#if defined(IMSTG) && defined(NOTE_PASS_ON_STACK)
  ...
  for (i = 0; i < num_actuals; i++)
	...
	if (IS_TREE_ASM_FUNCTION(fndecl)
        && (GET_CODE(args[i].value)==MEM ||
             (GET_CODE(args[i].value)==REG &&
              REGNO(args[i].value) <= LAST_VIRTUAL_REGISTER)))
	{ /* Always preload memory arguments to asm functions into registers,
             before trying matching, because this is what ic960 does.
             Also, we preload hard regs here because they match anything
             a hard reg would, and we can't tolerate hard regs in the
             asm bodies we generate anyway. */
          enum machine_mode m = GET_MODE(args[i].value);
	  if (m!=VOIDmode && m!=BLKmode)
...
#else
	  args[i].stack = gen_typed_mem_rtx
            (TREE_TYPE(args[i].tree_value), args[i].mode, addr);
#endif
...
#else
	  args[i].stack_slot = gen_typed_mem_rtx
            (TREE_TYPE(args[i].tree_value), args[i].mode, addr);
#endif
	}
    }
...
#ifdef IMSTG
{ rtx t = 0; enum machine_mode m = TYPE_MODE(TREE_TYPE(exp));
  if (!(ignore |= (m==VOIDmode)))
    if (structure_value_addr)
      t = gen_rtx(MEM, m, memory_address (m,structure_value_addr));
    else if (t=target)
    { if (GET_CODE(t)==SUBREG)
        t = SUBREG_REG(t);
      if (GET_CODE(t)!=REG||REGNO(t)<=LAST_VIRTUAL_REGISTER)
        t = 0;
      else
        t = target;
    }
#ifndef IMSTG_CPLUSPLUS
  if(try_expand_asm_func(fndecl,ignore,struct_value_size,args,&funexp,&t))
  { valreg = target = t;
    goto skip_asm_func;
  }
#endif
...
#endif
#ifdef IMSTG
  emit_note(NULL_PTR, NOTE_INSN_CALL_BEGIN);
#endif
  ...
  for (i = 0; i < num_actuals; i++)
      ...
      while (list)
	{
	  ...
	  nregs = (partial ? partial
		   : ((TYPE_MODE (TREE_TYPE (args[i].tree_value)) == BLKmode) ||
                      (!HARD_REGNO_MODE_OK(REGNO(reg),GET_MODE(reg)))
		      ? ((int_size_in_bytes (TREE_TYPE (args[i].tree_value))
			  + (UNITS_PER_WORD - 1)) / UNITS_PER_WORD)
		      : -1));
#endif
  ...
  emit_call_1 (funexp, funtype, args_size.constant, struct_value_size,
	       FUNCTION_ARG (args_so_far, VOIDmode, void_type_node, 1),
	       (valreg != 0 ? valreg : store_struct_after_call),
               old_inhibit_defer_pop, call_fusage, is_const,
               db_encode_call_ptype(TREE_TYPE (exp), TREE_OPERAND (exp, 1)));

#ifdef SMALL_AGGR_RETURN_IN_REGS
    if (store_struct_after_call)
      move_from_reg (REGNO(store_struct_after_call),
                     gen_rtx (MEM, BLKmode, structure_value_addr),
                     struct_value_size,
...
skip_asm_func:
  ...
  if (is_const && valreg != 0)
...
#ifdef IMSTG
      if (GET_CODE(insns) == NOTE &&
          NOTE_LINE_NUMBER(insns) == NOTE_INSN_CALL_BEGIN)
      {
    ...
    {
      /*
       * need this to assure that function return value gets copied
       * into a pseudo that cannot turn into memory.  This supports
       * 2-pass function inlining.  This already happens for is_const
       * routines above.  Extra copy there causes problems with
       * call being mistaken for dead later in compilation.
       */
      rtx treg = gen_reg_rtx(GET_MODE(valreg));
      emit_move_insn(treg, valreg);
      valreg = treg;
    }
#endif
			  ...
			  int nargs, ...))
{
  ...
  fun = orgfun;
#ifdef IMSTG
  { extern tree ptr_default_function_type;
    set_rtx_type (fun, ptr_default_function_type);
  }
#endif
  ...
  emit_call_1 (fun, get_identifier (XSTR (orgfun, 0)), args_size.constant, 0,
	       ...
	       outmode != VOIDmode ? hard_libcall_value (outmode) : NULL_RTX,
               old_inhibit_defer_pop + 1, call_fusage, no_queue,
               db_encode_call_ptype(NULL_TREE, NULL_TREE));
#endif
