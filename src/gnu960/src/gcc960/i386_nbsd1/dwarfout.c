...
#if defined(DWARF_DEBUGGING_INFO) && defined(IMSTG) && (DWARF_VERSION <= 1)
#include <stdio.h>
#include "dwarf.h"
#include "tree.h"
#include "flags.h"
...
#define UNALIGNED_INT_ASM_OP	".word"
#define POPSECTION_ASM_OP	""

static char* i960_sec_stack[100] = { ".text", NULL };
static int i960_sstop = 0;

#define ASM_OUTPUT_START_SECTION(FILE, SECTION) \
  do { \
  if (strcmp(SECTION, ".bss") == 0 \
      || strcmp(SECTION, ".data") == 0 \
      || strcmp(SECTION, ".text") == 0) \
    (void)fprintf((FILE), "\t%s\n", SECTION); \
  else \
    (void)fprintf ((FILE), PUSHSECTION_FORMAT, PUSHSECTION_ASM_OP, SECTION); \
  } while (0)

#define ASM_OUTPUT_PUSH_SECTION(FILE, SECTION) \
  do { \
    assert(i960_sstop < 99); \
    if (i960_sstop < 0 || strcmp(i960_sec_stack[i960_sstop], SECTION) != 0) \
      ASM_OUTPUT_START_SECTION(FILE, SECTION); \
    i960_sec_stack[++i960_sstop] = SECTION; \
  } while (0)

#define ASM_OUTPUT_POP_SECTION(FILE) \
  do { \
    assert(i960_sstop >= 0); \
    if (i960_sstop > 0 && strcmp(i960_sec_stack[i960_sstop-1], i960_sec_stack[i960_sstop]) != 0) \
      ASM_OUTPUT_START_SECTION(FILE, i960_sec_stack[i960_sstop-1]); \
    i960_sec_stack[i960_sstop--] = NULL; \
  } while (0)
  ...
  if (regno >= FIRST_PSEUDO_REGISTER)
    {
      ...
      warning_with_decl (dwarf_last_decl, "internal regno botch: regno = %d\n",
			 regno);
#else
      warning_with_decl (dwarf_last_decl, "internal regno botch in declaration of %s: regno = %d\n",
			 regno);
#endif
      regno = 0;
    }
...
#else
/* need at least one declaration in an object file. */
int	dwarfout_dummy_decl;
