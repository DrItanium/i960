...
enum built_in_function
{
  ...
  BUILT_IN_GET_UNALIGNED,
  BUILT_IN_SET_UNALIGNED,
...
#ifdef IMSTG
#define TYPE_IMSTG_I960ALIGN(NODE) ((NODE)->type.imstg_i960align)
#endif
...
#ifdef IMSTG
#define DECL_SOURCE_POS(NODE)  ((NODE)->decl.srcpos)

#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
#define DECL_DWARF_FIRST_ALID(NODE)  ((NODE)->decl.dwarf_first_alid)
#endif
#endif /* IMSTG */
...
#if defined(IMSTG)
#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
    int dwarf_first_alid;
      /* For PARM_DECLs and local VAR_DECLS:  if non-zero, this is the first of
	 a contiguous set of indices that represent locations for this decl.
       */
#endif
#endif
  /* Points to a structure whose details depend on the language in use.  */
...
#ifdef IMSTG
/* The type node for the generic pointer type. */
