...
#ifdef IMSTG
#ifndef PUT_SDB_NO_UNDERSCORE_DEF
#define PUT_SDB_NO_UNDERSCORE_DEF(a) fprintf(asm_out_file,"\t.def\t%s%s",a, SDB_DELIM)
#endif
#endif
...
#else
#ifndef PUT_SDB_TAG
#define PUT_SDB_TAG(a)  fprintf (asm_out_file, "\t.tag\t%s%s", a, SDB_DELIM)
#endif
#endif
...
#ifdef IMSTG
static int sdb_dim_size[SDB_MAX_DIM];
#endif
...
#ifdef IMSTG
      sdb_type_size = sdb_dim_size[sdb_n_dims-1];
#endif
      sdb_n_dims = 0;
     ...
     tree type;
{
  ...
  {
    char *tmpstr;
    tmpstr = (char *) permalloc (strlen (name) + 12);
    sprintf (tmpstr, "%s_%d", name, unnamed_struct_number++);
    name = tmpstr;
  }
#endif
     ...
     tree type;
{
  ...
  switch (TREE_CODE (type))
	...
	int size = int_size_in_bytes (type) * BITS_PER_UNIT;
        ...
        if (TYPE_NAME(type) != 0 &&
            DECL_NAME(TYPE_NAME(type)) != 0 &&
            IDENTIFIER_POINTER(DECL_NAME(TYPE_NAME(type))) != 0)
        {
          char *tname = IDENTIFIER_POINTER(DECL_NAME(TYPE_NAME(type)));
          is_short = 0;
          is_long = 0;
          is_short = (strncmp(tname, "short ", 6) == 0);
          is_long  = !is_short && (strncmp(tname, "long ", 5) == 0);
        }
	if (size == CHAR_TYPE_SIZE)
	  return (TREE_UNSIGNED (type) ? T_UCHAR : T_CHAR);
	if (size == SHORT_TYPE_SIZE && is_short)
	  return (TREE_UNSIGNED (type) ? T_USHORT : T_SHORT);
	if (size == LONG_TYPE_SIZE && is_long)
	  return (TREE_UNSIGNED (type) ? T_ULONG : T_LONG);
	if (size == INT_TYPE_SIZE)
	  return (TREE_UNSIGNED (type) ? T_UINT : T_INT);
#endif
	...
	if (sdb_n_dims < SDB_MAX_DIM)
	  ...
	  sdb_dims[sdb_n_dims++]
	    = (TYPE_DOMAIN (type)
	       ...
	       : 0);
#ifdef IMSTG
          sdb_dim_size[sdb_n_dims-1] = int_size_in_bytes(type);
        }
#endif
...
#ifdef IMSTG
#ifdef IS_LEAF_PROC
      if (IS_LEAF_PROC(decl))
        PUT_SDB_SCL (TREE_PUBLIC (decl) ? C_LEAFEXT : C_LEAFSTAT);
      else
#endif
      if (i960_system_function(decl))
        PUT_SDB_SCL (C_SCALL);
      else
#endif
  ...
  switch (TREE_CODE (type))
...
#else
	PUT_SDB_NO_UNDERSCORE_DEF (KNOWN_TYPE_TAG (type));
#endif
	...
	if (TREE_CODE (type) == ENUMERAL_TYPE)
	  ...
	  for (tem = TYPE_FIELDS (type); tem; tem = TREE_CHAIN (tem))
		...
		PUT_SDB_DEF (name);
#else
		PUT_SDB_NO_UNDERSCORE_DEF(name);
#endif
...
{
#ifdef IMSTG
#ifdef OUTPUT_SDB_ARG_PTR_DEF
  int arg_ptr_def_out = 0;
#endif
#endif
...
#ifdef IMSTG
#ifdef OUTPUT_SDB_ARG_PTR_DEF
            if (arg_ptr_def_out == 0)
              OUTPUT_SDB_ARG_PTR_DEF();
            arg_ptr_def_out = 1;
#endif
#endif
	    PUT_SDB_DEF (name);
...
#else
            /*
             * Make sure to pick the correct code based
             * on whether the reference is relative to the frame pointer
             * or the arg pointer.
             */
            {
              int scl_code;
              ...
              x = DECL_RTL (parms);
              if (GET_CODE(x = XEXP(x,0)) == PLUS &&
                  GET_CODE(x = XEXP(x,0)) == REG &&
...
#ifdef IMSTG
  /* Output a ".def .target" directive for functions returning an aggregrate.
   * This is for compatibility with ic960 R2.0, it is not known to be used
   * by current debuggers.  Emit this even if the aggregrate is small
   * enough to be returned in a register.
   */
  if (write_symbols == SDB_DEBUG && current_function_returns_struct)
  {
    char	*tag;
    int		base_type;
    tree	type;
    ...
    /* Not sure we need to do this checking, it's copied from sdbout_symbol */
    if (DECL_EXTERNAL(current_function_decl) ||
        GET_CODE(DECL_RTL (current_function_decl)) != MEM ||
        GET_CODE(XEXP (DECL_RTL (current_function_decl), 0)) != SYMBOL_REF)
      return;

    type = TYPE_MAIN_VARIANT(TREE_TYPE(DECL_RESULT(current_function_decl)));
    size = int_size_in_bytes(type);
    PUT_SDB_NO_UNDERSCORE_DEF(".target");
    PUT_SDB_INT_VAL(0);
    PUT_SDB_SCL(C_AUTO);
    tag = KNOWN_TYPE_TAG(type);
    PUT_SDB_TAG(tag);
    ...
    if (TREE_CODE(type) == RECORD_TYPE)
      base_type = T_STRUCT;
    PUT_SDB_TYPE(PUSH_DERIVED_LEVEL (DT_PTR, base_type));
    PUT_SDB_ENDEF;
  }
#endif
}
...
#else
  PUT_SDB_NO_UNDERSCORE_DEF (LABEL_NAME (insn));
#endif
  PUT_SDB_VAL (insn);
  PUT_SDB_SCL (C_LABEL);
  PUT_SDB_TYPE (T_NULL);
