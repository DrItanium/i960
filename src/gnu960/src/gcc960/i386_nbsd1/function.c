...
#ifdef IMSTG
rtx current_function_argptr_insn;
#endif
...
#ifdef IMSTG
  /*
   * We set this because if this is an array, then the implications of
   * MEM_IN_STRUCT_P are not true, and this is safe for the others also.
   */
  MEM_IN_STRUCT_UNSAFE (reg) = 1;
#endif
...
#ifdef IMSTG
		  COPY_MEM_MRD_FIELDS(newmem,tem);
#endif
     ...
     int second_time;
{
...
#ifdef IMSTG
  if (hide_last_arg) current_function_varargs = 1;
#endif
...
#ifdef IMSTG
#ifdef USE_PSEUDO_ARGPTR
       || USE_PSEUDO_ARGPTR(fndecl)
#endif
#endif
...
#ifdef IMSTG
  current_function_argptr_insn = get_last_insn();
#endif
  ...
  for (parm = fnargs; parm; parm = TREE_CHAIN (parm))
...
#if defined(IMSTG) && defined(REG_PARM_FAKE_STACK_SPACE)
          /* We need to keep track of stack size because of future non-reg
             parms, but there is not necessarily an argblock for this
             parm to live in if it came in in a register.  So, put
             it on the frame. */
          if (entry_parm && entry_parm != stack_parm)
            stack_parm = 0;
#endif
	}
      else
...
#ifdef IMSTG
      set_rtx_type (entry_parm, TREE_TYPE(parm));
      set_rtx_type (stack_parm, TREE_TYPE(parm));
#endif
...
#else
		  = embed_type (assign_stack_local (GET_MODE (entry_parm), size_stored, 0), TREE_TYPE(parm));
#endif
		  /* If this is a memory ref that contains aggregate components,
		     mark it as such for cse and loop optimize.  */
		  MEM_IN_STRUCT_P (stack_parm) = aggregate;
...
#ifdef IMSTG
          set_rtx_type (DECL_RTL(parm), TREE_TYPE(parm));
#endif
      ...
      else
	{
	  ...
	  DECL_RTL (parm) = stack_parm;
#ifdef IMSTG
          /* Bug fix for this not getting set correctly. */
          MEM_IN_STRUCT_P (DECL_RTL (parm)) = aggregate;
#endif
#ifdef IMSTG
          set_rtx_type (DECL_RTL(parm), TREE_TYPE(parm));
     ...
     struct args_size *arg_size_ptr;
{
  ...
  if (! in_regs)
    {
#if (defined(IMSTG) && defined(MAYBE_REG_PARM_STACK_SPACE))
...
#else
init_function_start (subr, filename, line, list_line)
#endif
     tree subr;
     char *filename;
     int line;
...
#else
  imstg_emit_line_note (filename, line, list_line, 0);
#endif
     ...
     int parms_have_cleanups;
{
...
#if defined(IMSTG) && defined(CHANGE_OPTIONS_FOR_FUNC)
  CHANGE_OPTIONS_FOR_FUNC(subr);
#endif
  ...
  emit_note (NULL_PTR, NOTE_INSN_FUNCTION_ENTRY);
#endif
...
#else
  imstg_emit_line_note_force (filename, line, LOOK_LPOS, 0);
#endif
