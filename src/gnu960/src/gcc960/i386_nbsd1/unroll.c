     ...
     int strength_reduce_p;
{
...
#ifdef IMSTG
  int loop_trip_count =
    (GET_CODE(loop_start) == NOTE &&
     NOTE_LINE_NUMBER(loop_start) == NOTE_INSN_LOOP_BEG) ?
      NOTE_TRIP_COUNT(loop_start) : 5 * PROB_BASE;
#endif
  ...
  {
    loop_trip_count = loop_n_iterations * PROB_BASE;
    if (GET_CODE(loop_start) == NOTE &&
        NOTE_LINE_NUMBER(loop_start) == NOTE_INSN_LOOP_BEG)
      NOTE_TRIP_COUNT(loop_start) = loop_trip_count;
  }
#endif
  ...
  if (unroll_number == 1)
	...
	unroll_number = 2;
#else
      if (loop_trip_count > (32 * PROB_BASE) &&
          8 * insn_count < MAX_UNROLLED_INSNS)
        unroll_number = 8;
      else if (loop_trip_count > (16 * PROB_BASE) &&
               4 * insn_count < MAX_UNROLLED_INSNS)
        unroll_number = 4;
      else if (loop_trip_count > (4 * PROB_BASE) &&
               2 * insn_count < MAX_UNROLLED_INSNS)
        unroll_number = 2;
#endif
...
#ifdef IMSTG
              JUMP_THEN_PROB(get_last_insn()) = PROB_BASE / unroll_number;
#endif
	    }
  ...
  if (unroll_type == UNROLL_NAIVE &&
      !(flag_unroll_all_loops ||
       (flag_unroll_small_loops && insn_count <= 10)))
#endif
    {
      if (loop_dump_stream)
...
#else
  /*
   * Turns out the above wasn't a massive enough overestimate.  In fact,
   * it was sometimes short. We smash a bigger one in here.  Really the
   * code that cares about this should be guarded, but it isn't, and its
   * very hard to find all that need to be.
   */
  new_maxregnum = maxregnum + ((temp+1) * unroll_number * 4);
#endif
...
#else
	  {
	    copy = imstg_emit_note (NOTE_SOURCE_FILE (insn),
				    NOTE_LINE_NUMBER (insn),
				    NOTE_LISTING_START (insn),
				    NOTE_LISTING_END (insn));
#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
	    if (flag_linenum_mods && copy)
	    {
...
#else
	  {
	    copy = imstg_emit_note (NOTE_SOURCE_FILE (insn), NOTE_LINE_NUMBER (insn),NOTE_LISTING_START(insn),NOTE_LISTING_END(insn));
#if defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
	    if (flag_linenum_mods && copy && NOTE_LINE_NUMBER(insn) >= 0)
	    {
	      NOTE_COPY_COLUMN_NUMBER(copy, insn);
	      NOTE_COPY_STMT_BEGIN(copy, insn);
	    }
...
#ifdef IMSTG
static int
splittable_address_ok(v, giv_inc, unroll_number)
struct induction *v;
rtx giv_inc;
int unroll_number;
{
  ...
  for (i = 1; i < unroll_number; i++)
    if (!memory_address_p (v->mem_mode,
			 ...
			 one fails, then can't do const elim here.  */
			  ...
			  && memory_address_p (v->mem_mode,
				       plus_constant (v->dest_reg,
						      ...
						      * (unroll_number - 1))))
#else
                      if (splittable_address_ok(v, giv_inc, unroll_number))
#endif
			{
...
#else
        /*
         * Actually givs are updated once for every time that the biv is
         * updated.  So set this to the biv_count.
         */
      splittable_regs_updates[REGNO (v->new_reg)] = bl->biv_count;
#endif
...
#ifdef IMSTG
                  /*
                   * above code doesn't even look for REG_EQUIV notes,
                   * which I would think would be much more likely, and
                   * which are important for us.
                   */
                  note = find_reg_note (insn, REG_EQUIV, NULL_RTX);

                  /* Only use REG_EQUIV if it is a constant. */
                  if (note && GET_CODE (XEXP (note, 0)) != EXPR_LIST
                      && CONSTANT_P (XEXP (note, 0)))
                    comparison_value = XEXP (note, 0);
#endif
