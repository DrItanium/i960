...
#ifdef IMSTG
      DECL_SOURCE_LINE (t) = PREV_LINE;
      DECL_SOURCE_FILE (t) = PREV_FILE;
      DECL_SOURCE_POS (t)  = PREV_LPOS;
#endif
...
#ifdef IMSTG
  /* The above comment is stupid.  The problem still exists on the
     conversion from unsigned into E.  See the use of FLOATUD
     below for the correct fix.  TMC Summer 1992 */
#endif
  ...
  if (TREE_INT_CST_HIGH (i) < 0 && ! TREE_UNSIGNED (TREE_TYPE (i)))
    {
      ...
      d *= e;
#ifdef IMSTG
      e = FLOATUD ((unsigned HOST_WIDE_INT) (~ TREE_INT_CST_LOW (i)));
#else
    ...
    {
#ifdef IMSTG
      d = FLOATUD ((unsigned HOST_WIDE_INT) TREE_INT_CST_HIGH (i));
#else
