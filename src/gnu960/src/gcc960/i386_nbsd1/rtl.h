...
#endif
#ifdef IMSTG
  unsigned int report_undef:1;
  unsigned int bit8_flag:1;
#else
...
#else
#define CALL_INSN_FUNCTION_USAGE(INSN)	((INSN)->fld[9].rtx)
#endif
...
#if defined(IMSTG) && !defined(DEV_465)
/*
 * This note is used to denote the first basic-block of a function,
 * and is used by various profiling, and global data base code to
 * communicate information about how many times this function was
 * callled.  Only expect 1 of these notes per function, and it should
 * be prior to any real insns in the function.
 */ 
#define NOTE_INSN_FUNCTION_ENTRY -14

/* A NOTE_INSN_DWARF_LE_BOUNDARY note marks the beginning or end of the valid
   range for one or more location expressions.  The note has one integer
   operand, accessed with NOTE_LE_BOUNDARY_NUMBER(N), which is used in the
   name of a label generated to mark the boundary.
   Such notes are inserted in the insn stream just before final(), and
   used by final().
 */
#define NOTE_INSN_DWARF_LE_BOUNDARY -15

/*
 * A NOTE_INSN_CALL_BEGIN marks the beginning of the code that sets up
 * the transmission registers for a function call.  Global inlining
 * algorithm uses this in attempting to perform parameter register
 * remapping.
 */
#define NOTE_INSN_CALL_BEGIN -16
#endif
...
#else
#define JUMP_LABEL(INSN)   ((INSN)->fld[8].rtx)
#endif
...
#ifdef IMSTG
#define MEM_IN_STRUCT_UNSAFE(R) ((R)->bit8_flag)
#define IS_CASE_MEM(RTX) ((RTX)->report_undef)
#define MEM_REALLY_IN_STRUCT(R) (MEM_IN_STRUCT_P(R) && !MEM_IN_STRUCT_UNSAFE(R))
#define MEM_REALLY_NOT_IN_STRUCT(R) (!MEM_IN_STRUCT_P(R) && !MEM_IN_STRUCT_UNSAFE(R))
#endif
...
#ifdef IMSTG
extern rtx imstg_copy_rtx		PROTO((rtx, rtvec *, rtvec *, rtvec *));
#endif
...
#ifdef IMSTG
extern rtx emit_call_insn_after         PROTO((rtx, rtx));
#endif
...
#ifdef IMSTG
extern rtx imstg_emit_line_note		PROTO((char *, int, int, int));
extern rtx imstg_emit_note		PROTO((char *, int, int, int));
#endif
...
#ifdef IMSTG
extern rtx prev_nonlinenote_insn	PROTO((rtx));
#endif
