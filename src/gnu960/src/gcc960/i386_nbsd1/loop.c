  ...
  for (insn = f; insn; insn = NEXT_INSN (insn))
	...
	max_loop_num++;
#ifdef IMSTG
      /*
       * make sure there aren't any line number notes between the
       * LOOP_END note, and whatever precedes it.  We just simply move
       * all consecutive line number notes preceding a LOOP_END after
       * it.  We do this because loop expects LOOP_END notes to
       * immediately follow the jump back to the top of the loop.
       * See calls to get_condition_for_loop in check_dbra_loop,
       * and unroll_loop.
       */
      if (GET_CODE (insn) == NOTE
          && NOTE_LINE_NUMBER (insn) == NOTE_INSN_LOOP_END)
      {
        ...
        rtx t_insn;
        for (t_insn = prev_insn; t_insn != 0; t_insn = prev_insn)
        {
          prev_insn = PREV_INSN(t_insn);
          if (GET_CODE(t_insn) != NOTE || NOTE_LINE_NUMBER(t_insn) < 0)
            break;
          reorder_insns(t_insn, t_insn, insn);
        }
      }
#endif
    }
...
#else
  /* make really sure we get tables big enough. */
  max_uid_for_loop = get_max_uid () * 2;
#endif
...
#ifdef IMSTG
  /*
   * reset max_uid_for_loop to exactly what we have used so that we
   * know which insns were created by loop after uid_luid assignment.
   */
     ...
     int nregs;
{
...
#ifdef IMSTG
		  /* Jeez! We want expensive constants hoisted. tmc */
		  && (!rtx_is_constant_p(SET_SRC(set)) ||
                       rtx_cost(SET_SRC(set), SET) <= 1)
#endif
  ...
  p = scan_start;
  while (p != end)
  { /* We remove VTOP notes so that in outer loops we can assume that VTOP
       notes refer to the loop being worked on. */ 
    if (GET_CODE(p)==NOTE && NOTE_LINE_NUMBER(p)==NOTE_INSN_LOOP_VTOP)
    { PUT_CODE (p, NOTE);
      NOTE_LINE_NUMBER (p) = NOTE_INSN_DELETED;
      NOTE_SOURCE_FILE (p) = 0;
    }
...
#if defined(IMSTG) && defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
  rtx p_note = 0;       /* line number note currently associated with insns
			   at the end of the preheader.
			 */
  rtx m_note;           /* line number note within loop that's associated
			   with an insn that's about to be moved.
			 */

  /* Create a new p_note, if needed, for an instruction that's about
     to be moved.  This macro assumes that m_note is set for the insn.
   */
#define IMSTG_MOVE_NOTE(insn, loop_start) \
  do { \
	  if (flag_linenum_mods && m_note \
	      && (!p_note || !NOTES_REP_SAME_SOURCE_P(p_note, m_note))   \
	      && ((p_note = emit_note_before(NOTE_LINE_NUMBER(m_note),   \
						loop_start)) != 0))      \
	  { \
	    NOTE_SOURCE_FILE(p_note) = NOTE_SOURCE_FILE(m_note); \
	    NOTE_COPY_COLUMN_NUMBER(p_note, m_note); \
	    NOTE_SET_INSTANCE_NUMBER(p_note,NOTE_GET_INSTANCE_NUMBER(m_note));\
	    NOTE_CLEAR_STMT_BEGIN(p_note); \
	    if (NOTE_STMT_BEGIN_P(m_note) && dwarfout_last_insn_p(insn,m_note))\
	      NOTE_SET_STMT_BEGIN(p_note); \
	  } \
  } while (0)
#endif
...
#if defined(IMSTG) && defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
	  if (flag_linenum_mods)
	    m_note = dwarfout_effective_note(p);
	  else
	    m_note = 0;
#endif
...
#if defined(IMSTG) && defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
		  IMSTG_MOVE_NOTE(m->insn, loop_start);
#endif
...
#if defined(IMSTG) && defined(DWARF_DEBUGGING_INFO) && DWARF_VERSION >= 2
		  /* Copy the effective note first, or it will be deleted. */
		  IMSTG_MOVE_NOTE(p, loop_start);
#endif
  ...
  for (insn = f; insn; insn = NEXT_INSN (insn))
	...
	if (this_loop_num != -1
	    ...
	    && get_max_uid () < max_uid_for_loop)
	  {
		...
		if (loc)
#else
		if (loc && get_max_uid() < max_uid_for_loop)
#endif
		  {
...
#ifdef IMSTG
          /*
           * If this jump exits multiple loops, then for each
           * enclosing loop that this jump exits, duplicate the LABEL_REF
           * and put it into the loop_number_exit_labels list for that loop.
           */
          {
            int t_loop = loop_num;
            while ((t_loop = loop_outer_loop[t_loop]) != -1 &&
                   dest_loop != t_loop)
            {
              rtx tlab_ref = copy_rtx(x);
              LABEL_OUTSIDE_LOOP_P (tlab_ref) = 1;
              LABEL_NEXTREF (tlab_ref) = loop_number_exit_labels[t_loop];
              loop_number_exit_labels[t_loop] = tlab_ref;
  ...
  if (! loop_iv_list)
    {
	 ...
	 strength reduction info available.  */
      ...
      if (flag_unroll_loops)
#else
      /* we don't bother unrolling a loop with volatile references. */
      if (flag_unroll_loops && !loop_has_volatile)
#endif
...
#ifdef IMSTG
              /*
               * get the jump probability off the jump at the
               * end of the loop
               */
              jump_prob = JUMP_THEN_PROB(PREV_INSN(loop_end));
#endif
