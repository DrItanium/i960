...
#else
	    error_with_file_and_line (input_filename, lineno,0,"stray '\\'");
#endif
	  c = getch ();
	  break;
...
#ifdef IMSTG
#ifdef HANDLE_PRAGMA
              HANDLE_PRAGMA(c, getch, put_back, (FILE *)0);
#endif
#endif
	    }
	  goto skipline;
...
#else
		  message_with_file_and_line (ER_ERROR, input_filename,lineno,0,
...
#else
  error_with_file_and_line (LOOK_FILE, LOOK_LINE, LOOK_LPOS, "%s (compiler error)", buf);
#endif
}
