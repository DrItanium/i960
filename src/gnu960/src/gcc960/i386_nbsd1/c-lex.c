...
/* Buffered-back input character; faster than using imstg_ungetc.  */
...
#ifdef IMSTG
  /* Note about asm handling:

       In ic960, asm is a keyword only if not hidden by a declaration.

       We implement this for both ic960 and gcc960;  this constitutes
       a minor extension for gcc960.

       To implement this, we handle asm, __asm, and __asm__ specially at
       IDENTIFIER scan time; they scan as ASM_KEYWORD iff there is no
       hiding declaration.  Furthermore, the 'identifier' production in
       the grammar goes to ASM_KEYWORD in addition to IDENTIFIER and
       TYPENAME.
                                                        tmc summer 1993
  */

  UNSET_RESERVED_WORD ("asm");
  UNSET_RESERVED_WORD ("__asm");
  UNSET_RESERVED_WORD ("__asm__");

  UNSET_RESERVED_WORD ("__attribute");
  UNSET_RESERVED_WORD ("__attribute__");
  UNSET_RESERVED_WORD ("__complex");
  UNSET_RESERVED_WORD ("__complex__");
  UNSET_RESERVED_WORD ("__imag");
  UNSET_RESERVED_WORD ("__imag__");
  UNSET_RESERVED_WORD ("__iterator");
  UNSET_RESERVED_WORD ("__iterator__");
  UNSET_RESERVED_WORD ("__real");
  UNSET_RESERVED_WORD ("__real__");
  UNSET_RESERVED_WORD ("iterator");
  UNSET_RESERVED_WORD ("complex");

  if (flag_ic960)
  {
    /* Currently implemented policy is that under -ic9690,
       the compiler will have exactly the same set of reserved
       non-underscored words as the real ic960 does, but that
       extra __ words are OK.  */

    UNSET_RESERVED_WORD ("inline");
    UNSET_RESERVED_WORD ("typeof");

#if 0
    UNSET_RESERVED_WORD ("__inline");
    UNSET_RESERVED_WORD ("__inline__");

    UNSET_RESERVED_WORD ("__typeof");
    UNSET_RESERVED_WORD ("__typeof__");

    UNSET_RESERVED_WORD ("__alignof");
    UNSET_RESERVED_WORD ("__alignof__");

    UNSET_RESERVED_WORD ("__extension__");
    UNSET_RESERVED_WORD ("__label__");

    UNSET_RESERVED_WORD ("__const");
    UNSET_RESERVED_WORD ("__const__");
    UNSET_RESERVED_WORD ("__signed");
    UNSET_RESERVED_WORD ("__signed__");
    UNSET_RESERVED_WORD ("__volatile");
    UNSET_RESERVED_WORD ("__volatile__");
#endif
  }
#endif
  if (flag_traditional)
  ...
  for (;;)
...
#else
	      char *s;
              s="unexpected carriage return (we only warn about the first one)";
              warning_with_file_and_line (input_filename, lineno, 0, s);
#endif
	      newline_warning = 1;
	    }
	  c = imstg_getc (finput);
	...
	case '\\':
	  ...
	  if (c == '\n')
	    ...
	    lineno++;
#else
	    (lineno++,(INC_LIST_LINE));
#endif
	  else
#ifndef IMSTG
	    error ("stray '\\' in program");
#else
          { char *s = "stray '\\' in program";
            error_with_file_and_line (input_filename, lineno, 0, s);
          }
#endif
	  c = imstg_getc (finput);
	  break;
...
#ifndef IMSTG
  imstg_ungetc (skip_white_space (c), finput);
#else
  c = skip_white_space(c);
  advance_tok_info();
  imstg_ungetc (c, finput);
#endif
}
...
check_newline ()
{
  ...
  if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))
    {
#if defined(IMSTG)
  /* This ain't really a token, but we need to advance the cursor so
     we're looking at the right place if there are messages... */
  advance_tok_info();
#endif
      if (c == 'p')
	{
	  if (imstg_getc (finput) == 'r'
	      && imstg_getc (finput) == 'a'
	      && imstg_getc (finput) == 'g'
	      && imstg_getc (finput) == 'm'
	      && imstg_getc (finput) == 'a'
	      && ((c = imstg_getc (finput)) == ' ' || c == '\t' || c == '\n'))
	    {
...
#ifdef HANDLE_PRAGMA
#if defined(IMSTG)
	      HANDLE_PRAGMA (c, imstg_getc, imstg_ungetc, finput);
#else
	      HANDLE_PRAGMA (c, finput);
#endif
#endif /* HANDLE_PRAGMA */
	      goto skipline;
	...
	{
	  if (imstg_getc (finput) == 'i'
	      && imstg_getc (finput) == 'n'
	      && imstg_getc (finput) == 'e'
	      && ((c = imstg_getc (finput)) == ' ' || c == '\t'))
	    goto linenum;
	}
      else if (c == 'i')
	{
	  if (imstg_getc (finput) == 'd'
	      && imstg_getc (finput) == 'e'
	      && imstg_getc (finput) == 'n'
	      && imstg_getc (finput) == 't'
	      && ((c = imstg_getc (finput)) == ' ' || c == '\t'))
...
linenum:
  ...
  if (token == CONSTANT
	  ...
	  && TREE_CODE (yylval.ttype) == INTEGER_CST)
	{
	  if (TREE_INT_CST_LOW (yylval.ttype) == 1)
		...
		= (struct file_stack *) xmalloc (sizeof (struct file_stack));
	      ...
	      if (debug_info_level == DINFO_LEVEL_VERBOSE
		  && write_symbols == DWARF_DEBUG)
#if !defined(IMSTG) || DWARF_VERSION <= 1
		dwarfout_start_new_source_file (input_filename);
#else
		dwarfout_start_new_source_file (input_filename, old_lineno);
#endif /* IMSTG */
#endif /* DWARF_DEBUGGING_INFO */
	...
	token = yylex ();
      ...
      && TREE_INT_CST_LOW(yylval.ttype)==9)
      { token = yylex();
        if (token != CONSTANT || TREE_CODE (yylval.ttype) != INTEGER_CST)
        { error ("invalid #line");
	  goto skipline;
          ...
          update_sinfo_map (TREE_INT_CST_LOW(yylval.ttype));
      }
#endif
    }
  else
 ...
 skipline:
  if (c == '\n')
    return c;
  while ((c = imstg_getc (finput)) != EOF && c != '\n');
  return c;
}

#ifdef HANDLE_SYSV_PRAGMA
...
readescape (ignore_ptr)
     int *ignore_ptr;
{
  register int c = imstg_getc (finput);
  register int code;
...
/*  yylloc.first_line = lineno; */
  ...
  switch (c)
    ...
    { extern int in_asm_ctrl;
      if (in_asm_ctrl != -1)
      { value = get_asm_ctrl_id (token_buffer, &yylval);
        break;
      }
    }
#endif
...
#ifdef IMSTG
	  if (lastiddecl == 0
	      && ((yylval.ttype==get_identifier("asm") &&!flag_no_asm)||
		  yylval.ttype==get_identifier("__asm") ||
		  yylval.ttype==get_identifier("__asm__")))
	      value = ASM_KEYWORD;
#endif
...
#define TOTAL_PARTS ((HOST_BITS_PER_WIDE_INT / HOST_BITS_PER_CHAR) * 2 + 2)
...
#else
	else if (num_chars != 1 && ! flag_traditional && !flag_ic960)
#endif
	...
	*p = 0;
	...
	token_buffer[1] = c1 = imstg_getc (finput);
	token_buffer[2] = 0;
