      ...
      if (modified || ! REGNO_QTY_VALID_P (regno))
	{
	  if (classp)
		  ...
		  && GET_MODE (classp->exp) == GET_MODE (x))
		{
		  ...
		  return 1;
#else
                  if (qty_mode[reg_qty[REGNO (classp->exp)]] == GET_MODE(x))
                  {
                    make_regs_eqv (regno, REGNO (classp->exp));
                    return 1;
                  }
...
#else
	  if (MEM_REALLY_IN_STRUCT (x)) hash_arg_in_struct = 1;
#endif
	}
  ...
  /* We can do some operations on integer CONST_DOUBLEs.  Also allow
	...
	return 0;
      ...
      if (code==FIX||code==UNSIGNED_FIX)
        code = (REAL_VALUE_NEGATIVE(d)) ? FIX : UNSIGNED_FIX;
#endif
...
#ifdef IMSTG
  /*
   * The code that is ifdefed out is completely illegal optimization for
   * unsigned.  While legal for signed arithmetic, we feel its in bad form
   * because the behavior differs from what the hardware would do and from
   * what constant folding would do in similiar situations.
   */
#else
...
#ifdef IMSTG
	    { /* Do not share replacements[j] into insn; that is, we make our
		 own copy for XEXP(x,i).  We do not perform the copy if we did
		 not change XEXP(x,i). */

	      if (old != XEXP(x,i))
		XEXP(x,i)=copy_rtx(XEXP(x,i));
#endif
	      break;
#ifdef IMSTG
	    }
...
#ifdef IMSTG
		    /* Do not share replacements[j] into insn;  that is, we make
		       our own copy for XEXP(x,1-i).  If XEXP(x,1-i) is the
		       same as the previous XEXP(x,i), then we did not change
		       anything (we only commuted operands) and thus we do
		       not copy XEXP(x,1-i). */

		    if (old != XEXP(x,1-i))
		      XEXP(x,1-i)=copy_rtx(XEXP(x,1-i));
#endif
...
#else
	    elt->in_struct = (MEM_REALLY_IN_STRUCT (sets[i].inner_dest)
#endif
...
#else
	  /* The comment above is horseshit.  Consider p[i], where i
             happens to be 0 and p is a pointer.  This most certainly
             can overwrite scalar variables. TMC Spring '94 */
	  if (GET_MODE(written)==QImode || !MEM_REALLY_IN_STRUCT (written))
#endif
	    writes_ptr->all = 1;
...
#else
      /* Allow for intervening line number notes.  Without this check,
	 we produce different results for ELF vs other OMFs.
       */
      && ((imstg_prev = prev_nonlinenote_insn(insn)) != NULL)
      && GET_CODE (imstg_prev) == JUMP_INSN
      && JUMP_LABEL (imstg_prev) != 0
      && LABEL_NUSES (JUMP_LABEL (imstg_prev)) == 1)
    cse_around_loop (JUMP_LABEL (imstg_prev));
#endif
