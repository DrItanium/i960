...
#else
  /*
   * first three fields are exactly same as tree_identifier node in tree.h,
   * they must be kept synced for complete portability, however cannot just
   * use struct tree_identifier because this could be a different size, and
   * some code accesses the extra fields assuming they immediately follow
   * these fields without the end padding.
   */
...
#endif
#ifdef IMSTG
/* So we can tell that a typedef was INT, not LONG, etc; we need this
   to implement ic960-style bit fields signedness (all fields except
   'int' get natural sign;  'int' becomes 'unsigned'.  Go figure.
                                                       TMC winter 93
*/
#define C_TYPEDEF_EXACTLY_INT(exp) DECL_LANG_FLAG_2 ((exp))
#endif
