 ...
 *	Arcsine, Arccosine
 *
 * $Header: /ffs/p1/dev/src/lib/libm/common/RCS/asin.c,v 1.2 1993/11/16 17:33:55 alex Exp $
 * $Locker:  $
 *
 */
#include	<fpsl.h>
...
/*
 * The arcsine is approximated using mathematical identity
 *	arcsin(x) = arctan(x / sqrt(1.0 - x*x))
 *
 * Note that the denominator is always >= 0, so the atan2() function
...
(asin)(GENERIC x)
{
	 ...
	 */
		...
		x = fp_satan2(rad, x);
	/*
	 * NOTE: fp_satan2 is a special version of atan2 function, where
	 *	 the first argument is always in extended precision.
	 */
		return fp_exit(x, env, FPSL_ASIN);
	}

	/* <<< is it legal for compiler to optimize cmpr out? >>> */
	if (fp_isnan(px))
		return fp_nan1(x, env, FPSL_ASIN);

	/*
	 * Domain error: abs(x) > 1.0
	 */
	errno = EDOM;
	return fp_faultexit(fp_copysign(qnan,x), FPX_INVOP, env, FPSL_ASIN);
}
