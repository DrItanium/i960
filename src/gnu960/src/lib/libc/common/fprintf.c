...
int fprintf(FILE *stream, const char *control, ...)
{
    va_list args;
    int i;
    int size, rw_flag;
    ...
    if (stream->_size <= 1) {
        ...
        save_base = stream->_base;
        stream->_base = (unsigned char *)buf;
        stream->_cnt = 128;
        stream->_ptr = (unsigned char *)buf;
        size = stream->_size;
        stream->_size = 128;
        rw_flag = stream->_flag;
        stream->_flag |= _IODIRTY;      /* update the buffer */
        _semaphore_signal(&stream->_sem);
    }

    i = _Ldoprnt(control, &args, stream, putc);

    if (buf_flag) {
        fflush(stream);
        _semaphore_wait(&stream->_sem);
        stream->_base = save_base;
        stream->_size = size;
        stream->_flag = rw_flag;
        _semaphore_signal(&stream->_sem);
    }
