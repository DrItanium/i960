...
int fflush(FILE *s)
{
    ...
    if (s->_base && s->_ptr != s->_base) {
        if (s->_flag & _IODIRTY) {
            if (write(s->_fd, (char *)s->_base, s->_ptr - s->_base) !=
                    (s->_ptr - s->_base)) {
		s->_flag |= _IOERR;
		result = EOF;
	    }
            ...
            /*
             * synchronize file and buffer ptr
             */
            lseek(s->_fd, (off_t)(-_map_length(s->_fd, s->_ptr, s->_cnt)), SEEK_CUR);
        }
