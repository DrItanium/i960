...
#ifdef DOS
/* The ~ will work, even on DOS, if $HOME is set. */
...
#else
  /* 
   * Using stat() and read() to slurp a file into a buffer doesn't
   * work very well in the DOS world.  The MSVC 2.0 libs are flat
   * buggy in this regard and the Metaware libs dump \r chars into
   * the buffer, which mightily confuses various portions of 
   * readline (particularly bind.c).
   *
   * Workaround using fgets().
   */
  {
    int  bl, bufcur, bufsiz;
    FILE *file_fp;

    file = -1;
    if (! (file_fp = fopen(openname, "r")))
      {
        free (openname);
        return (errno);
      }
    ...
    bufcur = finfo.st_size = 0;
    while (1)
      {
        if (bufsiz - bufcur < 256)
          {
            bufsiz *= 2;
            buffer  = (char *) xrealloc(buffer, bufsiz);
          }
        if (fgets(buffer + bufcur, bufsiz - bufcur - 1, file_fp) != NULL)
          {
            bl             = strlen(buffer + bufcur);
            finfo.st_size += bl;
            bufcur        += bl;
          }
        else
