...
#if defined(DOS)
/* Change every backslash in "name" to a forward slash. */
static void
fix_slash(char *name)
{
    char *cp;
...
#if defined (__HIGHC__) || defined (WIN95)
		/* 
		 * Metaware/MSC libs return filenames (usually) in UC,
		 * while the user probably typed in LC.  Compensate. 
		 */
		(strnicmp (filename, entry->d_name, filename_len) == 0))
#else
      ...
      else
	{
...
#if defined(__HIGHC__) || defined(WIN95)
      /* 
       * Metaware/MSC libs return filenames (usually) in UC, while the user
       * probably typed his/her commands in LC.  Compensate by forcing the
       * matched filenames to the same case as the user's input (based on a
       * rather naive heuristic).  This seems to be an okay thing to do for
       * hosts that have case insensitive file systems and it sure makes
       * TAB completion work a whole lot better for filenames!  Without
       * this change, users attempting file completion using lowercase
       * keystrokes would be forced to hit the tab key three times to
       * obtain a list of all matching filenames.
       */

      {
        ...
        if (text[0])
        {
          if (! text[1])
          {
            /* Only have one character of input on which to base decision. */
          ...
          {
            /* Have have at least 2 chars of input on which to base decision. */
            if (isupper(text[0]) && isupper(text[1]))
                lc = 0;
          }
        }
        for (cp = temp; *cp; cp++)
          *cp = (lc) ? tolower(*cp) : toupper(*cp);
      }
#endif
      return (temp);
    }
