...
#if defined (__GO32__) && defined (IMSTG)
...
#      include "win95.c"
#   endif
#endif      /* defined (__GO32_) && defined (IMSTG) */
...
#if defined (__GO32__) && defined (__HIGHC__) 
  _rl_highc_init();
#endif
#if defined (__GO32__) && defined (WIN95) 
  _rl_win95_init();
#endif
     ...
     int tty, ignore_env;
{
  ...
  if (!term_xn)
    ...
    /* Crude, but effective hack here.  Shorten the screen width by one
     * more column so that readline routines will properly insert and
     * delete characters without (and this is very important) resorting
     * to the use of special termcap strings (which we don't have).
     */
   if (screenwidth)
     screenwidth--;
#endif
...
#if defined(__GO32__) && defined(IMSTG)
/*
 * Add function to arbitrarily erase the current line.  Bound to ^C
 * in non-Unix based implementations because SIGINT is ignored while
 * readline is active.
 */
rl_feeble_cntlc()
{
...
#if defined (__HIGHC__) && defined (IMSTG)

  QUIT();             /* See comments in highc.c re: QUIT() macro. */
...
#if defined(IMSTG) && ! defined(__MSDOS__)
